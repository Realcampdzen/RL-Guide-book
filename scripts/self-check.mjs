...
console.log(
  devPort === vitePort && vitePort === batPort ? 'OK: ports are consistent' : 'WARN: ports mismatch'
);
...
console.log(
  `public/ai-data/MASTER_INDEX.json: ${existsSync(masterIndex) ? 'present' : 'MISSING (run: npm run sync:ai-data)'}`
);
const assets = [
...
console.log(
  appInline.includes('/ai_camp.png')
    ? 'OK: intro bg resolved to /ai_camp.png'
    : 'WARN: intro bg not updated'
);
console.log(
  appInline.includes('pattern_stickers.jpg') ? 'OK: benefit bg set' : 'WARN: benefit bg not set'
);

// Duplicate inline CSS vs public CSS
section('Styles duplication');
console.log(
  existsSync(resolve(root, 'public/app-inline.css')) ? 'NOTE: Public app-inline.css exists' : ''
);
console.log(
  existsSync(resolve(root, 'src/styles/app-inline.css')) ? 'NOTE: Src app-inline.css exists' : ''
);
...
if (env) {
  ...
  console.log(
    hasSecrets
      ? 'WARN: .env contains real-looking secrets — rotate and keep local only'
      : '.env present'
  );
} else {
