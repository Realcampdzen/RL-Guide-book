...
const outCampFactsPath = path.join(root, 'cf-api', 'src', 'neurovalyusha', 'generated_camp_facts.ts')
...
console.log('Synced chatbot/prompts/facts.json -> cf-api/src/neurovalyusha/generated_camp_facts.ts')
...
const outChatPromptPath = path.join(root, 'cf-api', 'src', 'neurovalyusha', 'generated_chat_prompt.ts')
...
try {
  ...
  if (e.status === 1 && e.message && e.message.includes('python')) {
    try {
      ...
      console.error('Failed to run Python export script. Ensure Python 3 is available (python or py).', e2.message)
      process.exit(1)
    }
  } else {
