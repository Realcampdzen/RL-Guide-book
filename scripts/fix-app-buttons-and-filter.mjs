...
replaceBlock(
  /<button\s+onClick=\{handleBackToIntro\}\s+className="back-button">[\s\S]*?<\/button>/,
  '<button onClick={handleBackToIntro} className="back-button">\n              ← Назад к категориям\n            </button>'
);

replaceBlock(
  /<button\s+onClick=\{handleBackToCategories\}\s+className="back-button">[\s\S]*?<\/button>/,
  '<button onClick={handleBackToCategories} className="back-button">\n            ← Назад к категориям\n          </button>'
);

replaceBlock(
  /<button\s+onClick=\{handleBackToCategoryFromAdditional\}\s+className="back-button">[\s\S]*?<\/button>/,
  '<button onClick={handleBackToCategoryFromAdditional} className="back-button">\n            ← Назад к категории\n          </button>'
);

replaceBlock(
  /<button\s+onClick=\{handleBackToCategoryFromIntroduction\}\s+className="back-button">[\s\S]*?<\/button>/,
  '<button onClick={handleBackToCategoryFromIntroduction} className="back-button">\n            ← Назад к категории\n          </button>'
);

replaceBlock(
  /<button\s+onClick=\{handleBackToAboutCamp\}\s+className="back-button">[\s\S]*?<\/button>/,
...
replaceBlock(
  /const\s+categoryBadges\s*=\s*badges\.filter\([\s\S]*?\);/,
