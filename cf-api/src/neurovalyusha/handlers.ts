...
import { type BadgeData, type BadgeFields, loadBadgeData } from './ai_data_loader';
import { buildSystemPrompt, FORBIDDEN_EMOJIS, NEUROVALYUSHA_MODEL } from './constants';
import { type BadgeIndexEntry, loadBadgeIndex, scoreBadges } from './guidebook_index';
...
function classifyBadgeIntent(params: {
  triggerText: string;
  hasBadgeHint: boolean;
}): SocialBadgeIntent {
...
async function selectSocialBadgeDecision(params: {
  ...
  const lastAssistant = threadMemory
    ? ([...threadMemory].reverse().find((m) => m.role === 'assistant') ?? null)
    : null;
  const lastAssistantHasBadge = Boolean(
    lastAssistant && extractExplicitBadgeId(lastAssistant.content)
  );
  ...
  const intent = classifyBadgeIntent({
    triggerText,
    hasBadgeHint: triggerHasBadgeWord || lastAssistantHasBadge,
  });
...
function formatSocialBadgeContextStandard(badge: BadgeData, intent: SocialBadgeIntent): string {
  const lines: string[] = [];
  lines.push(
    'Памятка: значки — маршруты развития; опыт важнее значка; выбор пути за участником. Не выдумывай критерии/уровни — опирайся только на данные ниже; если данных не хватает — уточни.'
  );
...
async function pushRecentBadgeId(
  kv: KVNamespace | undefined,
  key: string,
  badgeId: string
): Promise<void> {
...
async function generateValyushaText(
  ...
  const fallbackText =
    'Классная мысль! 💜 А как вы думаете, какой 4K‑навык тут прокачивается сильнее всего?';
  ...
  const proxyBaseUrlConfigured = isNonEmptyString(env.OPENAI_PROXY_BASE_URL)
    ? env.OPENAI_PROXY_BASE_URL
    : undefined;
  const proxyTokenConfigured = isNonEmptyString(env.OPENAI_PROXY_TOKEN)
    ? env.OPENAI_PROXY_TOKEN
    : undefined;
...
export function getVkConfirmationResponse(
  env: NeuroValyushaBindings,
...
export async function processVkCallbackEvent(
  env: NeuroValyushaBindings,
  payload: VkCallbackPayload
): Promise<void> {
  ...
  try {
    ...
    if (type === 'wall_post_new') {
      ...
      const ownerId =
        Number(object?.owner_id) ||
        (isNonEmptyString(env.VK_GROUP_ID) ? -Number(env.VK_GROUP_ID) : 0);
      ...
      if (!imageUrl) {
        imageUrl = await vkTryFetchBestPostPhotoUrl({
      ...
      const badgeSystem: OpenAIChatMessage =
        usedBadgeId && usedBadgeTitle
          ...
          : {
              role: 'system',
              content:
                'Для этого комментария значок не подходит — НЕ упоминай значки Путеводителя.',
            };
      ...
      const generated = await generateValyushaText(env, aiMessages, {
        platform: 'vk',
        kv,
        failMode: 'skip',
      });
      ...
      if (commentId) {
        ...
        await kvPutJson(
          ...
          'nv:vk:lastWallPostNew',
          {
            ts: nowTs(),
            ok: false,
            reason: 'vk_create_comment_failed',
            ownerId,
            postId,
            badgeId: usedBadgeId,
          },
    ...
    if (type === 'wall_reply_new') {
      ...
      if (
        ...
        isNonEmptyString(env.VK_GROUP_ID) &&
        fromId === -Number(env.VK_GROUP_ID)
      )
        return;
      ...
      if (newCommentId) {
        await kvPutText(kv, `nv:vk:myComment:${newCommentId}`, '1', {
...
async function vkCreateComment(params: {
  ...
  if (
    typeof replyToCommentId === 'number' &&
    Number.isFinite(replyToCommentId) &&
    replyToCommentId > 0
  ) {
...
async function getOrUpdateTelegramMediaGroupRootId(params: {
  ...
  const next =
    Number.isFinite(existing) && existing > 0 ? Math.min(existing, messageId) : messageId;
...
function computeTelegramPostIdentity(msg: TgMessage): string {
  ...
  if (
    ...
    Number.isFinite(msg.forward_from_message_id)
  ) {
...
function pickLargestTgPhoto(msg: TgMessage): { file_id: string; score: number } | null {
  ...
  const score =
    typeof largest.file_size === 'number' && Number.isFinite(largest.file_size)
      ? largest.file_size
...
async function upsertTelegramMediaGroupCtx(params: {
  ...
  if (photo) {
    const existingScore =
      typeof existing.photoScore === 'number' && Number.isFinite(existing.photoScore)
        ? existing.photoScore
        : -1;
  ...
  try {
    ...
    if (msg.is_automatic_forward) {
      const originChat: any =
        ...
        (msg as any).sender_chat ||
        (msg as any).forward_origin?.chat ||
        null;
      ...
      await kvPutText(kv, `nv:tg:root:${chatId}:${msg.message_id}`, String(rootId), {
        ttlSeconds: 60 * 60 * 24 * 60,
      });
      ...
      if (already) {
        await kvPutJson(
          kv,
          'nv:tg:lastAutoForward',
          { ...debugBase, decision: 'skip_already', existing: already },
          { ttlSeconds: 60 * 60 * 24 * 14 }
        );
        return;
      }
      ...
      if (await kvIsDuplicate(kv, postKey, { ttlSeconds: 120 })) {
        await kvPutJson(
          kv,
          'nv:tg:lastAutoForward',
          { ...debugBase, decision: 'skip_locked' },
          { ttlSeconds: 60 * 60 * 24 * 14 }
        );
        return;
      }
      ...
      if (sent?.message_id) {
        ...
        await kvPutText(kv, `nv:tg:myMessage:${chatId}:${sent.message_id}`, '1', {
          ttlSeconds: 60 * 60 * 24 * 60,
        });
        if (usedBadgeId) await pushRecentBadgeId(kv, 'nv:tg:recentBadges', usedBadgeId);
      }

      await kvPutText(kv, `nv:tg:root:${chatId}:${rootId}`, String(rootId), {
        ttlSeconds: 60 * 60 * 24 * 60,
      });
      await appendConversationMemory(kv, conversationKey, {
        role: 'assistant',
...
async function resolveTelegramRootId(
  kv: KVNamespace | undefined,
  chatId: number,
  parent: TgMessage
): Promise<number> {
