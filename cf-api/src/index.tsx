...
import { type BadgeData, loadBadgeData } from './neurovalyusha/ai_data_loader';
import {
  buildSystemPrompt,
  FORBIDDEN_EMOJIS,
  NEUROVALYUSHA_FULL_CHAT_PROMPT,
  type PromptMode,
} from './neurovalyusha/constants';
import {
  ...
  return c.render(
    <div>
          ...
          <div class="container mx-auto px-6 py-4">
            <div class="flex items-center justify-between">
              ...
              <div class="hidden md:flex items-center space-x-6">
                <a href="#about" class="hover:text-indigo-200 transition">
                  О нас
                </a>
                <a href="#services" class="hover:text-indigo-200 transition">
                  Услуги
                </a>
                <a href="#works" class="hover:text-indigo-200 transition">
                  Работы
                </a>
                <a href="#contact" class="hover:text-indigo-200 transition">
                  Контакты
                </a>
              </div>
            </div>
      ...
      <main class="pt-24">
        <section
          id="hero"
          class="min-h-screen bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 text-white flex items-center justify-center"
        >
          <div class="container mx-auto px-6 text-center">
            <h1 class="text-5xl md:text-7xl font-bold mb-6">
              Персональные AI-решения
              <br />
              для вашего бизнеса
            </h1>
            <p class="text-xl md:text-2xl mb-12 max-w-3xl mx-auto">
              Создаем умных ботов, автоматизируем процессы и внедряем нейросети. Персональный подход
              и результат 24/7.
            </p>
            <button
              onclick="openChat()"
        ...
        <section id="about" class="py-20 bg-white">
          <div class="container mx-auto px-6">
            <h2 class="text-4xl font-bold text-center mb-12 text-gray-800">Что мы делаем</h2>
            <div class="grid md:grid-cols-3 gap-8">
              ...
              <div class="text-center p-6 rounded-lg border-2 border-indigo-100 hover:border-indigo-300 transition">
                ...
                <p class="text-gray-600">Оптимизация бизнес-процессов с помощью нейросетей</p>
              </div>
              <div class="text-center p-6 rounded-lg border-2 border-indigo-100 hover:border-indigo-300 transition">
                <div class="text-5xl mb-4">🎯</div>
                <h3 class="text-2xl font-semibold mb-4">Интеграция</h3>
                <p class="text-gray-600">Внедрение AI-решений в существующие системы</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="services" class="py-20 bg-gray-50">
          <div class="container mx-auto px-6">
            <h2 class="text-4xl font-bold text-center mb-12 text-gray-800">Наши услуги</h2>
            <div class="max-w-4xl mx-auto space-y-6">
              <div class="bg-white p-8 rounded-lg shadow-lg hover:shadow-xl transition">
                ...
                <p class="text-gray-600 mb-4">
                  Создаем уникальных AI-помощников с характером для вашего бизнеса. От простых
                  чат-ботов до сложных персонажей с памятью и эмоциями.
                </p>
              ...
              <div class="bg-white p-8 rounded-lg shadow-lg hover:shadow-xl transition">
                ...
                <p class="text-gray-600 mb-4">
                  Анализируем ваши процессы и создаем индивидуальные AI-решения. Telegram-боты,
                  веб-сервисы, интеграции с API.
                </p>
                <div class="text-sm text-gray-500">
        ...
        <section id="contact" class="py-20 bg-indigo-600 text-white">
          <div class="container mx-auto px-6 text-center">
            <h2 class="text-4xl font-bold mb-8">Готовы начать?</h2>
            ...
            <div class="flex items-start space-x-2">
              ...
              <div class="bg-white p-3 rounded-lg shadow-sm max-w-[80%]">
                <p class="text-sm text-gray-800">
                  ...
                  <br />
                  <br />Я здесь, чтобы рассказать о студии и показать, как работают AI-боты. Задавай
                  вопросы!
                </p>
              </div>
            </div>
...
app.post('/api/hipych/chat', async (c) =>
  handleBotChat(c, HIPYCH_SYSTEM, getHipychFallbackResponse)
);
...
const CHAT_ALLOWED_ROLES = new Set([
  'participant',
  'parent',
  'counselor',
  'shift_leader',
...
app.post('/api/putevoditel/chat', async (c) => {
  const secret =
    typeof c.env.AUTH_JWT_SECRET === 'string' ? c.env.AUTH_JWT_SECRET.trim() : undefined;
  if (secret) {
...
app.post('/api/vk/callback', async (c) => {
  ...
  if (!isValidVkRequest(c.env, payload)) {
    ...
    if (
      expectedGroupId &&
      Number.isFinite(expectedGroupId) &&
      payloadGroupId !== null &&
      payloadGroupId !== expectedGroupId
    ) {
...
function formatLevelsSummary(levels: BadgeData['levels']): string {
  ...
  const items = levels
    .map((level) => {
      ...
      const label =
        typeof level.level === 'string' || typeof level.level === 'number'
          ? String(level.level)
          : '';
...
async function handleBotChat(
  ...
  try {
    const replyRaw =
      (await callOpenAI(apiKey, resolvedSystemPrompt, message, {
