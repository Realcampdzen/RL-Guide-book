...
**📋 Полная инструкция по обновлению:** См. [UPDATE_GUIDEBOOK_CHECKLIST.md](UPDATE_GUIDEBOOK_CHECKLIST.md) — пошаговый чек-лист для синхронизации данных Путеводителя при любых изменениях в `ai-data` (пересчёт индексов → синхронизация → генерация → деплой).
