...
- **Режим работы:** выполнение плана по задаче «Создание смен/отрядов руководителем смены» из [FEATURE_AUTH_ROLES_DVIZHKI_PLAN.md](FEATURE_AUTH_ROLES_DVIZHKI_PLAN.md) §7–8; реализация по приложенному плану (staff_flow_shifts_squads_mvp).
...
- **Цель:** фундамент «смена → отряды → коды» для онлайн-Движков; руководитель смены создаёт смены/отряды и выдаёт коды без глобального секрета в клиенте.
- **Уже было:** роли (shift_leader, counselor), JWT, verify-code, generate-code по секрету, counselor-squad (локально).
- **Не было:** уровень «смена», API смен/отрядов, staff‑панель.
...
- **Эндпоинты** (все требуют JWT shift_leader или developer):
...
- **[src/types/authRole.ts](../src/types/authRole.ts):** добавлена функция `canCreateShiftsAndSquads(role)`.
- **[src/utils/authStorage.ts](../src/utils/authStorage.ts):** добавлена миграция legacy роли `organizer` → `shift_leader`.
- **[src/views/ProfileView.tsx](../src/views/ProfileView.tsx):**
  - Импорт и использование `canCreateShiftsAndSquads`.
  - Состояние: список смен, карта отрядов по сменам, формы и модалки для смены/отряда/кода.
  - Загрузка: при роли shift_leader и наличии accessToken — GET /api/shifts, затем для каждой смены GET /api/shifts/:id/squads.
...
- Руководитель смены по JWT создаёт смены и отряды через API и через ЛК.
- Руководитель смены в ЛК выдаёт код (deviceId, роль, смена); код верифицируется через существующий verify-code и выдаёт JWT с ролью и campId = shiftId.
...
- Добавлена функция `_require_organizer_jwt()` после `_require_parent_snapshot_auth()` (проверка Bearer JWT, роль shift_leader или developer; legacy organizer маппится в shift_leader).
...
- Добавлена функция `canCreateShiftsAndSquads(role)` (возвращает true для shift_leader и developer).
...
- В блок отображения роли в паспорте добавлена подпись для `shift_leader`.
...
- Созданы и отмечены выполненными:
  ...
  3. Frontend: authRole + authStorage (legacy organizer→shift_leader), canCreateShiftsAndSquads
...
- Учтена миграция legacy роли `organizer` → `shift_leader`.
