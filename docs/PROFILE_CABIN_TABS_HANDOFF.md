...
- В кабине `counselor-squad` переведен на docked-tabs:
  - левый docked-nav с 5 вкладками: `Отряд`, `Фото`, `Планёрка`, `Беспорядок дня`, `Значки на флаг`;
...
- Источник данных: `CounselorSquadContext`, localStorage по `squadId` (`RL_COUNSELOR_SQUAD_CARDS_V1`). Таб «Беспорядок дня» использует `diaryProgress.shiftSchedule` и `myActivities` (общий источник с Реальным дневником).
...
8. `panelActiveView === 'counselor-squad'`: 5 левых табов (`Отряд`, `Фото`, `Планёрка`, `Беспорядок дня`, `Значки на флаг`), header скрыт, без карточной обёртки.
