...
test.describe('Performance & Chunking Tests', () => {
  test('Measure FCP, LCP and Profile Chunk Load Time', async ({ page }) => {
    ...
    console.log(
      `[Performance] FCP (First Contentful Paint): ${fcp ? fcp.startTime.toFixed(2) : 'N/A'} ms`
    );

    // Simulate Hover over Profile Button to trigger Prefetch
    const profileBtn = page
      .locator('.mobile-nav-item')
      .filter({ hasText: 'Мой путь' })
      .or(page.locator('.mobile-nav-item').filter({ hasText: 'Войти' }));
    ...
    console.log(
      `[Performance] Time to Render Profile View after click (with prefetch): ${profileRenderTime} ms`
    );
