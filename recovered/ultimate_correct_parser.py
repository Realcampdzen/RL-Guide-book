...
# -*- coding: utf-8 -*-
"""
УЛЬТИМАТИВНЫЙ ПРАВИЛЬНЫЙ ПАРСЕР
...
class UltimateCorrectParser:
    ...
    @staticmethod
    def canonicalize_level(level_value: Optional[str]) -> str:
        """Приводит значение уровня к человеко‑читаемому виду"""
        if level_value is None:
            return ''
        raw = str(level_value).strip()
        low = raw.lower()
        if low in {'1', 'базовый', 'базовый уровень'}:
            return 'Базовый уровень'
        if low in {'2', 'продвинутый', 'продвинутый уровень'}:
            return 'Продвинутый уровень'
        if low in {'3', 'экспертный', 'экспертный уровень', 'вожатский', 'вожатский уровень'}:
    ...
    def process_ai_data_file(self, file_path: Path):
        ...
        try:
            ...
            # Если не нашли в Путеводителе, используем AI‑данные как надёжный фоллбек
            if not guide_info:
                print(f"⚠️ Не найдена информация для {badge_id} в Путеводителе — берём из ai-data")
                badge = Badge(
                    id=badge_id,
                    title=data.get('title', ''),
                    emoji=str(data.get('emoji', '') or ''),
                    category_id=data.get('categoryId', ''),
                    level=self.canonicalize_level(data.get('level', 'Одноуровневый')),
                    description=data.get('description', ''),
                    importance=data.get('importance', ''),
                    skill_tips=data.get('skillTips', ''),
                    criteria=str(data.get('criteria', '') or ''),
                    confirmation=str(data.get('confirmation', '') or ''),
                )
            else:
                # Создаём значок из Путеводителя
                badge = Badge(
                    id=badge_id,
                ...
                for level_data in data['levels']:
                    ...
                    if level_guide_info:
                        subtask = Badge(
                            ...
                            category_id=data.get('categoryId', ''),
                            level=self.canonicalize_level(level_data.get('level', '')),
                        ...
                        # Фоллбек на AI‑данные для уровня
                        subtask = Badge(
                            id=level_id,
                            title=level_data.get('title', data.get('title', '')),
                            emoji=str(level_data.get('emoji', data.get('emoji', '')) or ''),
                            category_id=data.get('categoryId', ''),
                            level=self.canonicalize_level(level_data.get('level', '')),
                            description=data.get('description', ''),  # уровневые описания в ai‑data общие у значка
                            importance=data.get('importance', ''),
                            skill_tips=data.get('skillTips', ''),
                            criteria=str(level_data.get('criteria', '') or ''),
                            confirmation=str(level_data.get('confirmation', '') or ''),
                        )
                        badge.subtasks.append(subtask)
...
def main():
    parser = UltimateCorrectParser("ai-data", "Путеводитель.txt")
    result = parser.parse()
    
    if result:
        # Сохраняем результат
        output_file = "perfect_parsed_data_correct.json"
