...
s = s.replace(
  ...
  "text: 'Произошла ошибка сети. Проверьте, что бэкенд запущен.',\n          isUser: false,\n          timestamp:"
);

// Loading label
s = s.replace(
  /<span className="chatbot-loading-text">[\s\S]*?<\/span>/,
  '<span className="chatbot-loading-text">Чат-бот думает...</span>'
);

// Input placeholder
s = s.replace(
  /placeholder="[^"]*"\s*\n\s*className="chatbot-input"/,
  'placeholder="Введите сообщение..."\n              className="chatbot-input"'
);

// Send button label
s = s.replace(
  /className="chatbot-send-btn"[\s\S]*?<\/button>/,
...
s = s.replace(
  /src="\/[^"]*\.jpg"\s*\n\s*\s*alt="[^"]*"/,
  'src="/badges_photo.jpg"\n                  alt="Чат-бот"'
);
s = s.replace(
  /<h3 className="chatbot-welcome-title">[\s\S]*?<\/h3>/,
  '<h3 className="chatbot-welcome-title">\n                Привет! 👋\n              </h3>'
);
s = s.replace(
  /<p className="chatbot-welcome-text">[\s\S]*?<\/p>/,
  '<p className="chatbot-welcome-text">\n                Я ваш помощник.\n              </p>'
);
s = s.replace(
  /<p className="chatbot-welcome-subtext">[\s\S]*?<\/p>/,
