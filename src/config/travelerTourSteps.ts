...
export function getTravelerTourSteps(controller: AppController): HintStep[] {
  return [
    ...
    },
    {
      ...
      content: 'Здесь реализован мощный пульт управления всей системой! Родители видят достижения детей и могут забронировать путёвки. Вожатые координируют прогресс участников с программой лагеря «на земле», апрувят действия и используют генератор отрядных дел. Старший вожатый создаёт вожатский отряд с кабинетом и чатами для педколлектива. А для кружководов, тренеров и ведущих творческих мастерских предусмотрены свои цифровые инструменты и планировщики.',
