...
async function requestJson<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      throw new ApiError('Сервер долго не отвечает. Повторите попытку.', 408, {
...
export async function loadMyBadgeRequests(
  accessToken: string,
...
export async function loadBadgeRequestsInbox(
  ...
  const headers: Record<string, string> = accessToken
    ? { Authorization: `Bearer ${accessToken}` }
    : {};
  const data = await requestJson<{ requests: BadgeRequestItem[] }>(
    `/api/badges/requests/inbox${suffix}`,
    {
      headers: { ...headers, ...extraHeaders },
    }
  );
...
export async function approveBadgeRequest(
  accessToken: string,
  requestId: string,
  note?: string,
  extraHeaders?: Record<string, string>
  ...
  const data = await requestJson<{ request: BadgeRequestItem }>(
    `/api/badges/requests/${encodeURIComponent(requestId)}/approve`,
    {
      method: 'POST',
      headers,
...
export async function rejectBadgeRequest(
  accessToken: string,
  requestId: string,
  note?: string,
  extraHeaders?: Record<string, string>
  ...
  const data = await requestJson<{ request: BadgeRequestItem }>(
    `/api/badges/requests/${encodeURIComponent(requestId)}/reject`,
    {
      method: 'POST',
      headers,
      body: JSON.stringify(note ? { note } : {}),
...
export async function joinSquad(
  ...
  return requestJson<{ membership: SquadMembership; squad: SquadMeta }>(
    `/api/squads/${encodeURIComponent(squadId)}/join`,
    {
      method: 'POST',
...
export async function createSquadJoinRequest(
  ...
  return requestJson<{ status: string; request: SquadJoinRequestItem | null }>(
    `/api/squads/${encodeURIComponent(squadId)}/join-requests`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
      body: JSON.stringify(payload || {}),
...
export async function loadMySquadJoinRequests(
  accessToken: string
): Promise<SquadJoinRequestItem[]> {
  const data = await requestJson<{ requests: SquadJoinRequestItem[] }>(
    '/api/squads/join-requests/mine',
    {
      headers: { Authorization: `Bearer ${accessToken}` },
    }
  );
...
export async function fetchSquadCorner(
  accessToken: string,
  squadId: string
...
export async function createSquadInviteCode(
  accessToken: string,
  squadId: string
): Promise<SquadInviteCodeResponse> {
  return requestJson<SquadInviteCodeResponse>(
    `/api/squads/${encodeURIComponent(squadId)}/invite-code`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
      body: JSON.stringify({}),
...
export async function resolveSquadByInviteCode(
  accessToken: string,
  code: string
): Promise<SquadInviteResolveResponse> {
  const params = new URLSearchParams({ code });
  return requestJson<SquadInviteResolveResponse>(
    `/api/squads/by-invite-code?${params.toString()}`,
    {
      headers: { Authorization: `Bearer ${accessToken}` },
    }
  );
...
export async function fetchSquadPreview(
  accessToken: string,
  squadId: string
): Promise<SquadPreviewResponse> {
...
export async function kickSquadMember(
  ...
  await requestJson<{ squadId: string }>(
    `/api/squads/${encodeURIComponent(squadId)}/members/${encodeURIComponent(targetDeviceId)}`,
    {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${accessToken}` },
    }
...
export async function leaveSquad(
  ...
  return requestJson<{ status: string; squadId: string; membership: null }>(
    `/api/squads/${encodeURIComponent(squadId)}/leave`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
      body: JSON.stringify({}),
...
export async function fetchSquadMessages(
  ...
  const headers: Record<string, string> = accessToken
    ? { Authorization: `Bearer ${accessToken}` }
    : {};
  return requestJson<{ squadId: string; messages: SquadMessage[]; hasMore?: boolean }>(
    `/api/squads/${encodeURIComponent(squadId)}/messages${suffix}`,
    {
      headers: { ...headers, ...extraHeaders },
    }
  );
...
export async function postSquadMessage(
  ...
  const created = await requestJson<{ squadId?: string; message: SquadMessage }>(
    `/api/squads/${encodeURIComponent(squadId)}/messages`,
    {
      method: 'POST',
      headers,
      body: JSON.stringify(payload),
...
export async function deleteShift(
  ...
  return requestJson<{ ok: true; deleted: Record<string, number> }>(
...
export async function deleteSquadMessage(
  ...
  return requestJson<{ ok: true }>(
    `/api/squads/${encodeURIComponent(squadId)}/messages/${encodeURIComponent(msgId)}`,
    {
      method: 'DELETE',
      headers: { ...headers, ...extraHeaders },
    }
...
export async function pinSquadMessage(
  accessToken: string,
  squadId: string,
  msgId: string,
  pinned: boolean,
  extraHeaders?: Record<string, string>
): Promise<{ ok: true; pinned: boolean; message: SquadMessage | null }> {
  const headers: Record<string, string> = {
    'Content-Type': 'application/json',
    ...(accessToken ? { Authorization: `Bearer ${accessToken}` } : {}),
    ...extraHeaders,
  };
  return requestJson<{ ok: true; pinned: boolean; message: SquadMessage | null }>(
    `/api/squads/${encodeURIComponent(squadId)}/messages/${encodeURIComponent(msgId)}/pin`,
    {
      method: 'POST',
      headers,
...
export async function fetchPinnedMessage(
  ...
  squadId: string,
  extraHeaders?: Record<string, string>
): Promise<{ message: SquadMessage | null }> {
  const headers: Record<string, string> = accessToken
    ? { Authorization: `Bearer ${accessToken}` }
    : {};
  return requestJson<{ message: SquadMessage | null }>(
    `/api/squads/${encodeURIComponent(squadId)}/pinned`,
    {
      headers: { ...headers, ...extraHeaders },
    }
  );
...
export async function fetchTeamMessages(
  ...
  const headers: Record<string, string> = accessToken
    ...
    : {};
  return requestJson<{ squadId: string; messages: SquadMessage[]; hasMore?: boolean }>(
    `/api/teams/${encodeURIComponent(teamId)}/messages${suffix}`,
    {
      headers: { ...headers, ...extraHeaders },
    }
  );
...
export async function deleteTeamMessage(
  ...
  const headers: Record<string, string> = accessToken
    ...
    : {};
  return requestJson<{ ok: true }>(
    `/api/teams/${encodeURIComponent(teamId)}/messages/${encodeURIComponent(msgId)}`,
    {
      method: 'DELETE',
      headers: { ...headers, ...extraHeaders },
    }
