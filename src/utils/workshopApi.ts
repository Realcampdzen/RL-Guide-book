...
export async function fetchWorkshop(id: string): Promise<Workshop> {
  const data = await requestJson<{ workshop: Workshop }>(
    `/api/workshops/${encodeURIComponent(id)}`
  );
...
export async function addParticipant(
  ...
  return requestJson<{ workshop: Workshop }>(
    `/api/workshops/${encodeURIComponent(id)}/participants`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
      body: JSON.stringify(data),
...
export async function removeBadge(
  ...
  return requestJson<{ ok: true }>(
    `/api/workshops/${encodeURIComponent(id)}/badges/${encodeURIComponent(badgeId)}`,
    {
      method: 'DELETE',
...
export async function confirmBadge(
  ...
  return requestJson<{ ok: true }>(
    `/api/workshops/${encodeURIComponent(id)}/badges/${encodeURIComponent(badgeId)}/confirm`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
