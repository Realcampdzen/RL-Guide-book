...
export async function initiateBro(accessToken: string, squadId: string): Promise<{ event: BroEvent }> {
...
export async function fetchBroEvents(squadId: string): Promise<BroEvent[]> {
    const data = await requestJson<{ events: BroEvent[] }>(`/api/bro/events?squad_id=${encodeURIComponent(squadId)}`);
    return data.events || [];
}
...
export async function fetchMyPassport(deviceId: string): Promise<BroPassport | null> {
    try {
        const data = await requestJson<{ passport: BroPassport | null }>(`/api/bro/passport?device_id=${encodeURIComponent(deviceId)}`);
        return data.passport ?? null;
    } catch { return null; }
}

/** Start a BRO passport for a participant. */
export async function startPassport(accessToken: string, broEventId: string): Promise<{ passport: BroPassport }> {
...
export async function markTask(accessToken: string, passportId: string, taskId: string): Promise<{ passport: BroPassport }> {
    return requestJson<{ passport: BroPassport }>(`/api/bro/passport/${encodeURIComponent(passportId)}/task/${encodeURIComponent(taskId)}`, {
        method: 'PATCH',
