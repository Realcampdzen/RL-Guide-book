...
export async function fetchEngines(squadId: string): Promise<EngineItem[]> {
    const data = await requestJson<{ engines: EngineItem[] }>(`/api/engines?squad_id=${encodeURIComponent(squadId)}`);
    return data.engines || [];
}

/** Approve a pending engine (staff). */
export async function approveEngine(accessToken: string, id: string): Promise<{ engine: EngineItem }> {
...
export async function approveGoal(accessToken: string, id: string): Promise<{ engine: EngineItem }> {
    return requestJson<{ engine: EngineItem }>(`/api/engines/${encodeURIComponent(id)}/goal/approve`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${accessToken}` },
    });
}
