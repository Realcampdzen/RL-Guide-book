...
export async function fetchProgress(deviceId: string): Promise<InspectorTaskProgress[]> {
  const data = await requestJson<{ progress: InspectorTaskProgress[] }>(
    `/api/inspector/progress?device_id=${encodeURIComponent(deviceId)}`
  );
...
export async function approveTask(
  ...
  return requestJson<{ progress: InspectorTaskProgress }>(
    `/api/inspector/progress/${encodeURIComponent(taskId)}/approve`,
    {
      method: 'PATCH',
