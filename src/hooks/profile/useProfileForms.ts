...
export function useProfileForms() {
  ...
  const [childProgressFromFile, setChildProgressFromFile] = useState<Record<
    string,
    { status?: string; achievedAt?: string }
  > | null>(null);
  const [childReportMeta, setChildReportMeta] = useState<{
    nickname?: string;
    exportedAt?: string;
  } | null>(null);
  ...
  useEffect(() => {
    (window as any).__openBadgeProof__ = (badgeInfo: {
      id: string;
      title: string;
      image?: string;
    }) => {
