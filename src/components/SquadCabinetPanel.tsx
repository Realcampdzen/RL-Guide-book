...
const isImageAvatar = (value?: string | null): boolean =>
  Boolean(
...
export const SquadCabinetPanel: React.FC<SquadCabinetPanelProps> = ({
  ...
  const [inviteMeta, setInviteMeta] = useState<{ createdAt: string; expiresAt: string } | null>(
    null
  );
  ...
  const countFilledDays = useCallback((grid?: SquadCorner['planGridA'] | null) => {
    ...
    if ((shiftLength !== 9 && shiftLength !== 21) || typeof days !== 'object' || !days)
  ...
  const members = useMemo(() => {
    ...
    if (myDeviceId || myNickname) {
      for (const m of fallback) {
        if (
          (myDeviceId && m.deviceId === myDeviceId) ||
          (myNickname && m.nickname === myNickname)
        ) {
  ...
  const handleJoinByCode = async () => {
    ...
    try {
      ...
      try {
        ...
        if (pin) fallbackHeaders['X-Dev-Pin'] = pin;
      } catch {}
      const apiBase = (
        import.meta.env.VITE_API_URL ||
        import.meta.env.VITE_BACKEND_URL ||
      ...
      const confirmed = window.confirm(
        `Вступить в отряд ${preview.squadName ? `«${preview.squadName}»` : preview.squadId}?`
      );
      if (!confirmed) return;
      if (hasRealToken) {
        ...
        const res = await fetch(
          `${apiBase}/api/squads/${encodeURIComponent(preview.squadId)}/join`,
          {
            method: 'POST',
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', width: '100%' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="cab-card" style={{ padding: '20px 24px' }}>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 12, color: '#e8f0ff' }}>
              Вступить по коду
            </div>
          ...
          {isGodMode && (
            ...
            >
              ...
              <div style={{ fontSize: 13, opacity: 0.8, marginBottom: 14 }}>
                Список всех отрядов лагеря. Нажмите на любой, чтобы посмотреть его от лица
                Разработчика (работает в режиме песочницы).
              </div>
              {godModeSquads.length === 0 ? (
                ...
                >
                  {godModeSquads.map((sq) => (
                    ...
                    >
                      <span style={{ fontWeight: 700, fontSize: 14, color: '#e8f0ff' }}>
                        {sq.name || sq.id}
                      </span>
                      <span style={{ fontSize: 11, color: '#9ca3af' }}>
                        Смена: {sq.shiftName || sq.shiftId}
                      </span>
                    </motion.button>
                  ))}
                </div>
  ...
  const squadPhotoUrl =
    cornerPhotos.find((p) => p.key === 'photoSquad')?.url || cornerPhotos[0]?.url;
    ...
    <div style={{ maxWidth: 680, margin: '0 auto', width: '100%' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div
          ...
          style={{ display: 'flex', gap: 16, alignItems: 'center', padding: '24px 28px' }}
        >
          {squadPhotoUrl ? (
            <div
              style={{
        ...
        <div className="fade-in cab-card">
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span
                className={`m3-status-chip squad-corner-readiness-chip tone-${getSquadCornerReadinessTone(cornerReadiness)}`}
                style={{ marginTop: 0 }}
              >
                {getSquadCornerReadinessLabel(cornerReadiness)}
              </span>
            </div>
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{
                          ...
                          fontSize: 12,
                          borderRadius: 12,
                          boxShadow: '0 4px 12px rgba(93, 228, 255, 0.15)',
                        }}
                        onClick={() => {
                          setPlannerDay(1);
                          setPlannerOpen(true);
        ...
        <div className="fade-in cab-card">
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>
            Участники{' '}
            <span style={{ opacity: 0.5, fontWeight: 400, fontSize: 12 }}>({members.length})</span>
          </div>
          {members.length === 0 ? (
            ...
            >
              {members.map((m) => {
                const roleLabels: Record<string, string> = {
                  ...
                  camp_director: 'Директор лагеря',
                  developer: 'Разработчик',
                  parent: 'Родитель',
                };
                const roleLabel = roleLabels[m.role] || m.role;
                const isCounselor =
                  m.role === 'counselor' || m.role === 'shift_leader' || m.role === 'developer';
                      ...
                      {m.avatarUrl && isImageAvatar(m.avatarUrl) ? (
                        ...
                        >
                          <img
                            ...
                            onError={(e) => {
                              ...
                              e.currentTarget.parentElement!.style.fontSize = '18px';
                              e.currentTarget.parentElement!.style.background = isCounselor
                                ? 'linear-gradient(135deg, rgba(244,114,182,0.2), rgba(251,146,60,0.2))'
                                : 'linear-gradient(135deg, rgba(93,228,255,0.15), rgba(165,180,252,0.15))';
                              e.currentTarget.parentElement!.style.color = isCounselor
                                ? '#fdf2f8'
                                : '#e8f0ff';
                            }}
                          />
                        ...
                        <div
                          style={{
                            ...
                            justifyContent: 'center',
                            fontSize: 18,
                            color: isCounselor ? '#fdf2f8' : '#e8f0ff',
                            flexShrink: 0,
                          }}
                        >
                          {m.avatarUrl}
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        ...
                        <span
                          style={{
                            fontSize: 11,
                            fontWeight: 600,
                            color: isCounselor ? '#f472b6' : '#60a5fa',
                            textTransform: 'uppercase',
                            letterSpacing: 0.5,
                          }}
                        >
                      ...
                      {m.deviceId === deviceId ? (
                        <button
                          ...
                          style={{
                            ...
                            color: '#fff',
                            boxShadow: busy ? 'none' : '0 2px 8px rgba(220,38,38,0.45)',
                            opacity: busy ? 0.6 : 1,
                          }}
                        ...
                        <button
                          ...
                          style={{
                            ...
                            color: '#fff',
                            boxShadow: busy ? 'none' : '0 2px 8px rgba(217,119,6,0.45)',
                            opacity: busy ? 0.6 : 1,
                          }}
                          onClick={() => void handleKick(m.deviceId)}
                          disabled={busy}
                        >
                          Исключить
                        </button>
            ...
            {canManage && (
              <button
                ...
                style={{
                  ...
                  color: '#fff',
                  boxShadow: busy ? 'none' : '0 2px 8px rgba(8,145,178,0.4)',
                  opacity: busy ? 0.6 : 1,
                }}
          ...
          {inviteCode && (
            ...
            >
              ...
              <div style={{ fontSize: 11, opacity: 0.6, marginTop: 4 }}>
                Действует до:{' '}
                {inviteMeta ? new Date(inviteMeta.expiresAt).toLocaleString('ru-RU') : '—'}
              </div>
              <button
                type="button"
                style={{
        ...
        >
          <button
            ...
            style={{
              ...
              color: '#fff',
              boxShadow: busy ? 'none' : '0 4px 14px rgba(29,78,216,0.4)',
              opacity: busy ? 0.6 : 1,
            }}
          ...
          <button
            ...
            style={{
              ...
              color: '#fff',
              boxShadow: busy ? 'none' : '0 4px 14px rgba(220,38,38,0.45)',
              opacity: busy ? 0.6 : 1,
            }}
            onClick={() => void handleLeave()}
            disabled={busy}
        ...
        {(accessToken || (import.meta.env.DEV && deviceId)) && (
          <SquadChat
            squadId={squadId}
            accessToken={accessToken || ''}
            nickname={myNickname || mySquadInfo?.membership?.nickname || undefined}
            deviceId={deviceId}
            role={role}
            members={members}
          />
        ...
        {plannerOpen && (
          ...
          >
            ...
            >
              ...
              >
                <h3 style={{ fontSize: 18, fontWeight: 700, color: '#e8f0ff', margin: 0 }}>
                  Программа смены
                </h3>
                <button
                  type="button"
                      ...
                      {dayKeys.map((d) => (
                        <button
                          ...
                          style={{
                            ...
                            boxShadow:
                              plannerDay === d ? '0 2px 8px rgba(124,58,237,0.45)' : 'none',
