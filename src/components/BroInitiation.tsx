...
import { type BroDayMission, broMissions as defaultMissions } from '../types/bro';
...
export const BroInitiation: React.FC<BroInitiationProps> = ({
  ...
  const missions =
    dynamicBroMissions && dynamicBroMissions.length > 0 ? dynamicBroMissions : defaultMissions;
  ...
  const totalCompleted = Object.values(progress.completedDeeds).reduce(
  ...
  const isCounselor =
    userData.broProgress?.isBro ||
    userData.profile.stats.totalLevelsAchieved > COUNSELOR_LEVEL_THRESHOLD;
  ...
  const confirmBroLocally = () => {
    ...
    const ok = confirm(
      'Отметить, что Бросвящение подтверждено вожатым, и открыть Бро‑доступ на этом устройстве?'
    );
    if (!ok) return;
    receivePassport();
    becomeBro();
  ...
  if (isRitualActive) {
    return (
      <BroBonfire
        ...
        onCancel={finishRitual}
        usePortal={variant === 'cabin'}
      />
    );
  }
  ...
  if (progress.isBro) {
    return (
      ...
      >
        ...
        {isExpanded && (
          <>
            <p style={{ fontSize: '13px', opacity: 0.8, marginTop: '16px', color: '#e0d4ff' }}>
              Хранитель легендарных традиций с 2013 года
            </p>
            <div
              style={{
                position: 'absolute',
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h3 style={{ margin: 0, fontSize: isExpanded ? '20px' : '16px', fontWeight: 800 }}>
                БРОСВЯЩЕНИЕ
              </h3>
              {!isExpanded && (
  ...
  return (
    <div
      className="bro-initiation fade-in"
      style={{
    ...
    >
      ...
      >
        <div>
          ...
          >
            <span>
              Бропаспорт: {totalCompleted} / {totalDeeds}
            </span>
          ...
          >
            {isExpanded ? (
              ...
              <>
                Бропаспорт — {totalCompleted}/{totalDeeds} (
                {Math.round((totalCompleted / (totalDeeds || 1)) * 100)}%)
              </>
            )}
          </h3>
        </div>
      ...
      {isExpanded && (
        <>
                            ...
                            const newMissions = missions.map((m) => {
                              if (m.day === currentDayIndex) {
                                return {
                                  ...m,
                                  deeds: m.deeds.map((d) =>
                                    d.id === deed.id ? { ...d, text: e.target.value } : d
                                  ),
                                };
                              }
                              return m;
                            ...
                            const newMissions = missions.map((m) => {
                              if (m.day === currentDayIndex) {
                                return {
                                  ...m,
                                  deeds: m.deeds.map((d) =>
                                    d.id === deed.id ? { ...d, description: e.target.value } : d
                                  ),
                                };
                              }
                              return m;
          ...
          <div style={{ marginTop: '20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ fontSize: '12px', fontWeight: 800, color: '#c9b8ff' }}>
              {Math.round((totalCompleted / totalDeeds) * 100)}%
            </span>
          </div>
          ...
          {isReadyForRitual && !isRitualActive && !progress.isBro && (
            <div style={{ marginTop: 18, display: 'grid', gap: 10 }}>
              <div style={{ fontSize: 12, opacity: 0.85, lineHeight: 1.45 }}>
                Бродела выполнены. Дальше нужен реальный апрув вожатого: запроси подтверждение и
                только после этого нажми «Мне подтвердили».
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                ...
                <button
                  ...
                  title={
                    !accessToken && role !== 'developer'
                      ? 'Сначала войдите по коду участника смены'
                      : undefined
                  }
                >
                  ✅ Мне подтвердили
