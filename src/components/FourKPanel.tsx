...
import { type FourKStats, fetchStats } from '../utils/fourKApi';
...
const SKILLS = [
  {
    key: 'critical_thinking' as const,
...
export const FourKPanel: React.FC<FourKPanelProps> = ({ deviceId }) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setStats(await fetchStats(deviceId));
    } catch {
      setStats(null);
    } finally {
      setLoading(false);
  ...
  const programValues = useMemo(() => {
    ...
    const fourKAggregate = Math.round(
      (stats.critical_thinking + stats.creativity + stats.communication + stats.cooperation) / 4
    );
    return [
      fourKAggregate,
      Math.min(100, fourKAggregate * 0.7),
      Math.min(100, fourKAggregate * 0.5),
      Math.min(100, fourKAggregate * 0.6),
    ];
  }, [stats]);

  if (loading && !stats)
    return <div style={{ padding: 12, fontSize: 12, opacity: 0.6 }}>Загрузка навыков…</div>;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {stats ? (
                  ...
                  return (
                    <span
                      ...
                      style={{
                        ...
                        padding: '2px 6px',
                        borderRadius: 4,
                        background: `${sk?.color ?? '#6b7280'}22`,
                        color: sk?.color ?? '#6b7280',
                      }}
                    >
        ...
        <div style={{ fontSize: 12, opacity: 0.6 }}>
          Нет данных о навыках. Начните получать значки!
        </div>
      )}
    </div>
  );
