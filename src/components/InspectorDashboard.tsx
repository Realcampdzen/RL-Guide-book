...
import { createBadgeRequest, loadMyBadgeRequests, type BadgeRequestItem } from '../utils/badgeApprovalApi';
...
export const InspectorDashboard: React.FC<InspectorDashboardProps> = ({
  ...
  useEffect(() => {
    ...
    loadMyBadgeRequests(accessToken).then(reqs => {
  ...
  const handleResetMission = useCallback((dayNum: number) => {
  ...
  const handleSubmitMission = useCallback(async (mission: InspectorDayMission, completedTaskIds: string[]) => {
    ...
    setSubmitStatus(null);
    try {
      const req = await createBadgeRequest(accessToken, {
  ...
  }, [accessToken, deviceId, evidenceReflection, evidencePhotos, evidenceLink, durationMs, submitBusy, userData.profile.nickname]);
  ...
  if (variant === 'cabin') {
    ...
    return (
      <div id="inspector-dashboard" className="inspector-dashboard inspector-dashboard--cabin" style={{
        position: 'relative',
        ...
        <div className="inspector-dashboard__rules" style={{
          ...
          <p style={{ fontSize: '12px', fontWeight: 800, textTransform: 'uppercase', color: 'rgba(255,255,255,0.9)', letterSpacing: '0.1em', margin: '0 0 10px' }}>Правила Инспектора Пользы</p>
          <ul style={{ margin: 0, paddingLeft: '20px', fontSize: '14px', opacity: 0.85, lineHeight: 1.6 }}>
        ...
        {!onTabChange && (
        <div className="inspector-dashboard__tabs" style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '24px' }} role="tablist" aria-label="Разделы Инспектора">
            ...
            return (
              <button
                ...
                style={{
                  ...
                  gap: '6px',
                  padding: '10px 14px',
                  background: isActive ? 'rgba(255, 255, 255, 0.15)' : tabUnlocked ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.02)',
                  border: isActive ? '1px solid rgba(255, 255, 255, 0.3)' : '1px solid rgba(255, 255, 255, 0.08)',
                  borderRadius: '12px',
                  color: tabUnlocked ? (isActive ? '#fff' : 'rgba(255,255,255,0.7)') : 'rgba(255,255,255,0.3)',
                  fontSize: '13px',
        ...
        {lockedTabHint && (
          <div style={{ padding: '14px', background: 'rgba(255, 255, 255, 0.05)', borderRadius: '12px', marginBottom: '20px', fontSize: '13px', border: '1px solid rgba(255, 193, 7, 0.3)', color: 'rgba(255, 255, 255, 0.9)' }}>
            {lockedTabHint}
          </div>
        )}
        ...
        {!unlocked ? (
          ...
          <div className="inspector-dashboard__chief" style={{ padding: '24px', background: 'rgba(255,255,255,0.04)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <div className="inspector-dashboard__mission">
            {/* Goal + description */}
            <div style={{ padding: '18px 20px', background: 'rgba(255, 255, 255, 0.06)', borderRadius: '16px', marginBottom: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <MethodologyBlock mission={mission} isOpen={showMechanism} onToggle={() => setShowMechanism(!showMechanism)} />

            {onOpenDiary && (
              <div style={{ padding: '16px 20px', background: 'rgba(255, 255, 255, 0.04)', borderRadius: '14px', border: '1px solid rgba(255, 255, 255, 0.08)', marginBottom: '16px' }}>
            ...
            {!alreadyAchieved && !existingRequest && (
              <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 18px', background: isMissionActive ? 'rgba(34, 197, 94, 0.1)' : 'rgba(255,255,255,0.04)', borderRadius: '14px', border: `1px solid ${isMissionActive ? 'rgba(34, 197, 94, 0.3)' : 'rgba(255,255,255,0.08)'}`, transition: 'all 0.3s' }}>
                {!missionStartedAt ? (
                  <button type="button" onClick={() => setMissionStartedAt(Date.now())} style={{ padding: '10px 20px', background: '#22c55e', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '13px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ▶️ НАЧАТЬ МИССИЮ
                  </button>
                ) : isMissionActive ? (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1 }}>
                      ...
                      <span style={{ fontSize: '13px', fontWeight: 600, color: '#22c55e' }}>Миссия идёт</span>
                      <span style={{ fontSize: '18px', fontWeight: 700, fontFamily: 'monospace', color: '#fff', marginLeft: 'auto' }}>{timerDisplay}</span>
                    </div>
                    <button type="button" onClick={() => setMissionEndedAt(Date.now())} disabled={completedForDay.length === 0} style={{ padding: '8px 16px', background: completedForDay.length > 0 ? 'rgba(239, 68, 68, 0.2)' : 'rgba(255,255,255,0.05)', color: completedForDay.length > 0 ? '#ef4444' : 'rgba(255,255,255,0.3)', border: `1px solid ${completedForDay.length > 0 ? 'rgba(239, 68, 68, 0.4)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '10px', fontSize: '12px', fontWeight: 600, cursor: completedForDay.length > 0 ? 'pointer' : 'not-allowed' }}>
                      ⏹ ЗАВЕРШИТЬ
                    </button>
                  </>
                ) : (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1 }}>
                    ...
                    <span style={{ fontSize: '15px', fontWeight: 700, fontFamily: 'monospace', color: '#5de4ff' }}>{Math.floor(durationMs / 60000)}:{String(Math.floor((durationMs % 60000) / 1000)).padStart(2, '0')}</span>
                  </div>
                )}
                {!missionStartedAt && (
                  <span style={{ fontSize: '11px', opacity: 0.5, flex: 1, textAlign: 'right' }}>Нажми «Начать», чтобы открыть задания</span>
                )}
              </div>
            )}
            ...
            {existingRequest && (
              <div style={{ padding: '16px 20px', borderRadius: '14px', marginBottom: '16px', background: existingRequest.status === 'approved' ? 'rgba(34,197,94,0.12)' : existingRequest.status === 'rejected' ? 'rgba(239,68,68,0.12)' : 'rgba(255,193,7,0.12)', border: `1px solid ${existingRequest.status === 'approved' ? 'rgba(34,197,94,0.3)' : existingRequest.status === 'rejected' ? 'rgba(239,68,68,0.3)' : 'rgba(255,193,7,0.3)'}` }}>
                ...
                {existingRequest.resolutionNote && (
                  <div style={{ fontSize: '12px', opacity: 0.8, marginTop: '4px' }}>Комментарий: {existingRequest.resolutionNote}</div>
                )}
                {existingRequest.status === 'rejected' && (
                  <button type="button" onClick={() => { setExistingRequest(null); setMissionStartedAt(null); setMissionEndedAt(null); }} style={{ marginTop: '8px', padding: '6px 14px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#fff', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>
                    Попробовать снова
                  </button>
                )}
              </div>
            ...
            <div style={{ display: 'grid', gap: '8px', opacity: (!missionStartedAt && !alreadyAchieved && !existingRequest) ? 0.4 : 1, pointerEvents: (!missionStartedAt && !alreadyAchieved && !existingRequest) ? 'none' : 'auto', transition: 'opacity 0.3s' }}>
              {mission.tasks.map((task) => {
                ...
                return (
                  <div
                    key={task.id}
                    onClick={() => !taskLocked && !existingRequest && updateInspectorTask(mission.day, task.id, !isCompleted)}
                    style={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: '14px',
                      ...
                      borderRadius: '16px',
                      border: isCompleted ? '1px solid rgba(255, 255, 255, 0.2)' : '1px solid rgba(255, 255, 255, 0.08)',
                  ...
                  >
                    ...
                    {task.isChallenge && <span title="Челлендж" style={{ fontSize: '16px' }}>🔥</span>}
                    {task.isCoop && <span title="Кооператив" style={{ fontSize: '16px' }}>👥</span>}
                    {task.isCamp && <span title="Лагерное" style={{ fontSize: '16px' }}>✨</span>}
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ marginTop: '20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, opacity: 0.8 }}>{completedForDay.length} / {totalTasks}</span>
            </div>
            ...
            {completedForDay.length > 0 && !alreadyAchieved && !existingRequest && (
              <div style={{ marginTop: '10px', textAlign: 'right' }}>
                <button
                  type="button"
                  onClick={() => { if (window.confirm('Сбросить все задания и начать миссию заново?')) handleResetMission(mission.day); }}
                  style={{
                    padding: '5px 12px',
                  ...
                  onMouseLeave={e => { e.currentTarget.style.color = 'rgba(255,255,255,0.35)'; e.currentTarget.style.borderBottomColor = 'rgba(255,255,255,0.2)'; }}
                >
                  Начать заново
                </button>
              </div>
            ...
            {isMissionStopped && isDayComplete && !alreadyAchieved && !existingRequest && (
              <div className="fade-in" style={{ marginTop: '16px', padding: '20px', background: `rgba(${INSPECTOR_ACCENT_RGB}, 0.15)`, borderRadius: '16px', border: `1px solid ${INSPECTOR_ACCENT}` }}>
                <div style={{ fontSize: '28px', marginBottom: '8px', textAlign: 'center' }}>🏆</div>
                <div style={{ fontWeight: 800, fontSize: '15px', color: INSPECTOR_ACCENT, textTransform: 'uppercase', textAlign: 'center' }}>
                  Миссия выполнена!
                </div>
                <div style={{ fontSize: '13px', opacity: 0.9, marginTop: '4px', textAlign: 'center' }}>
                  Ты — <strong>{mission.resultTitle}</strong>
                </div>

                {/* Benefit */}
                <div style={{ margin: '16px 0', padding: '10px 14px', background: 'rgba(255,255,255,0.06)', borderRadius: '12px' }}>
                  <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: INSPECTOR_ACCENT, marginBottom: '4px' }}>Польза этой миссии</div>
                  <div style={{ fontSize: '13px', lineHeight: 1.5, opacity: 0.85 }}>{mission.benefit}</div>
                </div>

                {/* Auto-stats */}
                <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
                  <div style={{ padding: '8px 14px', borderRadius: '10px', background: 'rgba(93,228,255,0.1)', border: '1px solid rgba(93,228,255,0.2)', fontSize: '12px', fontWeight: 600, color: '#5de4ff' }}>
                    ⏱ {Math.floor(durationMs / 60000)} мин {Math.floor((durationMs % 60000) / 1000)} сек
                  </div>
                  <div style={{ padding: '8px 14px', borderRadius: '10px', background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)', fontSize: '12px', fontWeight: 600, color: '#22c55e' }}>
                ...
                <div style={{ marginBottom: '14px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: '6px' }}>📝 Рефлексия миссии *</label>
                  <textarea
                    ...
                    rows={4}
                    style={{ width: '100%', padding: '12px 14px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '12px', color: '#e8f0ff', fontSize: '13px', lineHeight: 1.5, resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }}
                  />
                  <div style={{ fontSize: '10px', opacity: 0.4, marginTop: '4px', textAlign: 'right' }}>{evidenceReflection.length} / 20+ символов</div>
                </div>

                {/* Photo uploader */}
                <div style={{ marginBottom: '14px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: '6px' }}>📸 Фото (до 3)</label>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                    {evidencePhotos.map((p, i) => (
                      <div key={i} style={{ position: 'relative', width: '64px', height: '64px', borderRadius: '10px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.15)' }}>
                        <img src={p} alt={`Фото ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        <button type="button" onClick={() => setEvidencePhotos(prev => prev.filter((_, idx) => idx !== i))} style={{ position: 'absolute', top: '2px', right: '2px', width: '18px', height: '18px', borderRadius: '50%', background: 'rgba(0,0,0,0.7)', border: 'none', color: '#fff', fontSize: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                      </div>
                    ))}
                    {evidencePhotos.length < 3 && (
                      <label style={{ width: '64px', height: '64px', borderRadius: '10px', border: '2px dashed rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '22px', opacity: 0.5, transition: 'opacity 0.2s' }}>
                        +
                ...
                <div style={{ marginBottom: '16px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: '6px' }}>📎 Ссылка на артефакт</label>
                  <input
                    type="url"
                    value={evidenceLink}
                    onChange={e => setEvidenceLink(e.target.value)}
                    placeholder="Сценарий, план, документ…"
                    style={{ width: '100%', padding: '10px 14px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '10px', color: '#e8f0ff', fontSize: '13px', fontFamily: 'inherit', boxSizing: 'border-box' }}
                  />
                </div>

                {/* Submit status */}
                {submitStatus && (
                  <div style={{ padding: '10px 14px', borderRadius: '10px', background: submitStatus.startsWith('✅') ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)', border: `1px solid ${submitStatus.startsWith('✅') ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}`, fontSize: '13px', marginBottom: '12px' }}>
                    {submitStatus}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center' }}>
                  {accessToken ? (
                    <button
                      ...
                      style={{
                        padding: '12px 24px',
                        background: evidenceReflection.trim().length >= 20 ? INSPECTOR_ACCENT : 'rgba(255,255,255,0.1)',
                        color: evidenceReflection.trim().length >= 20 ? '#fff' : 'rgba(255,255,255,0.4)',
                        border: 'none',
                        borderRadius: '100px',
                        fontSize: '13px',
                        fontWeight: 700,
                    ...
                    <div style={{ fontSize: '12px', opacity: 0.6, textAlign: 'center' }}>Для отправки заявки нужен код участника смены.</div>
                  )}
                  <button
                    type="button"
                    style={{ padding: '8px 16px', background: 'transparent', color: INSPECTOR_ACCENT, border: `1px solid ${INSPECTOR_ACCENT}`, borderRadius: '100px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}
            ...
            {alreadyAchieved && (
              <div style={{ marginTop: '16px', padding: '16px', background: 'rgba(34,197,94,0.12)', borderRadius: '14px', border: '1px solid rgba(34,197,94,0.3)', textAlign: 'center' }}>
                <div style={{ fontSize: '20px', marginBottom: '4px' }}>✅</div>
                <div style={{ fontSize: '14px', fontWeight: 700, color: '#22c55e' }}>Значок получен</div>
              </div>
            )}
          </div>
        ) : null}
    ...
    <div id="inspector-dashboard" className="inspector-dashboard" style={wrapperStyle}>
            ...
            <div style={{ marginTop: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '4px', marginTop: '6px', flexWrap: 'wrap' }}>
                {currentMission.skills.map(sk => {
                  ...
                  return (
                    <span key={sk} style={{ fontSize: '10px', padding: '2px 6px', borderRadius: '6px', background: `${meta.color}18`, color: meta.color, fontWeight: 600 }}>
      ...
      {isExpanded && (
        <div className="fade-in" style={{ display: 'grid', gap: '12px' }}>
          {/* Goal + description */}
          <div style={{ padding: '12px 14px', background: 'rgba(0,0,0,0.15)', borderRadius: '12px' }}>
          ...
          {/* 4К Skills tags */}
          <SkillTagsRow skills={currentMission.skills} skillDetails={currentMission.skillDetails} />
...
const SkillTagsRow: React.FC<{
  ...
  return (
    <div style={{ marginBottom: '16px' }}>
      <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.45)', marginBottom: '8px' }}>Навыки 4К</div>
      ...
      {expandedSkill && skillDetails[expandedSkill] && (
        <div className="fade-in" style={{
          ...
          fontSize: '13px', lineHeight: 1.5, opacity: 0.85,
        }}>
          <strong>{SKILL_4K_LABELS[expandedSkill as keyof typeof SKILL_4K_LABELS]?.emoji}</strong> {skillDetails[expandedSkill]}
        </div>
      )}
    </div>
  );
  ...
  <div style={{ marginBottom: '16px' }}>
    <button
      ...
      style={{
        ...
        border: '1px solid rgba(255, 255, 255, 0.08)',
        color: '#e8f0ff', fontSize: '13px', fontWeight: 600,
        cursor: 'pointer', transition: 'all 0.2s', textAlign: 'left',
      }}
    >
      <span style={{ fontSize: '14px' }}>📚</span>
      <span style={{ flex: 1 }}>Почему это важно?</span>
      <span style={{ fontSize: '12px', opacity: 0.5, transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.3s' }}>▾</span>
    </button>
    {isOpen && (
      <div className="fade-in" style={{ padding: '16px', marginTop: '8px', background: 'rgba(255,255,255,0.04)', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ marginBottom: '14px' }}>
          <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.45)', marginBottom: '8px' }}>Механизм пользы</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {mission.mechanism.map((m, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '13px', lineHeight: 1.5, opacity: 0.85 }}>
        ...
        <div style={{ padding: '12px 14px', background: 'rgba(93, 228, 255, 0.06)', borderRadius: '10px', border: '1px solid rgba(93, 228, 255, 0.12)' }}>
          <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#5de4ff', marginBottom: '4px' }}>Результат</div>
          <div style={{ fontSize: '13px', lineHeight: 1.5, opacity: 0.85 }}>{mission.benefit}</div>
        </div>
      </div>
    )}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
        ...
        <div>
          <div style={{ fontSize: '14px', fontWeight: 700, color: '#e8f0ff' }}>Мой путь Инспектора</div>
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '14px' }}>
        {Array.from({ length: totalMissions }, (_, i) => {
          ...
          return (
            <div key={i} title={`${m.title} ${done ? '✅' : '⬜'}`} style={{
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '14px' }}>
        {completedMissions.map(m => (
          <span key={m.day} style={{
            ...
            background: 'rgba(93, 228, 255, 0.1)', border: '1px solid rgba(93, 228, 255, 0.2)',
            fontSize: '11px', fontWeight: 600, color: '#5de4ff',
          }}>
      ...
      {/* Skills radar */}
      <div>
        <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.4)', marginBottom: '8px' }}>Развитые навыки</div>
