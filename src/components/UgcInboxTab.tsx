...
function loadCatProposals(): CategoryProposal[] {
  try {
    return JSON.parse(localStorage.getItem(CAT_LS_KEY) || '[]') as CategoryProposal[];
  } catch {
    return [];
  }
}
...
function saveCatProposals(items: CategoryProposal[]) {
  try {
    localStorage.setItem(CAT_LS_KEY, JSON.stringify(items));
  } catch {
    /* */
  }
}
...
export const UgcInboxTab: React.FC<UgcInboxTabProps> = ({ accessToken: _accessToken }) => {
  ...
  // Badge actions
  const handleBadgeAction = useCallback(
  ...
  // Category actions
  const handleCatAction = useCallback(
    (id: string, status: 'approved' | 'rejected') => {
            ...
            return (
              ...
              >
                <div style={{ fontSize: 11, fontWeight: 600 }}>
                  {p.badgeTitle} ({p.badgeId})
                </div>
                <div style={{ fontSize: 10, opacity: 0.6 }}>Поле: {FIELD_LABELS[p.field]}</div>
                <div style={{ fontSize: 11, opacity: 0.85, marginTop: 2, whiteSpace: 'pre-wrap' }}>
                  {p.text.slice(0, 200)}
                  {p.text.length > 200 ? '…' : ''}
                </div>
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                <button
                  ...
                  className="btn-secondary"
                  style={{ padding: '2px 8px', fontSize: 10, color: '#22c55e' }}
                  onClick={() => handleBadgeAction(b.id, 'approved')}
                >
                  ✅ Одобрить
                </button>
                <button
              ...
              {rejectTarget === b.id && (
                <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      handleBadgeAction(b.id, 'rejected');
        ...
        <div>
          ...
          {pendingCats.map((c) => (
            ...
            >
              ...
              {c.description && (
                <div style={{ fontSize: 10, opacity: 0.75, marginTop: 2 }}>{c.description}</div>
              )}
              <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                <button
                  type="button"
                  className="btn-secondary"
                  style={{ padding: '2px 8px', fontSize: 10, color: '#22c55e' }}
                  onClick={() => handleCatAction(c.id, 'approved')}
                >
                  ✅
                </button>
                <button
                  ...
                  className="btn-secondary"
                  style={{ padding: '2px 8px', fontSize: 10, color: '#ef4444' }}
                  onClick={() => handleCatAction(c.id, 'rejected')}
                >
                  ❌
                </button>
              </div>
