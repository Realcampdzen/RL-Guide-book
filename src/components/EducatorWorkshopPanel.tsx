...
export const EducatorWorkshopPanel: React.FC<EducatorWorkshopPanelProps> = ({
  ...
  const handleSaveInfo = useCallback(async () => {
    ...
    setBusy(true);
    try {
      await updateWorkshop(accessToken, workshopId, {
        name: editName.trim(),
        direction: editDir.trim() || undefined,
      });
      void load();
    } catch {
      /* */
  ...
  const handleAddParticipant = useCallback(async () => {
    ...
    setBusy(true);
    try {
      await addParticipant(accessToken, workshopId, {
  ...
  const handleAddBadge = useCallback(async () => {
    ...
    setBusy(true);
    try {
      await addBadge(accessToken, workshopId, addBadgeId.trim());
      setAddBadgeId('');
      void load();
    } catch {
      /* */
  ...
  const handleRemoveBadge = useCallback(
    async (badgeId: string) => {
      ...
      setBusy(true);
      try {
        await removeBadge(accessToken, workshopId, badgeId);
        void load();
      } catch {
        /* */
      } finally {
  ...
  const handleConfirm = useCallback(
    async (badgeId: string, deviceId: string) => {
      if (!accessToken) return;
      setBusy(true);
      try {
        await confirmBadge(accessToken, workshopId, badgeId, deviceId);
        void load();
      } catch {
        /* */
      } finally {
  ...
  if (loading && !ws)
    return <div style={{ padding: 12, fontSize: 12, opacity: 0.6 }}>Загрузка мастерской…</div>;
  if (!ws)
    return <div style={{ padding: 12, fontSize: 12, opacity: 0.6 }}>Мастерская не найдена.</div>;
        ...
        {TABS.map((t) => (
          <button
            ...
            style={{
              padding: '4px 10px',
              fontSize: 10,
              background: tab === t.id ? `${ACCENT}22` : undefined,
              color: tab === t.id ? ACCENT : undefined,
              border: tab === t.id ? `1px solid ${ACCENT}44` : undefined,
            }}
            onClick={() => setTab(t.id)}
          >
            {t.icon} {t.label}
            ...
            ws.participants.map((p) => (
              ...
              >
                ...
                <span style={{ flex: 1, fontSize: 12 }}>
                  {p.nickname || p.deviceId.slice(0, 8)}
                </span>
              </div>
            ))
          )}
          {canEdit && (
            ...
            >
              ...
              <button
                ...
                disabled={busy || !addDeviceId.trim()}
              ...
              >
                ...
                {canEdit && (
                  <button
                    ...
                    disabled={busy}
                    onClick={() => void handleRemoveBadge(b.badgeId)}
                  >
                    🗑
                  </button>
                )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {ws.badges.length === 0 ? (
            <div style={{ fontSize: 12, opacity: 0.6 }}>
              Добавьте значки, чтобы подтверждать получение.
            </div>
          ) : (
            ws.badges.map((b) => (
              <div
                    ...
                    >
                      ...
                      {!confirmed && canEdit && (
                        <button
                          ...
                          disabled={busy}
                          onClick={() => void handleConfirm(b.badgeId, p.deviceId)}
                        >
                          ✅
                        </button>
                      )}
