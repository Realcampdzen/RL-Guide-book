...
export type CounselorSquadTabId =
  | 'squad'
  | 'photos'
  | 'planner'
  | 'flag-badges'
...
const PHOTO_FIELDS = [
  {
    key: 'photoCorner' as const,
    label: 'Фото отрядного уголка',
    description: 'Общий вид уголка отряда — стенд, плакаты, украшения',
  },
  { key: 'photoFlag' as const, label: 'Флаг отряда', description: 'Флаг или символ вашего отряда' },
  {
    key: 'photoSquad' as const,
    label: 'Общее отрядное фото',
    description: 'Фото всего отряда вместе',
  },
  {
    key: 'photoWithCounselors' as const,
    label: 'Фото с вожатыми',
    description: 'Фото отряда с вожатыми',
  },
];
const FLAG_BADGE_NAMES: Record<string, string> = {
  '10.1': 'Мерцающий Маяк',
  '10.2': 'Алый Парус',
  '10.3': 'Горящий Факел',
};
...
export const CounselorSquadDashboard: React.FC<CounselorSquadDashboardProps> = ({
  ...
  const [activePlannerGrid, setActivePlannerGrid] = useState<'planGridA' | 'planGridB'>(
    'planGridA'
  );
  ...
  const [localPhotoWithCounselors, setLocalPhotoWithCounselors] = useState(
    squad.photoWithCounselors ?? ''
  );
  const [localPlanGridA, setLocalPlanGridA] = useState<CounselorSquadPlanGridData>(() =>
    squad.planGridA
      ? { shiftLength: squad.planGridA.shiftLength, days: { ...squad.planGridA.days } }
      : defaultPlanGrid()
  );
  const [localPlanGridB, setLocalPlanGridB] = useState<CounselorSquadPlanGridData>(() =>
    squad.planGridB
      ? { shiftLength: squad.planGridB.shiftLength, days: { ...squad.planGridB.days } }
      : defaultPlanGrid()
  );
  ...
  React.useEffect(() => {
    ...
    setLocalPlanGridA(
      s.planGridA
        ? { shiftLength: s.planGridA.shiftLength, days: { ...s.planGridA.days } }
        : defaultPlanGrid()
    );
    setLocalPlanGridB(
      s.planGridB
        ? { shiftLength: s.planGridB.shiftLength, days: { ...s.planGridB.days } }
        : defaultPlanGrid()
    );
  }, [activeSquadCard]);
  ...
  const setCurrentPlanGrid =
    activePlannerGrid === 'planGridA' ? setLocalPlanGridA : setLocalPlanGridB;
  const dayKeys = useMemo(
    () => Array.from({ length: currentPlanGrid.shiftLength }, (_, i) => i + 1),
    [currentPlanGrid.shiftLength]
  );
  ...
  const savePlanner = () =>
    updateActiveSquadCard({ planGridA: localPlanGridA, planGridB: localPlanGridB });
  ...
  const percent = useMemo(() => {
    ...
    const photosScore =
      [localPhotoCorner, localPhotoFlag, localPhotoSquad, localPhotoWithCounselors].filter(
        isImageUrl
      ).length / 4;
    ...
    return Math.min(
      100,
      Math.round(
        ((squadScore + photosScore + gridScore(localPlanGridA) + gridScore(localPlanGridB)) / 4) *
          100
      )
    );
  }, [
  ...
  const sectionWrap = (content: React.ReactNode) => (
    <div
      className="counselor-squad-cabin-section"
      ...
      {FLAG_BADGE_ORDER.map((id) => {
        ...
        return (
          ...
          >
            <BadgeIcon
              badgeId={id}
              badgeTitle={FLAG_BADGE_NAMES[id]}
              categoryId="10"
              emoji="🚩"
              size="responsive"
            />
  ...
  const pendingRequests = (() => {
    ...
    if (!pending.length)
      return <p className="profile-empty-state__text">Пока нет заявок на рассмотрении</p>;
    const allText = [
      'Заявки на значки флага отряда:',
      ...pending.map((req, i) => `${i + 1}. ${FLAG_BADGE_NAMES[req.badgeId] || req.badgeId}`),
    ].join('\n\n');
    return (
      <>
        <a
          href={`${COUNSELOR_TELEGRAM}?text=${encodeURIComponent(allText)}`}
          target="_blank"
          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <a
                  href={`${COUNSELOR_TELEGRAM}?text=${encodeURIComponent(`Заявка на значок флага отряда: ${name}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                {canEdit && (
                  <button
                    type="button"
                    className="btn-secondary"
                    onClick={() => approveActiveFlagBadgeRequest(req.badgeId)}
                  >
                    Вожатый утвердил
                  </button>
                )}
  ...
  const squadSection = sectionWrap(
    <>
      ...
      <textarea
        className="w-input"
        style={{ width: '100%' }}
        placeholder="Кричалки"
        value={localSquadChants}
        onChange={(e) => setLocalSquadChants(e.target.value)}
        rows={2}
        {...readOnlyProps}
      />
      <input
      ...
      <textarea
        ...
        style={{ width: '100%' }}
        placeholder="Мемы"
        value={localSquadMemes}
        onChange={(e) => setLocalSquadMemes(e.target.value)}
        rows={2}
        {...readOnlyProps}
      />
      {canEdit && (
              ...
              canEdit
                ? async (o) =>
                    requestImageGenerate(
                      { mode: 'generate', context: 'counselor_squad', prompt: o.prompt ?? '' },
                      accessToken ?? null
                    )
                : undefined
            }
    ...
    <>
      <div style={{ display: 'flex', gap: 8 }}>
        {(['planGridA', 'planGridB'] as const).map((id) => (
          <button
            ...
            type="button"
            className="btn-secondary"
            onClick={() => setActivePlannerGrid(id)}
            disabled={!canEdit}
          >
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <label>
          <input
            type="radio"
            checked={currentPlanGrid.shiftLength === 9}
            onChange={() => setShiftLength(9)}
            disabled={!canEdit}
          />{' '}
          9 дней
        </label>
        <label>
          <input
            type="radio"
            checked={currentPlanGrid.shiftLength === 21}
            onChange={() => setShiftLength(21)}
            disabled={!canEdit}
          />{' '}
          21 день
        </label>
  ...
  const [traditions, setTraditions] = useState<Tradition[]>(() => {
    try {
      return JSON.parse(localStorage.getItem(TRADITIONS_KEY) || '[]') as Tradition[];
    } catch {
      return [];
    }
  });
  ...
  const saveTraditions = useCallback(
    (next: Tradition[]) => {
      setTraditions(next);
      try {
        localStorage.setItem(TRADITIONS_KEY, JSON.stringify(next));
      } catch {
        /* */
      }
    },
  ...
  const approveTradition = useCallback(
    (id: string) => {
      saveTraditions(
        traditions.map((t) => (t.id === id ? { ...t, status: 'approved' as const } : t))
      );
    },
  ...
  if (!activeSquadId) {
    return (
      <div
        className="fade-in counselor-squad-cabin-content"
        style={{
          display: 'flex',
      ...
      >
        ...
        {!myJoinedSquad && !canCreateSquad && role !== 'counselor' && role !== 'educator' && (
          <div className="organizer-empty-state" style={{ marginBottom: 14 }}>
            ...
            <p
              className="organizer-empty-state__text"
              style={{ margin: '0 0 10px', fontSize: 12, opacity: 0.85 }}
            >
              Раздел предназначен для вожатых и старших. Ты видишь, что функционал предусмотрен, но
              пользоваться им не можешь.
            </p>
            <div
              className="organizer-empty-state__text"
              style={{ margin: 0, fontSize: 12, opacity: 0.85 }}
            >
        ...
        {!myJoinedSquad &&
          (role === 'counselor' || role === 'educator' || (canCreateSquad && !myCreatedSquad)) && (
            <div
              style={{ marginBottom: 14 }}
              className={role === 'counselor' || role === 'educator' ? 'organizer-empty-state' : ''}
            >
              {(role === 'counselor' || role === 'educator') && (
                <>
                  ...
                  <p
                    className="organizer-empty-state__title"
                  ...
                  >
                    Старший Вожатый (или Разработчик в песочнице) создаёт отряд и даёт код
                    приглашения. Вставьте код, чтобы присоединиться.
                  </p>
                </>
              )}
              ...
              {counselorJoinError && (
                <span style={{ fontSize: 12, color: '#ff6b6b', display: 'block', marginTop: 4 }}>
                  {counselorJoinError}
                </span>
              )}
            </div>
        ...
        {canCreateSquad && !myCreatedSquad && (
          <div className="organizer-empty-state" style={{ padding: '12px 0' }}>
            ...
            <p className="organizer-empty-state__title" style={{ margin: '0 0 8px' }}>
              Создать отряд вожатых
            </p>
            <p
              className="organizer-empty-state__text"
              style={{ margin: '0 0 12px', fontSize: 13, opacity: 0.85 }}
            >
                ...
                onClick={() => {
                  if (counselorSquadName.trim()) {
                    ...
                    onShowHint?.({
                      ...
                      content: 'Укажите название отряда, чтобы создать его.',
                    });
                  }
                }}
                className="btn-primary-gold"
  ...
  const emptyStateCard = !activeSquadCard && role === 'counselor' && (
    <div className="profile-empty-state organizer-empty-state" style={{ padding: 20 }}>
      <p className="profile-empty-state__text">
        Данные отряда недоступны в этом устройстве без синхронизации.
      </p>
    </div>
  );
        ...
        >
          <div style={{ minWidth: 0 }}>
            ...
            <h3 style={{ margin: 0, fontSize: 18 }}>
              {(activeSquadName || localSquadName || 'Отряд').trim()}
            </h3>
          </div>
          <div
            style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              ...
              <span
                style={{
                  fontSize: 12,
                  fontWeight: 800,
                  color: percent === 100 ? ACCENT : 'rgba(255,255,255,.7)',
                }}
              >
                {percent}%
              </span>
  ...
  const membersSection = sectionWrap(
    <>
      ...
      {(activeSquadCard as Record<string, unknown>)?.members &&
      Array.isArray((activeSquadCard as Record<string, unknown>).members) ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {(
            (activeSquadCard as Record<string, unknown>).members as Array<{
              deviceId: string;
              nickname?: string;
              role?: string;
              avatarUrl?: string;
            ...
            const rb = ROLE_BADGES[m.role ?? ''] ?? {
              label: m.role ?? 'участник',
              color: '#6b7280',
            };
            const isAvatarImage = Boolean(
              m.avatarUrl &&
                (m.avatarUrl.startsWith('http') ||
                  m.avatarUrl.startsWith('data:') ||
                  m.avatarUrl.startsWith('/'))
            );
            return (
              <div
                key={m.deviceId}
              ...
              >
                  ...
                  >
                    <img
                      ...
                      onError={(e) => {
                        ...
                        e.currentTarget.parentElement!.innerHTML = m.nickname
                        ...
                        e.currentTarget.parentElement!.style.background = `${rb.color}22`;
                        e.currentTarget.parentElement!.style.color = rb.color;
                      }}
                    />
                  </div>
                ) : (
                ...
                >
                  {m.nickname || m.deviceId.slice(0, 8)}
                </span>
                <span
                  style={{
                    fontSize: 9,
        ...
        <div style={{ fontSize: 12, opacity: 0.6 }}>
          Список участников загрузится из серверных данных (M14).
        </div>
      )}
    </>
  );
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
        ...
        <button
          ...
          onClick={() => {
            setChatInput('');
            onShowHint?.({ title: 'Чат', content: 'Серверный чат будет подключён в M14.' });
          }}
        >
          📤
        </button>
            ...
            .map((m) => (
              ...
              >
                ...
                <div style={{ fontSize: 11, opacity: 0.7 }}>
                  {m.workshop || 'Мастерская не указана'}
                </div>
              </div>
            ))}
          {(
            (activeSquadCard as Record<string, unknown>).members as Array<{ role?: string }>
          ).filter((m) => m.role === 'educator').length === 0 && (
            <div style={{ fontSize: 12, opacity: 0.6 }}>Нет педагогов в отряде</div>
          )}
        </div>
      ) : (
        <div style={{ fontSize: 12, opacity: 0.6 }}>
          Список педагогов загрузится из серверных данных (M14).
        </div>
      )}
    </>
  );
    ...
    <>
      ...
      {traditions.length === 0 ? (
        <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 8 }}>
          Нет традиций. Предложите первую!
        </div>
      ) : (
      ...
      {accessToken && (
        ...
        >
          ...
          <input
            type="text"
            value={tradTitle}
            onChange={(e) => setTradTitle(e.target.value)}
          ...
          <textarea
            value={tradDesc}
            onChange={(e) => setTradDesc(e.target.value)}
          ...
          <input
            type="text"
            value={tradBadge}
            onChange={(e) => setTradBadge(e.target.value)}
            placeholder="ID значка (необязательно)"
            style={{
              width: '100%',
              padding: '6px 8px',
              borderRadius: 6,
  ...
  return (
    <div
      className={`fade-in counselor-squad-cabin-content ${!canEdit ? 'counselor-squad-read-only' : ''}`}
      style={{
        display: 'flex',
        flexDirection: 'column',
        gap: 16,
