...
function lsKey(squadId: string) { return `rl-squad-traditions-${squadId}`; }

function loadTraditions(squadId: string): SquadTradition[] {
    try { return JSON.parse(localStorage.getItem(lsKey(squadId)) || '[]') as SquadTradition[]; }
    catch { return []; }
}

function saveTraditions(squadId: string, items: SquadTradition[]) {
    try { localStorage.setItem(lsKey(squadId), JSON.stringify(items)); } catch { /* */ }
}
...
export const SquadTraditionsSection: React.FC<SquadTraditionsSectionProps> = ({
    ...
    const save = useCallback((next: SquadTradition[]) => {
        setTraditions(next);
    ...
    const handleCreate = useCallback(() => {
        ...
        save([...traditions, t]);
        setTitle(''); setDesc(''); setBadgeId(''); setBadgeTitle(''); setShowForm(false);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button type="button" className="btn-secondary" style={{ padding: '4px 12px', fontSize: 11, color: ACCENT }}
            ...
            {/* Creation form */}
            {showForm && (
                <div style={{ padding: 12, borderRadius: 10, background: 'rgba(217,119,6,0.08)', border: '1px solid rgba(217,119,6,0.2)' }}>
                    <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Название традиции"
                        style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(0,0,0,0.3)', color: '#fff', fontSize: 12, marginBottom: 6, boxSizing: 'border-box' }} />
                    <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="Описание…"
                        style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(0,0,0,0.3)', color: '#fff', fontSize: 12, marginBottom: 6, minHeight: 40, boxSizing: 'border-box' }} />

                    <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                        {(['squad', 'camp'] as TraditionScope[]).map(s => (
                            <button key={s} type="button" className="btn-secondary"
                                style={{ padding: '4px 8px', fontSize: 10, background: scope === s ? `${SCOPE_LABELS[s].color}22` : undefined, color: scope === s ? SCOPE_LABELS[s].color : undefined }}
                                onClick={() => setScope(s)}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                        <input type="text" value={badgeId} onChange={e => setBadgeId(e.target.value)} placeholder="ID значка"
                        ...
                        <input type="text" value={badgeTitle} onChange={e => setBadgeTitle(e.target.value)} placeholder="Название значка"
                            style={{ flex: 1, padding: '6px 8px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(0,0,0,0.2)', color: '#fff', fontSize: 11 }} />
                    </div>

                    <button type="button" className="btn-primary-gold" disabled={!title.trim()} onClick={handleCreate}
                        ...
                        return (
                            <div key={t.id} style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                    ...
                                    <span style={{ fontSize: 8, padding: '1px 4px', borderRadius: 4, background: `${sc.color}22`, color: sc.color }}>{sc.icon} {sc.label}</span>
                                    <span style={{
                                    ...
                                    {t.fromEngineId && <span style={{ fontSize: 8, padding: '1px 4px', borderRadius: 4, background: 'rgba(22,163,74,0.15)', color: '#16a34a' }}>⚙️</span>}
                                </div>
                                {t.description && <div style={{ fontSize: 10, opacity: 0.7, marginTop: 2 }}>{t.description}</div>}
                                {t.linkedBadgeId && (
                                    <div style={{ fontSize: 10, opacity: 0.6, marginTop: 2, cursor: onNavigateToBadge ? 'pointer' : 'default' }}
            ...
            {/* Toast */}
            {toast && (
                <div style={{ padding: 10, borderRadius: 8, background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.3)', fontSize: 12, color: '#22c55e', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span>🏅</span>
                    <span style={{ flex: 1 }}>{toast}</span>
                    <button type="button" className="btn-secondary" style={{ padding: '2px 6px', fontSize: 10 }} onClick={() => setToast(null)}>✕</button>
                </div>
            )}
        </div>
    );
