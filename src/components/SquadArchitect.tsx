...
const TRADITION_SCENARIOS: Record<string, TraditionScenario> = {
  '🔥 Свечка (Огонек)': {
    step2: 'Круг. Каждый говорит слово, которое ассоциирует с отрядом. Передача символа (свеча, камень, шишка) по кругу.',
    step3: 'Огонёк «Почему я здесь». Вожатый задаёт вопрос: что ты принёс в отряд? Чего ждёшь от смены?',
  ...
  '🔥 Ритуал огня / костёр': {
    step2: 'Путь испытаний: каждый выполняет небольшое задание (ответить на вопрос, показать умение).',
...
export const SquadArchitect: React.FC<{
  ...
  return (
    <div className="fade-in" style={{
      marginTop: '20px', padding: '20px', background: 'rgba(0,0,0,0.3)', borderRadius: '16px', border: '1px dashed rgba(255,255,255,0.1)'
    }}>
      <h4 style={{ margin: '0 0 16px 0', color: '#ffd700', textTransform: 'uppercase', fontSize: '14px' }}>
        🏗️ Архитектор Отряда
      </h4>

      {step === 1 && (
        <div>
          <label style={{ fontSize: '12px', opacity: 0.7, display: 'block', marginBottom: '8px' }}>Название Отряда (Стартапа)</label>
          <input 
            value={squadName}
            onChange={e => setSquadName(e.target.value)}
            placeholder="Напр: 'Искра', 'Космодром', 'Новые Горизонты'"
            style={{ width: '100%', padding: '12px', borderRadius: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', marginBottom: '16px' }}
          />
          <button onClick={() => setStep(2)} style={{ padding: '8px 16px', background: '#8b00ff', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>
            ДАЛЕЕ
          </button>
        </div>
      )}
      ...
      {step === 2 && (
        <div>
          <label style={{ fontSize: '12px', opacity: 0.7, display: 'block', marginBottom: '8px' }}>Код Традиций (Выберите 3 для сценария посвящения)</label>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px' }}>
            {traditionOptions.map(t => (
              <button
                key={t}
                onClick={() => setTraditions(prev => prev.includes(t) ? prev.filter(x => x !== t) : prev.length >= 3 ? prev : [...prev, t])}
                style={{
                  padding: '6px 10px',
                  borderRadius: '8px',
          ...
          {traditions.length > 0 && (
            <p style={{ fontSize: '11px', opacity: 0.6, marginBottom: '12px' }}>
              4К: {[...new Set(traditions.map(t => TRADITION_SCENARIOS[t]?.skill).filter(Boolean))].join(', ')}
            </p>
          )}
          <button
            onClick={() => setStep(3)}
      ...
      {step === 3 && (() => {
        ...
        const buildCopyText = () => {
          const lines: string[] = [
            ...
            '',
            '4К: ' + (s1?.skill && s2?.skill && s3?.skill ? [...new Set([s1.skill, s2.skill, s3.skill])].join(', ') : 'Коммуникация, Коллаборация, Креативность')
          ];
        ...
        return (
          <div style={{ textAlign: 'center', position: 'relative' }}>
            ...
            <div style={{ fontSize: '16px', fontWeight: 800, marginBottom: '8px' }}>{squadName || 'Отряд'}</div>
            <div style={{ fontSize: '12px', opacity: 0.7, marginBottom: '16px' }}>Сценарий Посвящения готов</div>
            <div style={{ textAlign: 'left', background: 'rgba(255,255,255,0.05)', padding: '16px', borderRadius: '12px', fontSize: '13px', lineHeight: 1.7, marginBottom: '16px', border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ marginBottom: '12px' }}>
                <strong>1. Сбор.</strong> Атмосфера таинственности. Вожатый коротко говорит о важности посвящения.
              </div>
              <div style={{ marginBottom: '12px' }}>
                <strong>2. {traditions[0]}</strong>
                {(s1?.duration || s1?.materials || s1?.counselorTip) && (
                  <div style={{ fontSize: '11px', opacity: 0.8, marginTop: '4px', marginLeft: '8px' }}>
                    {s1?.duration && <span>⏱ {s1.duration}</span>}
                    {s1?.materials && <span style={{ marginLeft: s1?.duration ? '12px' : 0 }}>📦 {s1.materials}</span>}
                    {s1?.counselorTip && <div style={{ marginTop: '4px', fontStyle: 'italic' }}>💡 {s1.counselorTip}</div>}
                  </div>
                )}
                <br/>
              ...
              <div style={{ marginBottom: '12px' }}>
                ...
                {(s2?.duration || s2?.materials || s2?.counselorTip) && (
                  <div style={{ fontSize: '11px', opacity: 0.8, marginTop: '4px', marginLeft: '8px' }}>
                    ...
                    {s2?.materials && <span style={{ marginLeft: s2?.duration ? '12px' : 0 }}>📦 {s2.materials}</span>}
                    {s2?.counselorTip && <div style={{ marginTop: '4px', fontStyle: 'italic' }}>💡 {s2.counselorTip}</div>}
                  </div>
                )}
                <br/>
              ...
              <div>
                ...
                {(s3?.duration || s3?.materials || s3?.counselorTip) && (
                  <div style={{ fontSize: '11px', opacity: 0.8, marginTop: '4px', marginLeft: '8px' }}>
                    ...
                    {s3?.materials && <span style={{ marginLeft: s3?.duration ? '12px' : 0 }}>📦 {s3.materials}</span>}
                    {s3?.counselorTip && <div style={{ marginTop: '4px', fontStyle: 'italic' }}>💡 {s3.counselorTip}</div>}
                  </div>
                )}
                <br/>
            ...
            <p style={{ fontSize: '11px', opacity: 0.6, marginBottom: '12px', textAlign: 'left' }}>
              4К: {(s1?.skill && s2?.skill && s3?.skill) ? [...new Set([s1.skill, s2.skill, s3.skill])].join(', ') : 'Коммуникация, Коллаборация, Креативность'}
            </p>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
              <button
                onClick={handleCopy}
                style={{ flex: 1, padding: '10px', background: 'rgba(255,255,255,0.1)', color: 'white', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '10px', fontWeight: 600, cursor: 'pointer' }}
              >
                {copyToast ? '✓ Скопировано!' : '📋 Копировать сценарий'}
              </button>
              <button
                onClick={handleApprove}
                style={{ flex: 1, padding: '12px', background: '#ffd700', color: '#1a1a2e', border: 'none', borderRadius: '10px', fontWeight: 800, cursor: 'pointer' }}
              >
                УТВЕРДИТЬ МЕТОДИКУ
              </button>
            </div>
            {copyToast && (
              <div style={{ position: 'absolute', bottom: '-8px', left: '50%', transform: 'translateX(-50%)', background: '#38ef7d', color: '#1a1a2e', padding: '4px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: 600 }}>
                Скопировано!
              </div>
            )}
          </div>
