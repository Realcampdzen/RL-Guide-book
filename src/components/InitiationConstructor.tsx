...
export const InitiationConstructor: React.FC<InitiationConstructorProps> = ({
  ...
  const [step, setStep] = useState(() => {
    try { const s = localStorage.getItem(DRAFT_STEP_KEY); return s ? Math.max(1, Math.min(4, Number(s))) : 1; } catch { return 1; }
  });
  ...
  const selectTemplate = useCallback((t: InitiationTemplate | null) => {
    ...
    setState({
      ...
      hasKapusta: t.blocks.some(b => b.title.toLowerCase().includes('капуста') || b.title.toLowerCase().includes('обнимашк')),
      hasPhoto: false,
    });
    setStep(2);
  }, []);
  ...
  const updateBlockField = useCallback((id: string, field: keyof InitiationBlock, value: string) => {
    setState(prev => ({
      ...prev,
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      const tasks = state.blocks.map((b, i) => ({
        ...
        description: [b.description, b.materials && `📦 ${b.materials}`, b.location && `📍 ${b.location}`, b.duration && `⏱ ${b.duration}`].filter(Boolean).join('\n'),
        order: i + 1,
      }));
      // Add final elements as tasks
      if (state.hasOgonyok && !state.blocks.some(b => b.type === 'bonfire')) {
        tasks.push({ id: uid(), title: '🔥 Огонёк / Костёр', description: 'Финальный Огонёк у костра', order: tasks.length + 1 });
      }
      if (state.hasKapusta) {
        tasks.push({ id: uid(), title: '🤗 Капуста (обнимашки)', description: 'Массовые обнимашки в финале', order: tasks.length + 1 });
      }
      if (state.hasPhoto) {
        tasks.push({ id: uid(), title: '📸 Фото на память', description: 'Общее фото отряда для истории', order: tasks.length + 1 });
      }
      ...
      const description = [
        ...
        state.templateId && `Шаблон: ${INITIATION_TEMPLATES.find(t => t.id === state.templateId)?.name || ''}`,
      ].filter(Boolean).join(' · ');
      ...
      localStorage.removeItem(DRAFT_KEY);
      localStorage.removeItem(DRAFT_STEP_KEY);
      setTimeout(() => { setToast(null); onCreated?.(); }, 2000);
      setState({ ...DEFAULT_STATE });
      setStep(1);
    } catch (e: any) {
  ...
  const filteredLibrary = useMemo(() => {
    ...
    return BLOCK_LIBRARY.filter(b => b.sourceTemplate === libraryFilter || b.type === libraryFilter);
  }, [libraryFilter]);

  const selectedTemplate = state.templateId ? INITIATION_TEMPLATES.find(t => t.id === state.templateId) : null;
    ...
    <div className="init-constructor">
      ...
      <div className="init-constructor__header">
        <div className="init-constructor__header-top">
          <div>
            ...
            <h3 className="init-constructor__title">
              {step === 1 ? 'Выбери основу' :
               step === 2 ? 'Настрой посвящение' :
               step === 3 ? 'Собери этапы' :
               'Финальные штрихи'}
            </h3>
          </div>
          <div className="init-constructor__steps">
            ...
            {[1,2,3,4].map(s => (
              <button
                ...
                onClick={() => { if (s <= step || (s === 2 && step >= 1)) setStep(s); }}
                disabled={s > step + 1}
              >
                {step > s ? '✓' : s}
              </button>
        ...
        <div className="init-constructor__progress-track">
          <div className="init-constructor__progress-fill" style={{ width: `${((step - 1) / 3) * 100}%` }} />
        </div>
      </div>
      ...
      {step === 1 && (
        <div className="init-constructor__section">
          <div className="init-template-grid">
            ...
            >
              <span className="init-template-card__icon" style={{ background: 'rgba(255,255,255,0.08)' }}>+</span>
            ...
            {INITIATION_TEMPLATES.map(t => (
              ...
              >
                <span className="init-template-card__icon" style={{ background: t.color }}>{t.name.charAt(0)}</span>
                <span className="init-template-card__name">{t.name}</span>
                <span className="init-template-card__squad">{t.squadName}</span>
                <span className="init-template-card__desc">{t.blocks.length} этапов · {t.durationDays}д</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {step === 2 && (
        <div className="init-constructor__section">
          {selectedTemplate && (
            <div className="init-constructor__template-hint" style={{ borderLeftColor: selectedTemplate.color }}>
              <span>Основа: <strong>{selectedTemplate.name}</strong> — {selectedTemplate.description.slice(0, 80)}…</span>
            </div>
          )}
          ...
          <div className="init-constructor__nav">
            <button type="button" className="init-constructor__btn-secondary" onClick={() => setStep(1)}>← Назад</button>
            <button
              type="button"
        ...
        <div className="init-constructor__section">
          ...
          <div className="init-constructor__blocks">
            ...
            {state.blocks.map((block, idx) => {
              ...
              return (
                <div key={block.id} className="init-block" style={{ '--block-color': meta.color } as React.CSSProperties}>
                  <div className="init-block__header">
                    <span className="init-block__type-badge">{meta.label}</span>
                    <div className="init-block__actions">
                      <button type="button" className="init-block__move" disabled={idx === 0} onClick={() => moveBlock(block.id, -1)} title="Вверх">↑</button>
                      <button type="button" className="init-block__move" disabled={idx === state.blocks.length - 1} onClick={() => moveBlock(block.id, 1)} title="Вниз">↓</button>
                      <button type="button" className="init-block__remove" onClick={() => removeBlock(block.id)} title="Удалить">×</button>
                    </div>
                  </div>
                  <input
                    type="text"
          ...
          <div className="init-constructor__add-row">
            ...
            <button type="button" className="init-constructor__btn-library" onClick={() => setShowLibrary(true)}>
          ...
          <div className="init-constructor__type-row">
            {(Object.entries(BLOCK_TYPE_META) as [BlockType, typeof BLOCK_TYPE_META[BlockType]][]).map(([type, meta]) => (
              <button
                ...
                onClick={() => addBlock({ id: uid(), type, title: '', description: '', emoji: meta.emoji })}
          ...
          <div className="init-constructor__nav">
            <button type="button" className="init-constructor__btn-secondary" onClick={() => setStep(2)}>← Назад</button>
            <button
              type="button"
              className="init-constructor__btn-primary"
        ...
        <div className="init-constructor__section">
          ...
          <div className="init-constructor__summary">
            ...
            <div className="init-constructor__summary-meta">
              ...
              <span>{state.durationDays} {state.durationDays === 1 ? 'день' : 'дня'}</span>
          ...
          <div className="init-constructor__timeline">
            {state.blocks.filter(b => b.title.trim()).map((block, idx) => {
              const meta = BLOCK_TYPE_META[block.type];
              return (
                <div key={block.id} className="init-timeline__item">
                  <div className="init-timeline__dot" style={{ background: meta.color }}>{meta.label.charAt(0)}</div>
                  <div className="init-timeline__content">
                    <div className="init-timeline__title">{block.title}</div>
                    {block.description && <div className="init-timeline__desc">{block.description.slice(0, 60)}…</div>}
                  </div>
          ...
          <div className="init-constructor__finals">
            <label className="init-constructor__check">
              <input type="checkbox" checked={state.hasOgonyok} onChange={e => setState(p => ({ ...p, hasOgonyok: e.target.checked }))} />
              <span>Финальный Огонёк / Костёр</span>
            </label>
            <label className="init-constructor__check">
              <input type="checkbox" checked={state.hasKapusta} onChange={e => setState(p => ({ ...p, hasKapusta: e.target.checked }))} />
              <span>Капуста (массовые обнимашки)</span>
            </label>
            <label className="init-constructor__check">
              <input type="checkbox" checked={state.hasPhoto} onChange={e => setState(p => ({ ...p, hasPhoto: e.target.checked }))} />
          ...
          <button
            ...
            onClick={() => { setShowTips(!showTips); setTipIndex(Math.floor(Math.random() * TIPS.length)); }}
          >
          ...
          <div className="init-constructor__nav">
            <button type="button" className="init-constructor__btn-secondary" onClick={() => setStep(3)}>← Назад к этапам</button>
            <button
              type="button"
        ...
        <div className="init-library-overlay" onClick={() => setShowLibrary(false)}>
          <div className="init-library" onClick={e => e.stopPropagation()}>
            <div className="init-library__header">
              ...
              <button type="button" className="init-library__close" onClick={() => setShowLibrary(false)}>×</button>
            </div>
              ...
              {filteredLibrary.map(block => {
                ...
                return (
                  <button
                    ...
                    onClick={() => { addBlock(block); setShowLibrary(false); }}
                    title="Нажми, чтобы добавить"
                  >
                    <div className="init-library__item-top">
                      <span className="init-library__item-badge" style={{ background: meta.color }}>{meta.label.charAt(0)}</span>
                      <span className="init-library__item-title">{block.title}</span>
                      {srcTemplate && <span className="init-library__item-source">{srcTemplate.name.charAt(0)}</span>}
                    </div>
                    <div className="init-library__item-desc">{block.description.slice(0, 80)}{block.description.length > 80 ? '…' : ''}</div>
                  </button>
                );
              })}
            </div>
          ...
          <div className="init-confirm" onClick={e => e.stopPropagation()}>
            ...
            <div className="init-confirm__details">
              ...
              <div className="init-confirm__meta">
                {state.blocks.filter(b => b.title.trim()).length} этапов · {state.durationDays} {state.durationDays === 1 ? 'день' : 'дня'} · {state.timeOfDay}
              </div>
            </div>
            ...
            <div className="init-confirm__actions">
              <button type="button" className="init-constructor__btn-secondary" onClick={() => setShowConfirm(false)}>
                Отмена
              </button>
              <button
                type="button"
