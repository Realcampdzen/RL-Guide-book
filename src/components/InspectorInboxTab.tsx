...
export const InspectorInboxTab: React.FC<InspectorInboxTabProps> = ({ accessToken }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const taskMap = new Map<
        string,
        { title: string; checklistTitle: string; skill4k?: string }
      >();
      checklists.forEach((cl: InspectorChecklist) => {
        cl.tasks.forEach((t) =>
          taskMap.set(t.id, { title: t.title, checklistTitle: cl.title, skill4k: t.skill4k })
        );
      });
  ...
  if (pending.length === 0) {
    return (
      <div style={{ padding: 12, fontSize: 12, opacity: 0.6 }}>Нет заданий на подтверждение.</div>
    );
  }
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 10, opacity: 0.6 }}>
              {item.checklistTitle}
              {item.skill4k ? ` · ${item.skill4k}` : ''}
            </div>
          </div>
          <button
            type="button"
            className="btn-secondary"
            style={{ padding: '4px 10px', fontSize: 10, color: '#22c55e' }}
