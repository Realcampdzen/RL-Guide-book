...
export const ParentsContainer: React.FC<ParentsContainerProps> = ({
  ...
  const [campFacts, setCampFacts] = useState<{
    address?: { campName?: string; base?: string; address?: string; route?: string };
    contacts?: {
      phone?: string;
      email?: string;
      vk?: string;
    ...
    currentSeason?: { name?: string; dates?: string; price?: string; theme?: string };
    documents?: string[];
  } | null>(null);
  ...
  const [parentInsights, setParentInsights] = useState<{
    overallProgress?: { percent?: number; stage?: string };
    weeklyTrend?: { direction?: 'up' | 'flat' | 'down'; note?: string };
    dynamicSignals?: {
      windowDays?: number;
      currentWindowAchievements?: number;
      previousWindowAchievements?: number;
    };
    ...
    strengthsTop3?: Array<{ title?: string }>;
    nextSteps?: Array<{ hint?: string }>;
  } | null>(null);
  ...
  const fallbackParentInsights = useMemo(() => {
    ...
    return {
      overallProgress: {
        percent,
        stage: percent >= 80 ? 'high' : percent >= 40 ? 'steady' : 'start',
      },
      weeklyTrend: {
        direction: 'flat',
        note: 'История прогресса только формируется — начните с одного посильного шага на этой неделе.',
      },
      ...
      basedOn: {
        ...
        strongestAreas: [],
        weakestAreas: [],
        activityWindow: 'последние 7 дней и предыдущие 7 дней',
      },
      strengthsTop3: [
        {
          title:
            percent >= 60
              ? 'Ребёнок уверенно завершает начатые шаги'
              : 'Ребёнок включён в лагерный процесс',
        },
  ...
  useEffect(() => {
    ...
    const apiUrl = useLocalApi
      ? '/api/parent-insights'
      : `${(import.meta.env.VITE_API_URL || import.meta.env.VITE_BACKEND_URL || '').replace(/\/$/, '')}/api/parent-insights`;
    let cancelled = false;
  ...
  return (
    <div
      id="parents-section"
      className="profile-view-parents-section"
      style={{ marginTop: 24, display: 'flex', flexDirection: 'column', gap: 20 }}
    >
      <h2 style={{ fontSize: 20, fontWeight: 800, color: 'rgba(255,255,255,0.95)', margin: 0 }}>
        Для родителей
      </h2>
      ...
      {parentSectionMode === 'home' && campFactsError && (
        <p style={{ fontSize: 13, margin: 0, color: '#f59e0b' }}>
          Проверьте подключение. {campFactsError}
        </p>
      )}
        ...
        <>
          <div className="parents-section-block">
            ...
            {(campFacts.currentSeason?.name || campFacts.currentSeason?.theme) && (
              <div>
                {campFacts.currentSeason?.name && (
                  <p className="parents-section-block__text" style={{ margin: 0, fontWeight: 600 }}>
                    {campFacts.currentSeason.name}
                  </p>
                )}
                {campFacts.currentSeason?.theme && (
                  <p style={{ fontSize: 12, margin: '4px 0 0' }}>{campFacts.currentSeason.theme}</p>
                )}
              </div>
            )}
          ...
          {campFacts.address &&
            (campFacts.address.campName ||
              campFacts.address.base ||
              campFacts.address.address ||
              campFacts.address.route) && (
              <div className="parents-section-block">
                ...
                <div style={{ fontSize: 13, opacity: 0.9 }}>
                  ...
                  {campFacts.address.address && (
                    <p style={{ margin: '4px 0 0' }}>{campFacts.address.address}</p>
                  )}
                  {campFacts.address.route && (
                    <p style={{ margin: '4px 0 0' }}>Как добраться: {campFacts.address.route}</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="parents-section-block">
              ...
              <div className="parents-section__contacts">
                {campFacts.contacts.phone && (
                  <a href={`tel:${campFacts.contacts.phone.replace(/\s/g, '')}`}>
                    {campFacts.contacts.phone}
                  </a>
                )}
                ...
                {campFacts.contacts.telegram && (
                  <a href={campFacts.contacts.telegram} target="_blank" rel="noopener noreferrer">
                    Telegram
                  </a>
                )}
                {campFacts.contacts.site && (
                  <a href={campFacts.contacts.site} target="_blank" rel="noopener noreferrer">
                    Сайт
                  </a>
                )}
                {campFacts.contacts.vk && (
                  <a href={campFacts.contacts.vk} target="_blank" rel="noopener noreferrer">
                    ВКонтакте
                  </a>
                )}
                {campFacts.contacts.organizer && (
                  <a href={campFacts.contacts.organizer} target="_blank" rel="noopener noreferrer">
      ...
      {parentSectionMode === 'home' && !campFactsLoading && !campFactsError && !campFacts && (
        <p className="parents-section-block__text" style={{ margin: 0 }}>
          По вопросам документов и бронирования — контакты в разделе «О лагере».
        </p>
      )}
      ...
      {parentSectionMode === 'home' && typeof onNavigateToRegistrationForm === 'function' && (
        <button
          type="button"
          onClick={onNavigateToRegistrationForm}
      ...
      {parentSectionMode === 'child' && (
        <>
          <div
            className="parents-section-block"
            style={{ display: 'flex', flexDirection: 'column', gap: 10 }}
          >
            <h3 className="parents-section-block__heading" style={{ margin: 0 }}>
              Витрина прогресса ребёнка
            </h3>
            <p className="parents-section-block__text" style={{ margin: 0 }}>
              Здесь только безопасный read-only просмотр. Изменять прогресс ребёнка нельзя.
            </p>
            <button
              type="button"
              onClick={() => setShowChildBadges(true)}
              className="parents-section__btn-child"
          ...
          >
            <h3 className="parents-section-block__heading" style={{ margin: 0 }}>
              Рекомендации для поддержки ребёнка
            </h3>
            {parentInsightsLoading && (
              <p className="parents-section-block__text" style={{ margin: 0 }}>
                Собираем понятную сводку прогресса и ближайших шагов…
              </p>
            )}
            {!parentInsightsLoading && (
              <>
                <p className="parents-section-block__text" style={{ margin: 0 }}>
                  Общий прогресс:{' '}
                  <strong>
                    {parentInsights?.overallProgress?.percent ??
                      fallbackParentInsights?.overallProgress?.percent ??
                      0}
                    %
                  </strong>
                </p>
                <p className="parents-section-block__text" style={{ margin: 0 }}>
                  Тренд недели:{' '}
                  <strong>
                    {(parentInsights?.weeklyTrend?.direction ??
                      fallbackParentInsights?.weeklyTrend?.direction ??
                      'flat') === 'up'
                      ? 'рост'
                      : (parentInsights?.weeklyTrend?.direction ??
                            fallbackParentInsights?.weeklyTrend?.direction ??
                  ...
                  {' — '}
                  {parentInsights?.weeklyTrend?.note ??
                    fallbackParentInsights?.weeklyTrend?.note ??
                    'Темп ровный, поддерживайте регулярный ритм.'}
                </p>
                <div>
                  <div className="parents-section-block__label">Что уже хорошо</div>
                  <ul style={{ margin: '6px 0 0 18px', padding: 0 }}>
                    {(
                      parentInsights?.strengthsTop3 ||
                      fallbackParentInsights?.strengthsTop3 || [
                        {
                          title:
                            'Когда будет доступна витрина ребёнка, здесь появятся сильные стороны и достижения.',
                        },
                      ]
                    )
                      .slice(0, 3)
                      .map((s, idx) => (
                        <li
                          key={`pi-s-${idx}`}
                          className="parents-section-block__text"
                          style={{ margin: 0 }}
                        >
                          {s?.title}
                        </li>
                ...
                <div>
                  ...
                  <ul style={{ margin: '6px 0 0 18px', padding: 0 }}>
                    {(
                      parentInsights?.nextSteps ||
                      fallbackParentInsights?.nextSteps || [
                        { hint: 'Откройте витрину ребёнка, чтобы помочь ему с прогрессом.' },
                      ]
                    )
                      .slice(0, 2)
                      .map((s, idx) => (
      ...
      <div className="parents-section__actions">
          ...
          <button
            ...
            onClick={() => {
              if (
                !canRunParentChildMutation({
                  role,
                  hasChildProgressSnapshot: isParentChildReadonlyView,
                })
              )
                return;
              onOpenRouteForm();
        ...
        <p style={{ fontSize: 12, opacity: 0.7, margin: 0 }}>
          Предложить идею для лагеря — в блоке «Совет Лагеря» ниже.
        </p>
      </div>
