...
export const OrganizerContainer: React.FC<OrganizerContainerProps> = ({
  ...
  const [organizerShifts, setOrganizerShifts] = useState<
    Array<{
      id: string;
      name: string;
      startDate: string;
  ...
  >([]);
  const [organizerSquadsMap, setOrganizerSquadsMap] = useState<
    Record<
      string,
      Array<{
        id: string;
  ...
  const [organizerShiftForm, setOrganizerShiftForm] = useState({
    name: '',
    startDate: '',
    endDate: '',
  });
  ...
  const [organizerCodeForm, setOrganizerCodeForm] = useState({
    deviceId: '',
  ...
  const canUseOrganizerApiForRead =
    canReadShiftsAndSquads &&
    (Boolean(accessToken) || (organizerApiBase === '' && role === 'developer'));
  const canUseOrganizerApiForManage =
    canManageShiftsAndSquads &&
    (Boolean(accessToken) || (organizerApiBase === '' && role === 'developer'));
  ...
  const loadOrganizerData = useCallback(async () => {
    ...
    if (!canUseOrganizerApiForRead) {
      ...
      setOrganizerSquadsMap({});
      setOrganizerError(
        'Войдите по коду, чтобы просматривать смены и отряды (или используйте локальный режим разработчика).'
      );
      return;
    }
    ...
    try {
      ...
      for (const shift of shifts) {
        const r = await fetch(`${organizerApiBase}/api/shifts/${shift.id}/squads`, {
        ...
        if (!r.ok) {
          ...
          if (!partialError)
            partialError = formatOrganizerHttpError(
              r.status,
              payload,
              `Отряды смены «${shift.name || shift.id}»`
            );
          map[shift.id] = [];
          continue;
        }
        const squadData = (await r.json().catch(() => ({}))) as {
          squads?: Array<{
            id: string;
            shiftId: string;
            name: string;
      ...
      if (
        ...
        setOrganizerError(
          'Backend недоступен. Проверьте, что API запущен на http://localhost:4000 и dev proxy /api активен.'
        );
      } else {
  ...
  const removeShiftWithCleanup = useCallback(
    async (shift: { id: string; name: string }) => {
      ...
      if (!window.confirm(`Удалить смену «${shift.name}» вместе со всеми отрядами и данными?`))
        return;
      setOrganizerLoading(true);
      setOrganizerError(null);
      try {
  ...
  const removeSquadWithCleanup = useCallback(
    ...
    return createPortal(
      <div className="profile-organizer-modals-root" aria-live="polite">
        {organizerShiftFormOpen && (
          <div
            className="profile-utility-panel-overlay profile-utility-panel-overlay--organizer"
            onClick={() => setOrganizerShiftFormOpen(false)}
            aria-hidden="true"
          />
        )}
        {organizerShiftFormOpen && (
          <div
            className="profile-utility-panel profile-utility-panel--organizer-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="organizer-modal-shift-title"
          ...
          >
            ...
            <div className="profile-utility-panel-body">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  <label htmlFor="organizer-shift-name" style={{ fontSize: 12, opacity: 0.8 }}>
                    Название смены
                  </label>
                  <input
                    id="organizer-shift-name"
                    value={organizerShiftForm.name}
                    onChange={(e) => setOrganizerShiftForm((f) => ({ ...f, name: e.target.value }))}
                ...
                <div>
                  <label htmlFor="organizer-shift-start" style={{ fontSize: 12, opacity: 0.8 }}>
                    Дата начала
                  </label>
                  <input
                    id="organizer-shift-start"
                    type="date"
                    value={organizerShiftForm.startDate}
                    onChange={(e) =>
                      setOrganizerShiftForm((f) => ({ ...f, startDate: e.target.value }))
                    }
                    style={{
                      display: 'block',
                      width: '100%',
                ...
                <div>
                  <label htmlFor="organizer-shift-end" style={{ fontSize: 12, opacity: 0.8 }}>
                    Дата окончания
                  </label>
                  <input
                    id="organizer-shift-end"
                    type="date"
                    value={organizerShiftForm.endDate}
                    onChange={(e) =>
                      setOrganizerShiftForm((f) => ({ ...f, endDate: e.target.value }))
                    }
                    style={{
                      display: 'block',
                      width: '100%',
                ...
                <button
                  ...
                  disabled={
                    !organizerShiftForm.name.trim() ||
                    organizerLoading ||
                    !canUseOrganizerApiForManage
                  }
                  onClick={async () => {
                    if (!canUseOrganizerApiForManage) {
                      setOrganizerError(
                        'Для управления сменами войдите по коду (или используйте локальный режим разработчика).'
                      );
                      return;
                    }
                    setOrganizerLoading(true);
                    ...
                    try {
                      const res = await fetch(`${organizerApiBase}/api/shifts`, {
                        ...
                        body: JSON.stringify({
                          name: organizerShiftForm.name.trim(),
                          startDate: organizerShiftForm.startDate,
                          endDate: organizerShiftForm.endDate,
                        }),
                      });
                      const data = (await res.json().catch(() => ({}))) as {
                        shift?: {
                      ...
                      if (!res.ok) {
                        setOrganizerError(
                          formatOrganizerHttpError(res.status, data, 'Создание смены')
                        );
                        return;
                      }
        ...
        {organizerSquadFormOpen && (
          <div
            className="profile-utility-panel-overlay profile-utility-panel-overlay--organizer"
            onClick={() => setOrganizerSquadFormOpen(false)}
            aria-hidden="true"
          />
        )}
        {organizerSquadFormOpen && (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="organizer-modal-squad-title"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="profile-utility-panel-header">
            ...
            <div className="profile-utility-panel-body">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  <label htmlFor="organizer-squad-name" style={{ fontSize: 12, opacity: 0.8 }}>
                    Название отряда
                  </label>
                  <input
                    id="organizer-squad-name"
                    value={organizerSquadFormName}
                    onChange={(e) => setOrganizerSquadFormName(e.target.value)}
                ...
                <button
                  type="button"
                  className="btn-primary-gold"
                  aria-label="Добавить отряд"
                  disabled={
                    !organizerSquadFormName.trim() ||
                    organizerLoading ||
                    !canUseOrganizerApiForManage
                  }
                  onClick={async () => {
                    ...
                    try {
                      const res = await fetch(
                        `${organizerApiBase}/api/shifts/${organizerSquadFormShiftId}/squads`,
                        {
                          method: 'POST',
                          headers: getOrganizerHeaders(true),
                      ...
                      if (!res.ok) {
                        setOrganizerError(
                          formatOrganizerHttpError(res.status, data, 'Создание отряда')
                        );
                        return;
                      }
                      if (data.squad) {
                        setOrganizerSquadsMap((prev) => ({
                          ...prev,
                          [organizerSquadFormShiftId]: [
                            ...(prev[organizerSquadFormShiftId] || []),
                            data.squad!,
                          ],
                        }));
                      }
        ...
        {organizerCodeModalOpen && (
          <div
            className="profile-utility-panel-overlay profile-utility-panel-overlay--organizer"
            onClick={() => {
              setOrganizerCodeModalOpen(false);
        ...
        {organizerCodeModalOpen && (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="organizer-modal-code-title"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="profile-utility-panel-header">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  ...
                  <p style={{ margin: '0 0 4px', fontSize: 11, opacity: 0.75 }}>
                    Идентификатор устройства участника, к которому привязывается код (обычно
                    подставляется автоматически).
                  </p>
                  <input
                    value={organizerCodeForm.deviceId}
                    onChange={(e) =>
                      setOrganizerCodeForm((f) => ({ ...f, deviceId: e.target.value }))
                    }
                ...
                <div>
                  ...
                  <select
                    value={organizerCodeForm.role}
                    onChange={(e) =>
                      setOrganizerCodeForm((f) => ({ ...f, role: e.target.value as UserRole }))
                    }
                    style={{
                      display: 'block',
                      width: '100%',
                ...
                <div>
                  ...
                  <select
                    value={organizerCodeForm.shiftId}
                    onChange={(e) =>
                      setOrganizerCodeForm((f) => ({ ...f, shiftId: e.target.value }))
                    }
                    style={{
                      display: 'block',
                      width: '100%',
                ...
                <button
                  type="button"
                  className="btn-primary-gold"
                  aria-label="Сгенерировать код верификации"
                  disabled={
                    !organizerCodeForm.deviceId.trim() ||
                    organizerLoading ||
                    !canUseOrganizerApiForManage
                  }
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (res.status === 401) {
                        setOrganizerCodeResult('Сессия истекла. Войдите снова.');
                        fireOn401();
                        return;
                      }
                      if (!res.ok) {
                        setOrganizerCodeResult(
                          formatOrganizerHttpError(res.status, data, 'Генерация кода')
                        );
                        return;
                      }
                      if (data.code) {
                        setOrganizerCodeResult(
                          `Успешно сгенерирован код:\n\n${data.code}\n\nНикому не передавайте и используйте его для входа.`
                        );
                      }
                    } catch (e) {
                      const fallbackMessage =
                        e instanceof Error ? e.message : 'Ошибка генерации кода';
  ...
  return (
    <div
      id="organizer-shifts-tab-section"
      className="profile-view-parents-section organizer-shifts-section"
    >
      <h2 className="organizer-shifts-section__heading">Смены и отряды</h2>
      <div
        className="organizer-empty-state organizer-empty-state--squads organizer-block-darkened"
        style={{ marginBottom: 12 }}
      >
        <p className="organizer-empty-state__title">Как открыть кабинет отряда</p>
        <p className="organizer-empty-state__text">
          1. Создайте смену. 2. Добавьте отряд. 3. Вступите в отряд по коду/ссылке. 4. Перейдите в
          Отрядный уголок.
        </p>
      </div>
      <div
      ...
      >
        ...
        {mySquadInfo?.membership ? (
          <div style={{ marginTop: 6, display: 'grid', gap: 4 }}>
            <div>
              Смена:{' '}
              <strong>{mySquadInfo.shift?.name || mySquadInfo.membership.campId || '—'}</strong>
            </div>
            <div>
              Отряд:{' '}
              <strong>{mySquadInfo.squad?.name || mySquadInfo.membership.squadId || '—'}</strong>
            </div>
          ...
          <p style={{ margin: '6px 0 0', opacity: 0.85 }}>
            Вы пока не состоите в отряде. Вступите по коду ниже.
          </p>
        )}
      </div>
        ...
        <>
            ...
            {organizerShifts.map((shift) => (
              <div key={shift.id} className="organizer-shift-card parents-section-block">
                <div className="organizer-shift-card__header">
                  <div>
                    <h3 className="organizer-shift-card__title parents-section-block__heading">
                      {shift.name}
                    </h3>
                  ...
                  >
                    ...
                    {canDeleteShiftsAndSquads &&
                      (shift.name || '').trim().toLowerCase() !==
                        DEFAULT_SHIFT_NAME.toLowerCase() && (
                        <button
                          type="button"
                          className="btn-secondary"
                          style={{ padding: '6px 12px' }}
                ...
                {(organizerSquadsMap[shift.id] || []).length > 0 ? (
                    ...
                    {(organizerSquadsMap[shift.id] || []).map((s) => (
                      <li key={s.id} className="organizer-squad-row">
                          ...
                          onClick={() => {
                            if (role === 'parent') {
                              showHint({
                                title: 'Отряд',
                                content:
                                  'Для родителя вступление доступно по коду приглашения или через кнопку «Подать заявку».',
                              });
                              return;
                            }
                        ...
                        >
                          ...
                          <span className="organizer-squad-row__name">
                            {openingSquadId === s.id ? 'Открываем...' : s.name}
                          </span>
                        </button>
                        {accessToken && !canDeleteShiftsAndSquads && (
                          <button
                            ...
                            disabled={
                              Boolean(squadJoinRequestBusyId) ||
                  ...
                  <div className="organizer-empty-state organizer-empty-state--squads">
                    ...
                    <p className="organizer-empty-state__text">
                      Добавьте первый отряд, чтобы участники могли вступать по коду или ссылке.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="organizer-empty-state">
                {!canUseOrganizerApiForRead ? (
                  <>
                    ...
                    <p className="organizer-empty-state__text">
                      Войдите по коду, чтобы видеть смены и отряды (или используйте локальный режим
                      разработчика).
                    </p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="organizer-empty-state__text">
                      Создайте первую смену, чтобы добавлять отряды и выдавать коды.
                    </p>
                  </>
                )}
              </div>
          ...
          <div className="organizer-shifts-actions">
            ...
            <button
              ...
              className="btn-secondary"
              style={{ padding: '10px 20px' }}
              aria-label="Обновить список смен и отрядов"
              disabled={organizerLoading || !canUseOrganizerApiForRead}
              onClick={() => void loadOrganizerData()}
            >
              Обновить
            </button>
            {canManageShiftsAndSquads && (
              <button
                ...
                onClick={() => {
                  ...
                  setOrganizerCodeForm({
                    deviceId: deviceId || '',
                    role: 'participant',
                    shiftId: organizerShifts[0]?.id || '',
                  });
                  setOrganizerCodeResult(null);
