  ...
  const handleWorkshopSubmit = async () => {
    ...
    try {
      const created = await createWorkshopProposal(accessToken, {
        type: workshopProposalType === 'badge' ? 'badge' : workshopProposalType === 'version' ? 'version' : 'category',
            ...
            <div className="workshop-form workshop-form--card">
              ...
              {proposalType === 'badge' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <button onClick={handleWorkshopSubmit} disabled={!workshopForm.title.trim() || workshopProposalsBusy} className="btn-primary-gold" style={{ width: '100%' }}>
              ...
              {proposalType === 'category' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  }} disabled={!workshopForm.title.trim() || workshopProposalsBusy} className="btn-primary-gold" style={{ width: '100%' }}>
              ...
              {proposalType === 'version' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  }} disabled={!workshopForm.title.trim() || !workshopForm.level1?.trim() || workshopProposalsBusy} className="btn-primary-gold" style={{ width: '100%' }}>
              ...
              {workshopForm.image && (
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  ...
                  <button type="button" className="btn-primary-gold" style={{ fontSize: 12 }}
                    ...
                    }}>📤 {workshopProposalsBusy ? 'Отправляем...' : 'Отправить арт'}</button>
                </div>
              )}
            </div>
          ) : (
