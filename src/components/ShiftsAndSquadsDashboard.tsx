...
export const ShiftsAndSquadsDashboard: React.FC<ShiftsAndSquadsDashboardProps> = ({
    ...
    const [shiftForm, setShiftForm] = useState({ name: '', startDate: '', endDate: '', durationDays: 9 });
    const [squadFormShiftId, setSquadFormShiftId] = useState('');
    const [squadFormName, setSquadFormName] = useState('');

    const headers = useCallback((withJson = false): Record<string, string> => {
    ...
    const loadData = useCallback(async () => {
        if (!canRead) { setShifts([]); setSquadsMap({}); return; }
        setLoading(true);
        setError(null);
        try {
            ...
            if (import.meta.env.DEV) {
                const devShifts: Shift[] = [
                    { id: '64f3ca208702', name: 'Весенняя смена 2026', startDate: '', endDate: '', createdAt: '2026-02-25T22:06:08Z', createdBy: 'seed-dev-mode' },
                    { id: 'b8a1e47c3d59', name: 'Летняя смена 2026', startDate: '2026-06-01', endDate: '2026-06-21', createdAt: '2026-03-06T00:00:00Z', createdBy: 'seed-dev-mode' },
                ];
                const devSquads: Record<string, Squad[]> = {
                    '64f3ca208702': [{ id: '9f555852b996', shiftId: '64f3ca208702', name: 'СТО ЛЕТ ЛЕТА', createdAt: '2026-02-28T03:48:12Z' }],
                    'b8a1e47c3d59': [],
                };
                setShifts(devShifts);
    ...
    const createShift = useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok) { setError(`Ошибка создания: ${res.status}`); return; }
    ...
    const deleteShift = useCallback(async (shiftId: string, shiftName: string) => {
        ...
        try {
            const res = await fetch(`${apiBase}/api/shifts/${shiftId}`, { method: 'DELETE', headers: headers() });
            if (!res.ok) { setError(`Ошибка удаления: ${res.status}`); return; }
            setSelectedShiftId(null);
            await loadData();
        } catch (e) { setError(e instanceof Error ? e.message : 'Ошибка'); }
    }, [apiBase, headers, loadData]);

    const deleteSquad = useCallback(async (squadId: string, squadName: string) => {
        if (!confirm(`Удалить отряд «${squadName}»?`)) return;
        try {
            const res = await fetch(`${apiBase}/api/squads/${squadId}`, { method: 'DELETE', headers: headers() });
            if (!res.ok) { setError(`Ошибка удаления: ${res.status}`); return; }
            await loadData();
        } catch (e) { setError(e instanceof Error ? e.message : 'Ошибка'); }
    }, [apiBase, headers, loadData]);
    ...
    const cardStyle: React.CSSProperties = {
        ...
        background: 'rgba(15, 10, 42, 0.12)', border: '1px solid rgba(255,255,255,0.08)',
        backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)',
    };
    const btnStyle: React.CSSProperties = {
        padding: '8px 16px', borderRadius: 10,
        border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.06)',
        color: '#e8f0ff', fontSize: 12, fontWeight: 600,
        cursor: 'pointer', fontFamily: FONT,
    };
    const btnPrimaryStyle: React.CSSProperties = {
        ...btnStyle, background: 'rgba(93,228,255,0.15)', color: '#5de4ff',
    ...
    if (!canRead) {
        return (
            <div className="fade-in" style={{ ...cardStyle, textAlign: 'center' }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(93,228,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                    <svg viewBox="0 0 24 24" fill="none" stroke="#5de4ff" strokeWidth="1.5" width="20" height="20"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
                </div>
                <h3 style={{ margin: '0 0 8px', fontSize: 16, color: '#e8f0ff' }}>Доступ к сменам и отрядам</h3>
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>Войдите по коду, чтобы видеть смены и отряды.</p>
            </div>
        );
    }
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <button type="button" style={{ ...btnStyle, padding: '6px 12px', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }}
                    ...
                    <div
                        ...
                        style={{
                            ...
                            fontSize: 36, fontWeight: 700, color: '#e8f0ff',
                    ...
                    {canManageSquads && (
                        <button type="button" style={btnPrimaryStyle}
                            onClick={() => { setSquadFormShiftId(selectedShift.id); setSquadFormName(''); }}>
                            + Отряд
                        </button>
                    )}
                </div>
                ...
                {selectedSquads.length > 0 ? (
                        ...
                        {selectedSquads.map(s => (
                            ...
                            >
                                <div style={{
                                    ...
                                    background: 'linear-gradient(135deg, rgba(138,130,255,0.3), rgba(93,228,255,0.2))',
                                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                                ...
                                {canManageSquads && (
                                    <button type="button" title="Удалить отряд"
                                        ...
                                        style={{ ...btnStyle, padding: '6px 10px', fontSize: 14, color: '#ff6b6b', borderColor: 'rgba(255,107,107,0.2)' }}>
                                        ×
                                    </button>
                                )}
                    ...
                    <div className="fade-in" style={{ ...cardStyle, textAlign: 'center' }}>
                        <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(93,228,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                            <svg viewBox="0 0 24 24" fill="none" stroke="#5de4ff" strokeWidth="1.5" width="20" height="20"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>
                        </div>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <input
                            ...
                            style={{
                                ...
                                color: '#e8f0ff', fontSize: 13, fontFamily: FONT,
                            }}
                        />
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#e8f0ff' }}>Выберите смену</h3>
            ...
            {error && (
                <div style={{ padding: 12, borderRadius: 10, background: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.2)', fontSize: 12, color: '#ff6b6b' }}>
                    {error}
                </div>
            )}
                    ...
                    >
                        <div 
                            ...
                            background: shift.name.toLowerCase().includes('весен') ? 'linear-gradient(135deg, rgba(77,205,196,0.3), rgba(93,228,255,0.2))'
                                : shift.name.toLowerCase().includes('лет') ? 'linear-gradient(135deg, rgba(255,215,0,0.3), rgba(255,159,67,0.2))'
                                    : 'linear-gradient(135deg, rgba(138,130,255,0.3), rgba(93,228,255,0.2))',
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontSize: 24, fontWeight: 700, color: '#e8f0ff',
                            overflow: 'hidden',
                        }}>
                            {shift.avatarUrl ?
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginTop: 4 }}>
                                {squadCount > 0 ? `${squadCount} отряд${squadCount > 1 ? (squadCount < 5 ? 'а' : 'ов') : ''}` : 'Нет отрядов'}
                            </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            {canManageShifts && (
                                <button type="button" title="Удалить смену"
                                    onClick={(e) => { e.stopPropagation(); void deleteShift(shift.id, shift.name); }}
                                    style={{ ...btnStyle, padding: '6px 10px', fontSize: 14, color: '#ff6b6b', borderColor: 'rgba(255,107,107,0.2)' }}>
                                    ×
                                </button>
                            )}
            ...
            {shifts.length === 0 && !loading && (
                <div className="fade-in" style={{ ...cardStyle, textAlign: 'center' }}>
                    <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(93,228,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="#5de4ff" strokeWidth="1.5" width="20" height="20"><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><line x1="3" y1="9" x2="21" y2="9" /><line x1="9" y1="21" x2="9" y2="9" /></svg>
                    </div>
                    <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', margin: 0 }}>Пока нет смен. Создайте первую.</p>
                </div>
            )}
                ...
                <div className="fade-in" style={cardStyle}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        <input
                            ...
                            style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.04)', color: '#e8f0ff', fontSize: 13, fontFamily: FONT }}
                        />
                        <div style={{ display: 'flex', gap: 8 }}>
                            <input
                                type="date" value={shiftForm.startDate}
                                onChange={e => setShiftForm(f => ({ ...f, startDate: e.target.value }))}
                                style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.04)', color: '#e8f0ff', fontSize: 13, fontFamily: FONT }}
                            />
                            <input
                        ...
                        {/* Duration selector */}
                        <div>
                            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', marginBottom: 6 }}>Длительность смены</div>
                            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                <button type="button" style={{ ...btnStyle, ...(shiftForm.durationDays === 9 ? { background: 'rgba(93,228,255,0.15)', color: '#5de4ff', borderColor: 'rgba(93,228,255,0.25)' } : {}) }} onClick={() => setShiftForm(f => ({ ...f, durationDays: 9 }))}>9 дней</button>
                                <button type="button" style={{ ...btnStyle, ...(shiftForm.durationDays === 21 ? { background: 'rgba(93,228,255,0.15)', color: '#5de4ff', borderColor: 'rgba(93,228,255,0.25)' } : {}) }} onClick={() => setShiftForm(f => ({ ...f, durationDays: 21 }))}>21 день</button>
                                <input
                                    type="number" min="1" max="90" value={shiftForm.durationDays}
                                    onChange={e => setShiftForm(f => ({ ...f, durationDays: Math.max(1, parseInt(e.target.value) || 1) }))}
                                    style={{ width: 60, padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.04)', color: '#e8f0ff', fontSize: 13, fontFamily: FONT, textAlign: 'center' }}
                                />
                                <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>дн.</span>
                            </div>
                        </div>
                        <div style={{ display: 'flex', gap: 8 }}>
                            <button type="button" style={btnPrimaryStyle} onClick={() => void createShift()}>Создать смену</button>
                            <button type="button" style={btnStyle} onClick={() => setShiftFormOpen(false)}>Отмена</button>
                        </div>
                    </div>
                </div>
            )}
