...
const GRADIENT =
  'linear-gradient(135deg, rgba(217, 119, 6, 0.08) 0%, rgba(120, 53, 15, 0.12) 100%)';
...
export const SquadCornerDashboard: React.FC<SquadCornerDashboardProps> = ({
  ...
  const readOnlyHint = !canEditCorner ? (
    ...
    >
      ...
      <div style={{ fontSize: 12, opacity: 0.8 }}>
        {hasSquadMembership === false
          ? 'Сначала вступите в отряд, чтобы заполнить уголок.'
          : 'Недостаточно прав для редактирования уголка.'}
      </div>
      {(onOpenShiftsAndSquads || onOpenCabinet) && (
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 }}>
          ...
          {onOpenCabinet && (
            <button type="button" className="btn-secondary" onClick={onOpenCabinet}>
              Открыть кабинет
            </button>
          )}
        </div>
  ...
  const [localPlanGridA, setLocalPlanGridA] = useState<PlanGridData>(() =>
    squad.planGridA
      ? { shiftLength: squad.planGridA.shiftLength, days: { ...squad.planGridA.days } }
      : defaultPlanGrid()
  );
  const [localPlanGridB, setLocalPlanGridB] = useState<PlanGridData>(() =>
    squad.planGridB
  ...
  const hasAtLeastOnePhoto = useMemo(
    () =>
      [localPhotoCorner, localPhotoFlag, localPhotoSquad, localPhotoWithCounselors].some(
        isImageUrl
      ),
  ...
  const sectionWrap = (
    ...
    <div
      className={variant === 'cabin' ? 'squad-corner-cabin-section' : undefined}
      style={variant === 'accordion' ? card : {}}
    >
      {variant === 'accordion' && (
        <button type="button" onClick={onToggle} style={headerBtn}>
          {title}
      ...
      {(variant === 'cabin' || open) && (
        <div
          style={{
            marginTop: variant === 'accordion' ? 16 : 0,
            display: 'flex',
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <button
                  type="button"
                  className="btn-secondary"
                  onClick={() => approveFlagBadgeRequest(req.badgeId)}
                >
                  Вожатый утвердил
                </button>
              </div>
        ...
        <div key={key}>
          ...
          <ImageSourceBlock
              ...
              canEditCorner
                ? async (o) =>
                    requestImageGenerate(
                      {
                        mode: 'generate',
                        context: key === 'photoWithCounselors' ? 'counselor_squad' : 'squad_corner',
                        prompt: o.prompt ?? '',
                      },
                      accessToken ?? null
                    )
            ...
            lockReason={
              !canEditCorner
                ? hasSquadMembership === false
                  ? 'Сначала вступите в отряд, чтобы добавлять фото.'
                  : 'Недостаточно прав для добавления фото.'
                : undefined
            }
  ...
  const summary = (
    <div
      style={{
        display: 'flex',
        justifyContent: 'space-between',
        marginBottom: variant === 'accordion' && isExpanded ? 20 : 12,
      }}
    >
      <div
        onClick={variant === 'accordion' ? () => setIsExpanded((v) => !v) : undefined}
        style={{ cursor: variant === 'accordion' ? 'pointer' : 'default', flex: 1, minWidth: 0 }}
      >
        <div
        ...
        >
          <div style={{ minWidth: 0 }}>
            ...
            <h3 style={{ margin: 0, fontSize: 18 }}>
              {localSquadName.trim() || mySquadName || 'Отряд'}
            </h3>
            <div
              className={`m3-status-chip squad-corner-readiness-chip tone-${getSquadCornerReadinessTone(readiness)}`}
            >
              {getSquadCornerReadinessLabel(readiness)}
            </div>
          </div>
          <div
            style={{
  ...
  if (variant === 'accordion') {
    return (
      <div
        className="squad-corner-dashboard"
        style={{
          background: GRADIENT,
          borderRadius: 24,
          padding: 20,
              ...
              [
                {
                  ...
                  value: localSquadName,
                  onChange: setLocalSquadName,
                  multiline: false,
                  placeholder: 'Как называется ваш отряд?',
                },
                {
                  emoji: '💬',
                  label: 'Девиз',
                  value: localSquadMotto,
                  onChange: setLocalSquadMotto,
                  multiline: false,
                  placeholder: 'Девиз отряда — короткая фраза',
                },
                {
                  emoji: '📣',
                  label: 'Кричалки',
                  value: localSquadChants,
                  onChange: setLocalSquadChants,
                  multiline: true,
                  placeholder: 'Кричалки, речёвки, слоганы...',
                  rows: 3,
                },
                {
                  emoji: '👋',
                  label: 'Приветствие',
                  value: localSquadGreeting,
                  onChange: setLocalSquadGreeting,
                  multiline: false,
                  placeholder: 'Как отряд приветствует гостей?',
                },
                {
                  emoji: '😄',
                  label: 'Мемы / традиции',
                  value: localSquadMemes,
                  onChange: setLocalSquadMemes,
                  multiline: true,
                  placeholder: 'Внутренние шутки, традиции, особенности...',
                  rows: 3,
                },
              ] as const
            ).map((f, i, arr) => (
        ...
        >
          <button
            ...
            style={{
              ...
              background: saveBusy
                ? 'rgba(217,119,6,0.3)'
                : `linear-gradient(135deg, ${ACCENT}, #f59e0b)`,
              color: '#fff',
              fontSize: 14,
              fontWeight: 800,
          ...
          {saveStatus && (
            <span
              style={{
                fontSize: 13,
                fontWeight: 600,
                color: saveStatus.includes('✓') ? '#86efac' : '#fca5a5',
              }}
            >
              {saveStatus}
            </span>
  ...
  return (
    <div
      className="fade-in squad-corner-cabin-content"
      style={{
        maxWidth: 720,
        margin: '0 auto',
        width: '100%',
