...
import {
  ...
  rejectRoleRequest,
} from '../utils/roleRequestAdminApi';
...
export const RoleRequestsAdminPanel: React.FC<RoleRequestsAdminPanelProps> = ({ accessToken }) => {
    ...
    async (id: string) => {
      ...
      try {
        ...
        if (result.roleCode) {
          ...
          try {
            await navigator.clipboard.writeText(result.roleCode);
          } catch {
            /* */
          }
  ...
  const handleReject = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        setRequests((prev) => prev.map((r) => (r.id === id ? { ...r, ...result.request } : r)));
      } catch (e) {
        setError(e instanceof Error ? e.message : 'Ошибка');
      } finally {
        setActionBusy(null);
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      >
        <span style={{ fontSize: 15, fontWeight: 700, color: '#fff' }}>Заявки на роли</span>
        {pendingCount > 0 && (
          <span
            style={{
              padding: '2px 8px',
      ...
      {loading && (
        <div
          style={{ textAlign: 'center', padding: 16, color: 'rgba(255,255,255,0.4)', fontSize: 13 }}
        >
          Загрузка заявок…
        </div>
      )}
          ...
          {requests.map((req) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div>
                    Роль: <span style={{ color: '#f59e0b' }}>{roleLabel}</span>
                  </div>
