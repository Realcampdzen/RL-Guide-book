...
export const ODeConstructorPanel: React.FC = () => {
  ...
  const [cStep, setCStep] = useState(() => {
    try {
      ...
      return s ? Math.max(1, Math.min(3, Number(s))) : 1;
    } catch {
      return 1;
    }
  });
  ...
  const votesFor = (ini: ODeInitiative) =>
    Object.values(ini.votes || {}).filter((v) => v === true).length;
  const votesAgainst = (ini: ODeInitiative) =>
    Object.values(ini.votes || {}).filter((v) => v === false).length;
  ...
  const selectedTemplate = state.templateId
    ? ODE_TEMPLATES.find((t) => t.id === state.templateId)
    : null;
      ...
      {showTip && (
        ...
        >
          ...
          <div style={{ marginTop: 8, textAlign: 'right' }}>
            <button
              type="button"
              onClick={() => setTipIndex((tipIndex + 1) % contextualTips.length)}
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            ...
            <div>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: '#e8f0ff' }}>
                {t.name}
              </h3>
          ...
          {t.steps.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              ...
              {t.steps.map((s, i) => (
                ...
                >
                  <span style={{ fontSize: 11, fontWeight: 700, color: '#e8f0ff' }}>
                    {i + 1}. {s.title}
                  </span>
                  {s.duration && (
          ...
          {t.materials && (
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', marginBottom: 8 }}>
              📦 <strong>Материалы:</strong> {t.materials}
            </div>
          )}
          {t.roles && t.roles.length > 0 && (
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', marginBottom: 8 }}>
              🎭 <strong>Роли:</strong> {t.roles.join(', ')}
            </div>
          )}
          {t.relatedBadge && (
            <div style={{ fontSize: 11, color: '#fbbf24', marginBottom: 8 }}>
              🏅 <strong>Значок:</strong> {t.relatedBadge}
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button type="button" onClick={() => setPreviewTemplate(null)} style={btnSecondary}>
              Закрыть
            </button>
            <button type="button" onClick={() => selectTemplate(t)} style={btnPrimary}>
              Использовать шаблон
            </button>
          </div>
        </div>
    ...
    >
      ...
      <div className="fade-in" style={sectionCard}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
          ...
          <div>
            <h3 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: '#e8f0ff' }}>
              ОДэ Генератор
            </h3>
            <div
              style={{
        ...
        <p style={{ margin: 0, fontSize: 12, color: 'rgba(255,255,255,0.55)', lineHeight: 1.5 }}>
          Собери ОДэ из готовых шаблонов или создай своё с нуля. Каждый шаблон — проверенная
          активность из книги с подробными шагами, материалами и советами.
        </p>
      ...
      {constructorOpen && (
        <div className="fade-in" style={sectionCard}>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
            {['Шаблон', 'Настройки', 'Предпросмотр'].map((label, i) => (
              <button
                ...
                key={label}
                onClick={() => {
                  if (i + 1 < cStep) setCStep((i + 1) as 1 | 2 | 3);
                }}
                style={{
                  flex: 1,
                  padding: '8px 4px',
                  ...
                  fontSize: 11,
                  fontWeight: 700,
                  cursor: i + 1 < cStep ? 'pointer' : 'default',
                  fontFamily: 'inherit',
                  transition: 'all 0.15s',
                }}
              >
          ...
          {cStep === 1 && (
            <div>
              ...
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 8 }}>
                ...
                {(
                  Object.entries(ODE_CATEGORY_META) as [
                    ODeCategory,
                    (typeof ODE_CATEGORY_META)[ODeCategory],
                  ][]
                ).map(([key, meta]) => (
                  <button
                    type="button"
                    ...
                    style={{
                      ...
                      padding: '4px 8px',
                      background:
                        categoryFilter === key ? `${meta.color}22` : 'rgba(255,255,255,0.04)',
                      color: categoryFilter === key ? meta.color : 'rgba(255,255,255,0.4)',
                      borderColor:
                        categoryFilter === key ? `${meta.color}44` : 'rgba(255,255,255,0.06)',
                    }}
                  >
              ...
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 8 }}>
                ...
                {(
                  Object.entries(ODE_AGE_META) as [
                    ODeAgeGroup,
                    (typeof ODE_AGE_META)[ODeAgeGroup],
                  ][]
                ).map(([key, meta]) => (
                  <button
                    type="button"
                    ...
                    style={{
                      ...
                      padding: '4px 8px',
                      background:
                        ageFilter === key ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.04)',
              ...
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 12 }}>
                ...
                {(
                  Object.entries(ODE_SCALE_META) as [ODeScale, (typeof ODE_SCALE_META)[ODeScale]][]
                ).map(([key, meta]) => (
                  <button
                    type="button"
                    key={key}
                    ...
                    style={{
                      ...
                      padding: '4px 8px',
                      background:
                        scaleFilter === key ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.04)',
          ...
          {cStep === 2 && (
            <div>
              ...
              <input
                ...
                placeholder="Например: Минное поле"
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {DURATION_OPTIONS.map((d) => (
                    <button
                      key={d}
                      type="button"
                      onClick={() => setState((p) => ({ ...p, duration: d }))}
                      style={{
                        padding: '6px 14px',
                        borderRadius: 8,
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {AUDIENCE_OPTIONS.map((a) => (
                    <button
                      key={a}
                      type="button"
                      onClick={() => setState((p) => ({ ...p, targetAudience: a }))}
                      style={{
                        padding: '6px 14px',
                        borderRadius: 8,
              ...
              >
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setState((p) => ({ ...p, ageGroup: e.target.value as ODeAgeGroup }))
                    }
                ...
                <div>
                  ...
                  <select
                    value={state.scale}
                    onChange={(e) => setState((p) => ({ ...p, scale: e.target.value as ODeScale }))}
                    style={{ ...fieldStyle, cursor: 'pointer' }}
                  >
                    {(
                      Object.entries(ODE_SCALE_META) as [
                        ODeScale,
                        (typeof ODE_SCALE_META)[ODeScale],
                      ][]
                    ).map(([k, v]) => (
                      <option key={k} value={k}>
                        {v.emoji} {v.label}
              ...
              <input
                ...
                onChange={(e) => setState((p) => ({ ...p, materials: e.target.value }))}
                placeholder="Мелки, ватман, верёвки…"
                style={{ ...fieldStyle, marginBottom: 12 }}
              />
              ...
              >
                ...
                <button type="button" onClick={addStep} style={{ ...btnSecondary, fontSize: 11 }}>
                  + Этап
                </button>
              </div>
              ...
              {state.steps.map((step, idx) => (
                ...
                >
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          fontSize: 10,
                          padding: '2px 6px',
                          opacity: idx === state.steps.length - 1 ? 0.3 : 1,
                        }}
                      >
                        ↓
                      </button>
                  ...
                  <input
                    ...
                    style={{ ...fieldStyle, marginBottom: 6, fontSize: 12 }}
                  />
                  <textarea
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                <button type="button" onClick={() => setCStep(1)} style={btnSecondary}>
                  ← Назад
                </button>
                <button type="button" onClick={validateAndProceed} style={btnPrimary}>
                  Предпросмотр →
                </button>
              </div>
            </div>
          ...
          {cStep === 3 && (
            <div>
              ...
              >
                ...
                <h4 style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 800, color: '#e8f0ff' }}>
                  {state.name}
                </h4>
                <p
                  style={{
                ...
                {state.steps.length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    <div
                      style={{ fontSize: 11, fontWeight: 700, color: '#c4b5fd', marginBottom: 6 }}
                    >
                      ПЛАН ({state.steps.length} этапов)
                    </div>
                    {state.steps.map((s, i) => (
                      <div
                        key={s.id}
                      ...
                      >
                        <div style={{ fontSize: 12, fontWeight: 700, color: '#e8f0ff' }}>
                          {i + 1}. {s.title}
                        </div>
              ...
              {/* Actions */}
              <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                <button type="button" onClick={() => setCStep(2)} style={btnSecondary}>
                  ← Редактировать
                </button>
                <button
                  type="button"
          ...
          <div style={{ marginTop: 12, textAlign: 'center' }}>
            <button
              type="button"
              onClick={() => setConstructorOpen(false)}
      ...
      {showConfirm && (
        ...
        >
          ...
          >
            <div style={{ fontSize: 15, fontWeight: 800, color: '#e8f0ff', marginBottom: 10 }}>
              Подтверждение
            </div>
            <div
              style={{
            ...
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 14 }}>
              ...
              {ODE_SCALE_META[state.scale].label}
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button type="button" onClick={() => setShowConfirm(false)} style={btnSecondary}>
                Отмена
              </button>
              <button
                type="button"
      ...
      <div className="fade-in" style={sectionCard}>
        ...
        {items.filter((i) => i.title.startsWith('[ОДэ]')).length === 0 ? (
          ...
          items
            .filter((i) => i.title.startsWith('[ОДэ]'))
            .map((ini) => (
              <div
                key={ini.id}
                style={{
              ...
              >
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      {ini.description.length > 150
                        ? ini.description.slice(0, 150) + '…'
                        : ini.description}
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
                  <button
                    type="button"
                    onClick={() => vote(ini.id, true)}
                    disabled={myVote(ini) === true}
                    style={{
                      ...btnSecondary,
                      fontSize: 10,
                      opacity: myVote(ini) === true ? 0.4 : 1,
                    }}
                  >
                    👍 За
                  </button>
                  <button
                    type="button"
                    onClick={() => vote(ini.id, false)}
                    disabled={myVote(ini) === false}
                    style={{
                      ...btnSecondary,
                      fontSize: 10,
                      opacity: myVote(ini) === false ? 0.4 : 1,
                    }}
                  >
                    👎 Против
                  </button>
