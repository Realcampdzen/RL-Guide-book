...
const DIARY_GRADIENT =
  'linear-gradient(135deg, rgba(199, 119, 48, 0.12) 0%, rgba(120, 53, 15, 0.18) 100%)';
const EMOJI_OPTIONS = [
  // Настроение
  '😊',
  '😁',
...
export const RealDiaryDashboard: React.FC<RealDiaryDashboardProps> = ({
  variant = 'accordion',
  activeTab = 'diary',
  onTabChange,
  onNavigateToBadge,
  onScrollToInspector: _onScrollToInspector,
}) => {
  const { userData, updateDiaryEntry, updateDiaryShiftTemplates, setDiaryDay, updateDiaryPhotos } =
    useUserProgress();
  ...
  const [localDayMoodEmoji, setLocalDayMoodEmoji] = useState(
    (currentEntry as any).dayMoodEmoji ?? ''
  );
  ...
  const [localContribution, setLocalContribution] = useState(
    (currentEntry as any).contribution ?? ''
  );
  ...
  const [localShift, setLocalShift] = useState<Partial<Record<ShiftScheduleKey, ScheduleCell>>>(
    () => savedShift
  );
  const [localActivities, setLocalActivities] = useState<
    Partial<Record<MyActivityKey, ScheduleCell>>
  >(() => savedActivities);
  ...
  const buildPresentationText = () => {
    ...
    if (text(e.morningText))
      lines.push(
        `Утро: ${text(e.morningText)}${text(e.morningEmoji) ? ` ${text(e.morningEmoji)}` : ''}`
      );
    if (text(e.dayText))
      lines.push(`День: ${text(e.dayText)}${text(e.dayEmoji) ? ` ${text(e.dayEmoji)}` : ''}`);
    if (text(e.eveningText))
      lines.push(
        `Вечер: ${text(e.eveningText)}${text(e.eveningEmoji) ? ` ${text(e.eveningEmoji)}` : ''}`
      );
    if (text(e.memorableText))
      lines.push(
        `Чем запомнился: ${text(e.memorableText)}${text(e.memorableEmoji) ? ` ${text(e.memorableEmoji)}` : ''}`
      );
    ...
    const shiftLines = SHIFT_ITEMS.map(({ key, label }) => ({
      label,
      time: text(savedShift[key]?.time),
      note: text(savedShift[key]?.note),
    })).filter((x) => x.time || x.note);
    const activityLines = ACTIVITY_ITEMS.map(({ key, label }) => ({
      label,
      time: text(savedActivities[key]?.time),
      note: text(savedActivities[key]?.note),
    })).filter((x) => x.time || x.note);
    if (shiftLines.length) lines.push('', 'Распорядок смены:');
    shiftLines.forEach((x) =>
      lines.push(`${x.label}: ${x.time && x.note ? `${x.time} - ${x.note}` : x.time || x.note}`)
    );
    if (activityLines.length) lines.push('', 'Мои занятия (кружки/тренировки):');
    activityLines.forEach((x) =>
      lines.push(`${x.label}: ${x.time && x.note ? `${x.time} - ${x.note}` : x.time || x.note}`)
    );
  ...
  const generateDiaryCardImage = async () => {
    ...
    try {
      ...
      const cardConclusionsText = useCustomCardText
        ? customConclusions.trim()
        : text(e.conclusions);
      ...
      ctx.fillText(
        '\u0420\u0435\u0430\u043b\u044c\u043d\u044b\u0439 \u041b\u0430\u0433\u0435\u0440\u044c \u2022 \u0440\u0435\u0430\u043b\u044c\u043d\u044b\u0439\u043b\u0430\u0433\u0435\u0440\u044c.\u0440\u0444',
        W / 2,
        H - 50
      );
      ctx.textAlign = 'start';
  ...
  const onTelegram = async () => {
    ...
    window.open(
      `https://t.me/Stivanovv?text=${encodeURIComponent(caption)}`,
      '_blank',
      'noopener,noreferrer'
    );
  ...
  const saveSchedulesToDevice = async () => {
    ...
    try {
      ...
      const cardBodyHeight = (rows: Array<{ rowHeight: number }>) =>
        rows.reduce((sum, row) => sum + row.rowHeight + rowGap, 0) - rowGap;
      const shiftCardHeight =
        cardHeaderHeight + cardBodyHeight(preparedShiftRows) + cardBottomPadding;
      const activityCardHeight =
        cardHeaderHeight + cardBodyHeight(preparedActivityRows) + cardBottomPadding;
      ...
      const drawCard = (
        ...
        roundedRect(
          ...
          y,
          cardWidth,
          cardHeaderHeight + cardBodyHeight(rows) + cardBottomPadding,
          20
        );
      ...
      drawCard(
        ...
        cardsY,
        'Мои занятия (кружки/тренировки)',
        preparedActivityRows
      );
  ...
  const entriesCountForPreview = Object.values(entries).filter(
    (e) =>
      e.mainMoments?.trim() ||
      e.friends?.trim() ||
      e.conclusions?.trim() ||
      ...
      e.dayText?.trim() ||
      e.eveningText?.trim() ||
      e.memorableText?.trim()
  ).length;
  const mvpFilledCount = [localMain, localFriends, localConclusions].filter((value) =>
    text(value)
  ).length;
  ...
  const canExportSchedules =
    !editingShift &&
    !editingActivities &&
    hasValues(localShift, SHIFT_ITEMS) &&
    hasValues(localActivities, ACTIVITY_ITEMS);
      ...
      >
        ...
        {editing ? (
          ...
          <button
            ...
            onClick={onEdit}
            style={{ padding: '8px 16px', fontSize: 13, minWidth: 'unset' }}
          >
            Изменить
          </button>
        )}
            ...
            return (
              ...
              >
                ...
                {editing ? (
                  ...
                  <>
                    <div style={{ color: 'var(--cabin-neon-cyan)', fontWeight: 600, fontSize: 14 }}>
                      {text(values[key]?.time) || '—'}
                    </div>
                    <div style={{ color: '#fff', fontSize: 14, lineHeight: 1.4 }}>
                      {text(values[key]?.note) || <span style={{ opacity: 0.3 }}>—</span>}
                    </div>
                  </>
                )}
              </div>
    ...
    <div className={sectionClass} style={variant === 'accordion' ? sectionWrapStyle : {}}>
      <div style={{ display: 'grid', gap: 16 }}>
        {scheduleCard(
          ...
          saveShift,
          (k, f, v) => patchCell(setLocalShift, k, f, v)
        )}
        {scheduleCard(
          'Мои занятия (кружки/тренировки)',
          ACTIVITY_ITEMS,
          ...
          saveActivities,
          (k, f, v) => patchCell(setLocalActivities, k, f, v)
        )}
      </div>
      {canExportSchedules ? (
        <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
          <button
            type="button"
            className="cab-btn-glass"
            onClick={saveSchedulesToDevice}
            style={{ minWidth: 'unset', padding: '10px 20px' }}
          >
            Сохранить на устройство
          </button>
          {scheduleSavedToast ? (
            <span style={{ fontSize: 13, color: 'var(--cabin-neon-cyan)' }}>
              Картинка сохранена
            </span>
          ) : null}
        </div>
  ...
  const diaryCard = (
    <div className={sectionClass} style={variant === 'accordion' ? sectionWrapStyle : {}}>
      {variant === 'accordion' ? (
        <button
          type="button"
          onClick={() => setPresentationExpanded((v) => !v)}
          style={{
            width: '100%',
            background: 'none',
            border: 'none',
        ...
        >
          ...
          >
            ...
            <ImageSourceBlock
              ...
              buttonLayout="bento"
              labels={{
                placeholder: 'Сэлфи или фото дня',
                upload: 'Загрузить фото',
                uploadReplace: 'Изменить фото',
                generate: 'Сгенерировать с ИИ',
                process: 'Обработать с ИИ',
                generateModalTitle: 'Сгенерировать фото для карточки',
                generateModalDescription: 'ИИ создаст изображение для твоей карточки дневника.',
              }}
              onGenerate={async (o) =>
                requestImageGenerate(
                  { mode: 'generate', context: 'diary_card', prompt: o.prompt ?? '' },
                  accessToken ?? null
                )
              }
              onProcess={async (imageBase64, o) =>
                requestImageGenerate(
                  { mode: 'process', context: 'diary_card', imageBase64, prompt: o?.prompt ?? '' },
                  accessToken ?? null
                )
              }
            />
          ...
          >
            <button
              type="button"
              onClick={() => {
                generateDiaryCardImage();
                setPresentationText(buildPresentationText());
              }}
                ...
                {telegramToast && (
                  <span
                    style={{
                      ...
                      textAlign: 'center',
                      fontSize: 12,
                      color: 'var(--cabin-neon-purple)',
                      marginTop: 8,
                    }}
                  >
                    Картинка скачана — прикрепи в чат
                  </span>
                )}
              </div>
            </div>
  ...
  const daySwitcher = (
    <div className={sectionClass} style={variant === 'accordion' ? sectionWrapStyle : {}}>
      <div
        className="real-diary-day-switcher"
        style={{
          display: 'flex',
          gap: 6,
          flexWrap: 'wrap',
      ...
      >
        {dayKeys.map((d) => {
          ...
          const hasContent =
            dayEntry.morningText ||
            dayEntry.dayText ||
            dayEntry.eveningText ||
            dayEntry.memorableText;
          ...
          return (
            ...
            >
              {moodEmoji ? (
                <span style={{ fontSize: 16, lineHeight: 1 }}>{moodEmoji}</span>
              ) : hasContent ? (
                <span style={{ fontSize: 10, opacity: 0.6, color: '#fde68a' }}>●</span>
              ) : null}
              <span>День {d}</span>
            </button>
          );
      ...
      <div
        style={{
          height: 2,
          background: `linear-gradient(to right, ${DIARY_ACCENT_LIGHT}, transparent 80%)`,
          marginBottom: 12,
          borderRadius: 2,
          marginTop: -14,
        }}
  ...
  const floatingToast =
    diarySavedToast !== null
      ? createPortal(
          <div
            key="diary-save-toast"
            style={{
              position: 'fixed',
              top: '50%',
              left: '50%',
              ...
              pointerEvents: 'none',
              opacity: 1,
              animation: 'diary-toast-fade 4s ease-out forwards',
            }}
          >
            ...
            <div
              style={{
                ...
                boxShadow: '0 16px 48px rgba(0, 0, 0, 0.6), 0 0 24px rgba(93, 228, 255, 0.15)',
                display: 'flex',
                alignItems: 'center',
                gap: 12,
              }}
  ...
  const diaryTab = (
    <div className={sectionClass} style={variant === 'accordion' ? sectionWrapStyle : {}}>
      <article className="cab-card fade-in" style={{ padding: 24 }}>
        <div style={{ display: 'grid', gap: 16 }}>
          <DiaryRow
            label="Утро"
            value={localMorningText}
            emoji={localMorningEmoji}
            onText={setLocalMorningText}
            onEmoji={setLocalMorningEmoji}
          />
          <DiaryRow
            label="День"
            value={localDayText}
            emoji={localDayEmoji}
            onText={setLocalDayText}
            onEmoji={setLocalDayEmoji}
          />
          <DiaryRow
            label="Вечер"
            value={localEveningText}
            emoji={localEveningEmoji}
            onText={setLocalEveningText}
            onEmoji={setLocalEveningEmoji}
          />
          <DiaryRow
            label="Мем дня"
            value={localMemeText}
            emoji={localMemeEmoji}
            onText={setLocalMemeText}
            onEmoji={setLocalMemeEmoji}
          />
          <DiaryRow
            label="Чем запомнился день"
            value={localMemorableText}
        ...
        <div style={{ display: 'grid', gap: 20 }}>
          <div style={{ display: 'grid', gap: 8 }}>
            <label style={{ fontSize: 13, fontWeight: 500, color: 'rgba(255, 255, 255, 0.9)' }}>
              Что хорошего я сделал(а) сегодня?
            </label>
            <textarea
              className="cab-input"
              placeholder="Любое доброе дело, помощь, поступок…"
              rows={3}
              style={{ resize: 'vertical' }}
              value={localMain}
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            ...
            <textarea
              className="cab-input"
              placeholder="Чем ты можешь гордиться — пусть даже мелочью…"
              rows={3}
              style={{ resize: 'vertical' }}
              value={localFriends}
              onChange={(e) => setLocalFriends(e.target.value)}
            />
          </div>
          <div style={{ display: 'grid', gap: 8 }}>
            <label style={{ fontSize: 13, fontWeight: 500, color: 'rgba(255, 255, 255, 0.9)' }}>
            ...
            <textarea
              className="cab-input"
              placeholder="Момент, достижение или решение, которым гордишься…"
              rows={3}
              style={{ resize: 'vertical' }}
              value={localConclusions}
              onChange={(e) => setLocalConclusions(e.target.value)}
            />
          </div>
          <div style={{ display: 'grid', gap: 8 }}>
            <label style={{ fontSize: 13, fontWeight: 500, color: 'rgba(255, 255, 255, 0.9)' }}>
              Мой вклад в наш отряд сегодня
            </label>
            <textarea
              className="cab-input"
              placeholder="Что я сделал(а) для ребят, для нашей команды…"
              rows={3}
              style={{ resize: 'vertical' }}
              value={localContribution}
              onChange={(e) => setLocalContribution(e.target.value)}
            />
          </div>
        </div>
        {variant === 'cabin' ? (
  ...
  const summary = isShiftScheduleTab ? null : (
    <div style={{ marginBottom: variant === 'accordion' && isExpanded ? 20 : 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div
          onClick={variant === 'accordion' ? () => setIsExpanded((v) => !v) : undefined}
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
            ...
            <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', fontWeight: 500 }}>
              {weekday} ·{' '}
              {entriesCountForPreview > 0
                ? `${entriesCountForPreview} ${entriesCountForPreview === 1 ? 'запись' : 'записей'}`
                : 'пусто'}
            </span>
          </div>
        </div>
          ...
          <div style={{ position: 'relative', flexShrink: 0, marginLeft: 12 }}>
            <button
              ...
              style={{
                ...
                borderRadius: 16,
                fontSize: 28,
                border: localDayMoodEmoji
                  ? `2px solid ${DIARY_ACCENT}`
              ...
              >
                ...
                {localDayMoodEmoji && (
                  <button
                    type="button"
                    onClick={() => {
                      setLocalDayMoodEmoji('');
                      setDayMoodPopover(false);
                    }}
                    style={{
                      width: '100%',
                      padding: '4px 0',
                ...
                {EMOJI_OPTIONS.map((em) => (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 22,
                      border:
                        localDayMoodEmoji === em
                          ? `2px solid ${DIARY_ACCENT}`
                          : '1px solid rgba(255,255,255,0.1)',
                      background:
                        localDayMoodEmoji === em ? DIARY_ACCENT_LIGHT : 'rgba(255,255,255,0.04)',
                      cursor: 'pointer',
                    }}
                  >
                    {em}
  ...
  if (variant === 'accordion') {
    return (
      <div
        className="real-diary-dashboard"
        style={{
          background: DIARY_GRADIENT,
          borderRadius: 24,
          padding: 20,
          border: `1px solid ${mvpFilledCount === 3 ? DIARY_ACCENT : DIARY_ACCENT_LIGHT}`,
          marginBottom: 24,
          position: 'relative',
          overflow: 'hidden',
        }}
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {onNavigateToBadge ? (
                <button
                  type="button"
                  className="cab-btn-glass"
                  onClick={() => onNavigateToBadge('2.6')}
                  style={{ padding: '12px 24px', minWidth: 'unset' }}
                >
                  Требования значка 2.6
                </button>
              ) : null}
            </div>
          </div>
  ...
  const DIARY_PHOTO_SLOTS = [
    {
      key: 'topMoment',
      label: 'Топ момент',
      description: 'Лучший момент смены — то, что запомнилось больше всего',
    },
  ...
  return (
    <div
      className="real-diary-cabin-content"
      style={{ display: 'grid', gap: 16, paddingBottom: 120 }}
    >
      {summary}
      {isShiftScheduleTab ? null : daySwitcher}
      {wrappedTab}
      {floatingToast}
    </div>
  );
