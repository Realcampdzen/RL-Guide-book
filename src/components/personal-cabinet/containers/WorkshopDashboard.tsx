...
export const WorkshopDashboard: React.FC<WorkshopDashboardProps> = ({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetchMyProposals(effectiveToken)
      .then((rows) => {
        if (!cancelled) setCabinetProposals(rows);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
        ...
        >
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: 13, color: 'rgba(255,255,255,0.6)', lineHeight: 1.5 }}>
              Предложи свой значок, категорию или версию. Всё пройдёт проверку вожатым. Ваши лучшие
              идеи попадут в путеводитель!
            </p>
          </div>
          ...
          <div>
            ...
            <p style={{ margin: '8px 0 0 4px', fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>
              {wsProposalType === 'badge'
                ? 'Предложи оригинальный значок в любую категорию'
                : wsProposalType === 'category'
                  ? 'Предложи новую масштабную категорию для значков'
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <>
                <div style={{ display: 'flex', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      value={wsBadgeId}
                      onChange={(e) => setWsBadgeId(e.target.value)}
              ...
              >
                ...
                {wsImage && (
                  <div style={{ display: 'flex', justifyContent: 'center', marginTop: 12 }}>
                    <button
                      type="button"
                      className="cab-btn cab-btn--danger"
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-start', marginTop: 8 }}>
              <button
                onClick={handleWsSubmit}
                disabled={
                  !wsTitle.trim() || (wsProposalType === 'version' && !wsBadgeId.trim()) || wsBusy
                }
