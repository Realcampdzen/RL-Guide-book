      ...
      {broTab === 'brodela' ? (
        ...
        <SquadChat
          ...
          members={[
            {
              deviceId: deviceId || '',
              nickname: userData?.profile?.nickname || null,
        ...
        broPassportComplete ? (
          ...
          >
            ...
            >
              Создание своего Крыла откроется после завершения Посвящения. Пройди Бросвящение, чтобы
              получить доступ к категории БРО и сформировать Крыло.
            </div>
            <button
              type="button"
              onClick={() => setBroTab('initiation')}
              style={{
                marginTop: 20,
                padding: '10px 24px',
                borderRadius: 12,
        ...
        <BroPassportPanel
          squadId={
            mySquadInfo?.membership?.squadId || userData?.diaryProgress?.squad?.name || 'dev-squad'
          }
