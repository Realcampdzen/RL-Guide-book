...
export const ProfileSettingsContainer: React.FC<ProfileSettingsContainerProps> = ({ onClose }) => {
  ...
  const currentLevels =
    profile?.stats?.totalLevelsAchieved ||
    Object.values(userData?.progress || {}).filter((p: any) => p.status === 'achieved').length;
  const rankThresholds = [0, 5, 15, 30, 50, 75, 100];
  const currentRankIdx = rankThresholds.findIndex(
    (_, i) => (rankThresholds[i + 1] ?? Infinity) > currentLevels
  );
  const nextRankAt =
    rankThresholds[currentRankIdx + 1] ?? rankThresholds[rankThresholds.length - 1];
  const prevRankAt = rankThresholds[currentRankIdx] ?? 0;
  const xpPercent =
    nextRankAt > prevRankAt
      ? Math.min(100, ((currentLevels - prevRankAt) / (nextRankAt - prevRankAt)) * 100)
      : 100;
  const rankNames = [
    'Новичок',
    'Исследователь',
      ...
      >
        ...
        >
          {isImageAvatar(avatarInput) ? (
            ...
            <span style={{ fontSize: 64, opacity: 0.8 }}>
              {avatarInput || nicknameInput.charAt(0).toUpperCase()}
            </span>
          )}
        </div>
        <input
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
        ...
        <button
          ...
          onMouseEnter={(e) => {
            ...
            e.currentTarget.style.boxShadow = '0 6px 24px rgba(139,0,255,0.45)';
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.transform = 'none';
            e.currentTarget.style.boxShadow = '0 4px 16px rgba(139,0,255,0.3)';
          }}
        >
          Сохранить
        </button>
