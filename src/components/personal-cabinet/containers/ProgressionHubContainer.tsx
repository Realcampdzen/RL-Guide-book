...
interface ProgressionHubContainerProps {
    userData: any;
    badgeLookupMap: Map<string, { title: string; emoji: string; category_id: string; level?: string }>;
            ...
            <div className="path-card path-card--vertical">
                ...
                <div className="path-card__actions">
                 ...
                 <div className="path-card__footer">
                     {isPath ? (
                         <button type="button" onClick={(e) => { e.stopPropagation(); if (window.confirm('Удалить?')) removeRoute(baseId); }} className="btn-action-round trash" aria-label="Удалить из пути"><CabIcons.Trash /></button>
                     ) : (
                    ...
                    {(() => {
                        const achievedItems = Object.entries(progress)
                            ...
                            .map(([levelId, p]: [string, any]) => {
                                ...
                                return { baseId, levelId, categoryId: parts[0] || '1', title: meta?.title || baseId, emoji: meta?.emoji || '', achievedAt: p?.achievedAt || '' };
                            });
