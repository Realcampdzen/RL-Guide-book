...
import { type BroEvent, type BroPassport, fetchBroEvents, fetchMyPassport } from '../utils/broApi';
...
export const BroPassportPanel: React.FC<BroPassportPanelProps> = ({
  ...
  const getTaskSubmission = useCallback(
    (taskId: string): BroSubmission | null => {
    ...
    async (taskId: string) => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/bro/passport/${encodeURIComponent(passport.id)}/task/${encodeURIComponent(taskId)}/submit`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', ...devHeaders },
  ...
  const handleCreateWing = useCallback(async () => {
    ...
    try {
      ...
      console.error('Wing creation failed:', e);
      setToast('Ошибка при создании крыла');
      setTimeout(() => setToast(null), 3500);
    } finally {
      setBusy(false);
    }
    ...
    >
      ...
      {canModerate && !activeEvent && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button
            ...
            disabled={busy}
            onClick={() => (showTaskEditor ? void handleInitiate() : setShowTaskEditor(true))}
          >
            {busy
              ? 'Загрузка…'
              : showTaskEditor
                ? 'Объявить Бросвящение'
                : 'Настроить и объявить Бросвящение'}
          </button>
          ...
          {showTaskEditor && (
            ...
            >
              ...
              {editTasks.map((task, idx) => (
                ...
                >
                  <div
                    style={{ display: 'flex', alignItems: 'center', gap: 12, paddingBottom: 10 }}
                  >
                    <span
                      style={{
                        width: 28,
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                <button
                  type="button"
                  onClick={() => setEditTasks(DEFAULT_BRO_TASKS.map((t) => ({ ...t })))}
                  style={{
                    padding: '6px 12px',
                    borderRadius: 8,
                    border: '1px solid rgba(255,255,255,0.1)',
      ...
      {activeEvent && !passport && (
        <div className="fade-in cab-card" style={{ padding: '28px 32px', borderRadius: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            ...
            />
            <span style={{ fontSize: 14, fontWeight: 700, color: '#e0d4ff' }}>
              Бросвящение активно
            </span>
          </div>
          <div
      ...
      {passport && (
        <div className="fade-in cab-card" style={{ padding: '28px 32px', borderRadius: 20 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            >
              <span>
                {tasksDone} из {tasksTotal} заданий
              </span>
              <span style={{ fontWeight: 700, color: '#e0e7ff', fontSize: 14 }}>
                {progressPct}%
              </span>
            </div>
            <div
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                      ...
                      <div
                        style={{
                          ...
                          border:
                            task.done || isPending ? 'none' : '1.5px solid rgba(255,255,255,0.15)',
                        }}
                      >
                      ...
                      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center' }}>
                        ...
                        {canSubmit && hasAuth && (
                          <button
                            ...
                            disabled={busy}
                            onClick={() => {
                              setSubmitTaskId(submitTaskId === task.id ? null : task.id);
                              setSubmitText('');
                              setSubmitPhoto(null);
                            }}
                          >
      ...
      {canModerate &&
        (() => {
          ...
          return (
            <div
              className="fade-in cab-card"
              style={{ padding: '28px 32px', borderRadius: 20, borderColor: 'rgba(234,179,8,0.2)' }}
            >
      ...
      {showComplete && (
        ...
        >
          <div style={{ fontSize: 22, fontWeight: 700, color: '#4ade80', marginBottom: 4 }}>
            Паспорт БРО завершён
          </div>
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', marginTop: 4 }}>
            Теперь вы можете создать своё Крыло.
          </div>
          <button
            type="button"
            style={{
      ...
      {showWingModal && (
        ...
        >
          ...
          >
            <h4 style={{ margin: '0 0 16px', fontSize: 18, fontWeight: 700, color: '#e0d4ff' }}>
              Создать Крыло
            </h4>
            <input
              type="text"
              placeholder="Название Крыла"
              value={wingName}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                disabled={busy || !wingName.trim()}
                onClick={() => void handleCreateWing()}
              >
  ...
  const handleReview = async (action: 'approve' | 'reject') => {
    ...
    try {
      const res = await fetch(`/api/bro/submissions/${encodeURIComponent(submission.id)}/review`, {
        ...
        body: JSON.stringify({
          action,
          comment: action === 'reject' ? rejectComment.trim() || null : null,
        }),
      });
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
        ...
        {submission.userRole && submission.userRole !== 'traveler' && (
          ...
          >
            {submission.userRole === 'counselor'
              ? 'Вожатый'
              : submission.userRole === 'educator'
                ? 'Педагог'
                : submission.userRole === 'shift_leader'
                  ? 'Ст. вожатый'
                  : submission.userRole === 'camp_director'
                    ? 'Нач. лагеря'
                    : submission.userRole === 'developer'
                      ? 'Dev'
                      : submission.userRole === 'participant'
                        ? 'Участник'
                        : submission.userRole}
          </span>
        )}
