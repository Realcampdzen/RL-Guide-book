...
import { type ArtItem, fetchArtsInbox, reviewArt } from '../utils/badgeArtApi';
...
export const ArtInboxTab: React.FC<ArtInboxTabProps> = ({ accessToken }) => {
  ...
  const handleReview = useCallback(
    async (id: string, status: 'approved' | 'rejected' | 'canon', note?: string) => {
      if (busyId) return;
      setBusyId(id);
      try {
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <div style={{ fontSize: 10, opacity: 0.6 }}>
              {art.submittedByNickname || 'Аноним'} · {art.source || '—'} ·{' '}
