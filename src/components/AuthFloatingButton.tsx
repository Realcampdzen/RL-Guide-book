...
export const AuthFloatingButton: React.FC = () => {
  ...
  const setPendingDevOAuth = (v: boolean) => {
    try {
      if (v) localStorage.setItem(LS_PENDING_DEV, '1');
      else localStorage.removeItem(LS_PENDING_DEV);
    } catch {
      /* */
    }
  };
  const isPendingDevOAuth = (): boolean => {
    try {
      return localStorage.getItem(LS_PENDING_DEV) === '1';
    } catch {
      return false;
    }
  };
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session: s } }) => {
      console.log(
        ...
        s ? `email=${s.user?.email}, has_token=${!!s.access_token}` : 'null'
      );
      ...
      if (s?.access_token) {
        if (isPendingDevOAuth()) {
          ...
          if (hasPending || isUnauthenticated) {
            ...
            console.log(
              '[AUTH] getSession: already authenticated as',
              currentAuth.role,
              '- skipping resolve'
            );
          }
      ...
      if (s?.access_token) {
        if (isPendingDevOAuth()) {
          ...
          if (hasPending || isUnauthenticated) {
            ...
            console.log(
              '[AUTH] onAuthStateChange SIGNED_IN: already have role',
              currentAuth.role,
              '- skipping'
            );
          }
  ...
  const resolveOAuthSession = useCallback(
    async (s: Session) => {
      ...
      if (currentAuth.role && currentAuth.role !== 'traveler' && currentAuth.accessToken) {
        console.log(
          '[AUTH] resolveOAuthSession: already authenticated as',
          currentAuth.role,
          '- skipping'
        );
    ...
    try {
      ...
      if (res.ok && data.role === 'developer') {
        ...
        auth.setAuth({
          ...
          accessToken: (data.accessToken as string) || token,
        });
        setActiveModal('none');
        setOauthError(null);
      } else {
    ...
    (result: RoleFlowResult) => {
      switch (result.type) {
        ...
        case 'request-sent':
          setActiveModal('none');
          setPendingToast(
            `Заявка на роль «${ROLE_DISPLAY[result.role]?.label || result.role}» отправлена. Ожидайте одобрения.`
          );
          setTimeout(() => setPendingToast(null), 6000);
          break;
    ...
    <>
      ...
      >
        {isLoggedIn ? (
          ...
          <div style={{ position: 'relative' }}>
            ...
            {showMenu && (
              ...
              >
                ...
                >
                  <span
                    style={{ width: 6, height: 6, borderRadius: 3, background: displayInfo.color }}
                  />
                  {displayInfo.label}
                </button>
                <button
          ...
          >
            <span style={{ fontSize: 13, fontWeight: 600, letterSpacing: '-0.01em' }}>Войти</span>
          </button>
        )}
      </div>
