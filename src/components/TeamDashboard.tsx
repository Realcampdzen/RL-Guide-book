...
import type { TeamPlanGridData, EngineProject } from '../types/teams';
...
export type TeamTabId = 'my-engines' | 'engine' | 'engine-project' | 'engine-plan' | 'engine-path' | 'engine-chat' | 'camp-control' | 'engine-create' | 'ode';
...
export const TeamDashboard: React.FC<TeamDashboardProps> = ({
  ...
  const { myTeam, myTeams, setActiveTeam, isLoading, loadError, syncTeam, createTeam, updateTeam, joinTeam, leaveTeam, deleteTeam, generateInviteUrl } = useTeam();
  ...
  const emojiPickerEmojis = useMemo(() => EMOJI_CATEGORIES.find(c => c.id === emojiPickerCat)?.emojis || [], [emojiPickerCat]);

  // Engine projects
  const [engineProjects, setEngineProjects] = useState<EngineProject[]>([]);
  const [projectsLoading, setProjectsLoading] = useState(false);
  const [projectForm, setProjectForm] = useState({ title: '', description: '', plan: '', targetBadgeId: '', reflection: '', scenario: '' });
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input className="w-input" placeholder="Эмодзи/лого" value={isImageUrl(teamForm.logo) ? 'Фото загружено' : teamForm.logo} onChange={(e) => setTeamForm({ ...teamForm, logo: e.target.value })} style={{ flex: 1 }} readOnly={isImageUrl(teamForm.logo)} />
              <button type="button" onClick={() => setEmojiPickerOpen(v => !v)} title="Выбрать эмодзи" style={{
                width: 40, height: 40, borderRadius: 10, border: emojiPickerOpen ? '1px solid rgba(139,0,255,0.5)' : '1px solid rgba(255,255,255,0.15)',
                background: emojiPickerOpen ? 'rgba(139,0,255,0.15)' : 'rgba(255,255,255,0.06)',
                cursor: 'pointer', fontSize: 20, display: 'grid', placeItems: 'center', flexShrink: 0,
                transition: 'all 0.15s',
              }}>😊</button>
              <button type="button" onClick={() => createLogoInputRef.current?.click()} title="Загрузить фото" style={{
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    ...
                    reader.onload = () => setTeamForm(f => ({ ...f, logo: reader.result as string }));
              ...
              {teamForm.logo && (
                isImageUrl(teamForm.logo)
                  ? <img src={teamForm.logo} alt="" style={{ width: 42, height: 42, borderRadius: 10, objectFit: 'cover', border: '1px solid rgba(139,0,255,0.3)', flexShrink: 0 }} />
                  : <span style={{ fontSize: 28, lineHeight: 1 }}>{teamForm.logo}</span>
              )}
            </div>
            {emojiPickerOpen && (
              <>
                <div onClick={() => setEmojiPickerOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 100 }} />
                <div style={{
                  position: 'absolute', top: '100%', right: 0, zIndex: 101, marginTop: 6, width: 320,
                  background: 'rgba(10, 8, 30, 0.92)', border: '1px solid rgba(255,255,255,0.12)',
                  borderRadius: 14, padding: 10, backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
                    ...
                    {EMOJI_CATEGORIES.map(cat => (
                      <button key={cat.id} type="button" title={cat.label}
                        ...
                        style={{
                          ...
                          background: emojiPickerCat === cat.id ? 'rgba(139,0,255,0.2)' : 'transparent',
                          cursor: 'pointer', fontSize: 16, lineHeight: 1, flexShrink: 0, transition: 'background 0.12s',
                        }}
                      >{cat.icon}</button>
                    ))}
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: 2, maxHeight: 200, overflowY: 'auto', padding: 2 }}>
                    {emojiPickerEmojis.map((e, i) => (
                      <button key={i} type="button"
                        onClick={() => { setTeamForm(f => ({ ...f, logo: e })); setEmojiPickerOpen(false); }}
                        style={{ padding: 4, border: 'none', borderRadius: 6, background: 'transparent', cursor: 'pointer', fontSize: 20, lineHeight: 1, textAlign: 'center', transition: 'background 0.1s' }}
  ...
  const renderActiveEngine = (opts: { withInitiative: boolean; withPath: boolean; showCollapse: boolean }) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 720, margin: '0 auto', width: '100%', paddingBottom: 80 }}>
        {/* Header card */}
        <div className="fade-in" style={glassCard}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8 }}>
            <div style={{ width: 64, height: 64, borderRadius: 14, border: `1px solid ${TEAM_ACCENT}`, background: 'rgba(139,0,255,0.18)', overflow: 'hidden', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ fontSize: 13, fontStyle: 'italic', opacity: 0.76, marginTop: 2 }}>«{myTeam.motto}»</div>
        ...
        <div className="fade-in" style={glassCard}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            {!engineEditing ? (
              <button type="button" className="btn-secondary" style={{ padding: '5px 10px', fontSize: 11 }} onClick={startEditing}>✏️ Редактировать</button>
            ) : (
              <div style={{ display: 'flex', gap: 6 }}>
                <button type="button" className="btn-primary-gold" style={{ padding: '5px 12px', fontSize: 11 }} onClick={saveEditing}>Сохранить</button>
                <button type="button" className="btn-secondary" style={{ padding: '5px 10px', fontSize: 11 }} onClick={() => setEngineEditing(false)}>Отмена</button>
              </div>
            )}
          </div>
          {engineEditing ? (
            <div style={{ display: 'grid', gap: 10 }}>
              <div>
                <div style={{ fontSize: 11, opacity: 0.5, fontWeight: 500, marginBottom: 4 }}>Название</div>
                <input className="w-input" value={editName} onChange={(e) => setEditName(e.target.value)} placeholder="Название Движка" />
              </div>
              <div>
                <div style={{ fontSize: 11, opacity: 0.5, fontWeight: 500, marginBottom: 4 }}>Девиз</div>
                <input className="w-input" value={editMotto} onChange={(e) => setEditMotto(e.target.value)} placeholder="Девиз Движка" />
              </div>
              <div>
                <div style={{ fontSize: 11, opacity: 0.5, fontWeight: 500, marginBottom: 4 }}>Эмодзи / Лого</div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <input className="w-input" style={{ flex: 1 }} value={editLogo} onChange={(e) => setEditLogo(e.target.value)} placeholder="🚀" />
                  <button type="button" className="btn-secondary" style={{ padding: '6px 10px', fontSize: 11 }} onClick={() => logoInputRef.current?.click()}>Загрузить фото</button>
                </div>
              </div>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '6px 14px', fontSize: 13, lineHeight: 1.6 }}>
              ...
              <span>{myTeam.scope === 'squad' ? 'Отряд' : myTeam.scope === 'shift' ? 'Смена' : 'Лагерь'}</span>
              ...
              <span>{myTeam.createdAt ? new Date(myTeam.createdAt).toLocaleDateString('ru-RU') : '—'}</span>
            </div>
          )}
        </div>
        ...
        {/* Crew card */}
        <div className="fade-in" style={glassCard}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>Экипаж <span style={{ opacity: 0.5, fontWeight: 400, fontSize: 12 }}>({myTeam.members.length})</span></div>
        ...
        <div className="fade-in" style={glassCard}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <button type="button" className="btn-secondary" style={{ padding: '7px 14px', fontSize: 12 }} onClick={() => { void navigator.clipboard.writeText(generateInviteUrl()); showHint({ title: 'Ссылка скопирована', content: 'Приглашение скопировано.' }); }}>
              📋 Копировать ссылку
            </button>
            <button type="button" className="btn-secondary" style={{ padding: '7px 14px', fontSize: 12 }} onClick={() => { void navigator.clipboard.writeText(myTeam.id); showHint({ title: 'Код скопирован', content: `Код Движка: ${myTeam.id}` }); }}>
              🔑 Копировать код
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, padding: '4px 0' }}>
          {opts.showCollapse ? <button type="button" className="btn-secondary" style={{ padding: '8px 16px', fontSize: 12 }} onClick={toggleCollapsed}>Свернуть</button> : null}
          <button type="button" className="btn-secondary" style={{ padding: '8px 16px', fontSize: 12 }} onClick={() => void syncTeam()}>🔄 Обновить</button>
          <button type="button" className="btn-secondary" style={{ padding: '8px 16px', fontSize: 12, opacity: 0.7 }} onClick={() => void leaveTeam()}>Выйти из Движка</button>
          {isLeader ? <button type="button" className="btn-secondary" style={{ padding: '8px 16px', fontSize: 12, opacity: 0.7, color: '#fca5a5' }} onClick={() => { if (window.confirm('Удалить Движок? Это действие нельзя отменить.')) void deleteTeam(); }}>Удалить</button> : null}
        </div>

        {opts.withInitiative && onSuggestInitiative ? (
          <div><button type="button" className="btn-secondary" onClick={onSuggestInitiative}>Предложить инициативу в совет лагеря</button></div>
        ) : null}
        {opts.withPath ? (
          <div>
        ...
        <div className="fade-in" style={glassCard}>
          ...
          {myTeams.length === 0 ? (
            <div style={{ fontSize: 13, opacity: 0.65, padding: '20px 0', textAlign: 'center' }}>
              У тебя пока нет Движков.<br/>Создай свой первый Движок или вступи по коду!
            </div>
          ) : (
              ...
              {myTeams.map(t => {
                ...
                return (
                  <div key={t.id} style={{
                    ...
                    border: isActive ? `1px solid ${TEAM_ACCENT}` : '1px solid rgba(255,255,255,0.06)',
                    transition: 'all 0.2s',
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 42, height: 42, borderRadius: 10, background: 'rgba(139,0,255,0.15)', border: `1px solid ${TEAM_ACCENT}`, display: 'grid', placeItems: 'center', flexShrink: 0 }}>
                        {t.logo && isImageUrl(t.logo)
                          ? <img src={t.logo} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 10 }} />
                          : <span style={{ fontSize: 20 }}>{t.logo || '🚀'}</span>
                        }
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: 14 }}>{t.name}</div>
                        {t.motto && <div style={{ fontSize: 11, opacity: 0.6, marginTop: 2 }}>«{t.motto}»</div>}
                        <div style={{ fontSize: 10, opacity: 0.45, marginTop: 3 }}>{t.members.length} чел. · Код: {t.id}</div>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'flex-end' }}>
                        {isActive ? (
                          <span style={{ fontSize: 10, fontWeight: 700, color: '#4ecdc4', padding: '3px 8px', borderRadius: 6, background: 'rgba(78,205,196,0.15)', border: '1px solid rgba(78,205,196,0.3)' }}>Активен</span>
                        ) : (
                          <button type="button" className="btn-secondary" style={{ padding: '3px 10px', fontSize: 10 }} onClick={() => { setActiveTeam(t.id); onTabChange?.('engine' as any); }}>Открыть →</button>
                        )}
                        <button type="button" style={{
                          background: 'none', border: 'none', fontSize: 10, color: 'rgba(255,255,255,0.35)',
                          cursor: 'pointer', padding: '2px 4px',
                        }} onClick={() => {
                          if (window.confirm(isLeader ? 'Удалить Движок? Это действие необратимо.' : 'Выйти из Движка?')) {
                            void (isLeader ? deleteTeam(t.id) : leaveTeam(t.id));
                          }
                        }}>{isLeader ? 'Удалить' : 'Выйти'}</button>
                      </div>
                    </div>
                  </div>
                );
          ...
          {myTeams.length < 3 && (
            <button type="button" className="btn-primary-gold" style={{ marginTop: 14, width: '100%', padding: '10px 0', fontSize: 13 }} onClick={() => onTabChange?.('engine-create' as any)}>+ Создать новый Движок</button>
          )}
        </div>
      </div>
    );
      ...
      return (
        <div className="fade-in" style={glassCard}>
          <div style={{ textAlign: 'center', padding: '30px 0' }}>
            ...
            <div style={{ fontSize: 12, opacity: 0.6 }}>Ты уже состоишь в 3 Движках. Выйди из одного, чтобы создать новый.</div>
          </div>
        </div>
      );
    }
  ...
  const renderEnginePlanSection = () => (
    <div className={`fade-in ${variant === 'cabin' ? 'team-cabin-section' : ''}`} style={sectionCard}>
      {!myTeam ? (
  ...
  // Initiative state
  const [initiatives, setInitiatives] = useState<{ id: string; teamId: string; title: string; description: string; createdBy: string; createdAt: string; votes: Record<string, boolean>; status: string; totalMembers: number }[]>([]);
  ...
  useEffect(() => { void loadInitiatives(); }, [loadInitiatives]);
  ...
  const renderCampControlSection = () => {
    ...
    const votedCount = (ini: typeof initiatives[0]) => Object.values(ini.votes || {}).filter(Boolean).length;
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 720, margin: '0 auto', width: '100%' }}>
        <div className="fade-in" style={glassCard}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            {!iniCreating ? (
              <button type="button" className="btn-secondary" style={{ padding: '5px 12px', fontSize: 11 }} onClick={() => setIniCreating(true)}>+ Создать</button>
            ) : null}
          </div>
          ...
          {iniLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {initiatives.map(ini => {
                ...
                const statusLabel = ini.status === 'voting' ? '🗳 Голосование' : ini.status === 'approved' ? '✅ Одобрена' : ini.status === 'sent_to_council' ? '📤 Отправлена' : '❌ Отклонена';
                return (
                  <div key={ini.id} style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 6 }}>
                      <div style={{ fontWeight: 700, fontSize: 13 }}>{ini.title}</div>
                      <span style={{ fontSize: 10, opacity: 0.6, whiteSpace: 'nowrap' }}>{statusLabel}</span>
                    </div>
                    {ini.description && <div style={{ fontSize: 12, opacity: 0.75, marginBottom: 8 }}>{ini.description}</div>}
                    {/* Progress bar */}
                    <div style={{ marginBottom: 8 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, opacity: 0.5, marginBottom: 3 }}>
                        <span>Голосов: {vc}/{ini.totalMembers}</span>
                        <span>{Math.round(progress)}%</span>
                      </div>
                      <div style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.1)' }}>
                        <div style={{ height: '100%', borderRadius: 2, background: ini.status === 'rejected' ? '#ff6b6b' : TEAM_ACCENT, width: `${progress}%`, transition: 'width 0.3s' }} />
                      </div>
                    </div>
                    {/* Action buttons */}
                    <div style={{ display: 'flex', gap: 8 }}>
                      {ini.status === 'voting' && !myVoted(ini) && (
                        <button type="button" className="btn-primary-gold" style={{ padding: '5px 12px', fontSize: 11 }} onClick={() => void voteOnInitiative(ini.id)}>👍 За</button>
                      )}
                      {ini.status === 'voting' && myVoted(ini) && (
                        <span style={{ fontSize: 11, opacity: 0.6, padding: '5px 0' }}>✓ Вы проголосовали</span>
                      )}
                      {ini.status === 'approved' && (
                        <button type="button" className="btn-primary-gold" style={{ padding: '5px 12px', fontSize: 11 }} onClick={() => void sendInitiativeToCouncil(ini.id)}>📤 Отправить в Совет</button>
                      )}
                      {ini.status === 'sent_to_council' && (
                        <span style={{ fontSize: 11, color: '#4ecdc4', padding: '5px 0' }}>Отправлена в Совет лагеря</span>
                      )}
                    </div>
                  </div>
                );
        ...
        {onSuggestInitiative ? (
          <button type="button" className="btn-secondary" onClick={onSuggestInitiative}>Предложить инициативу в совет лагеря (старый формат)</button>
        ) : null}
      </div>
    );
  };
  ...
  const summary = (
    <div className="team-cabin-section fade-in" style={{ display: 'flex', justifyContent: 'space-between', gap: 12, ...sectionCard }}>
  ...
  const renderEngineProject = () => {
    if (!myTeam) return <div className="fade-in" style={glassCard}><div style={{ textAlign: 'center', opacity: 0.6 }}>Сначала создай Движок</div></div>;

    const teamId = myTeam.id;
    const isFirstProject = !engineProjects.some(p => p.status === 'approved');
    const statusColors: Record<string, string> = { draft: '#999', in_progress: '#f0ad4e', review: '#5bc0de', approved: '#5cb85c', rejected: '#d9534f' };
    const statusLabels: Record<string, string> = { draft: '📝 Черновик', in_progress: '🔧 В работе', review: '📤 На проверке', approved: '✅ Утверждён', rejected: '❌ Отклонён' };
    ...
    const createProject = async (badgeIdOverride?: string) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setEngineProjects(prev => [...prev, proj]);
          setProjectForm({ title: '', description: '', plan: '', targetBadgeId: '', reflection: '', scenario: '' });
    ...
    const updateProject = async (projectId: string, patch: Record<string, unknown>) => {
      try {
        ...
        if (res.ok) { const updated = await res.json(); setEngineProjects(prev => prev.map(p => p.id === updated.id ? updated : p)); }
      } catch { /* */ }
    };
          ...
          <div className="fade-in" style={glassCard}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
              <div>
                ...
                {badge && (
                  <button type="button" onClick={() => onNavigateToBadge?.(badge.id)} style={{ fontSize: 12, opacity: 0.65, marginTop: 4, background: 'none', border: 'none', padding: 0, cursor: 'pointer', color: 'inherit' }}>
                    {badge.emoji} Цель: {badge.title} →
                  </button>
                )}
              </div>
              <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 8, background: `${statusColors[selectedProject.status]}22`, color: statusColors[selectedProject.status], border: `1px solid ${statusColors[selectedProject.status]}44`, flexShrink: 0 }}>
                {statusLabels[selectedProject.status]}
              </span>
            </div>
            {selectedProject.description && <div style={{ fontSize: 13, opacity: 0.75, marginBottom: 8 }}>{selectedProject.description}</div>}
            {selectedProject.plan && <div style={{ fontSize: 12, opacity: 0.6, whiteSpace: 'pre-wrap', background: 'rgba(255,255,255,0.03)', padding: 10, borderRadius: 8 }}>{selectedProject.plan}</div>}
          </div>
          ...
          {selectedProject.status === 'in_progress' && (
            <div className="fade-in" style={glassCard}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                {/* Photos */}
                <div>
                  <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 6 }}>📷 Фото (до 5 шт.)</div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {projectPhotos.map((p, i) => <img key={i} src={p} alt="" style={{ width: 56, height: 56, borderRadius: 8, objectFit: 'cover', border: '1px solid rgba(255,255,255,0.1)' }} />)}
                    {projectPhotos.length < 5 && (
                      <button type="button" onClick={() => projectPhotoRef.current?.click()} style={{ width: 56, height: 56, borderRadius: 8, border: '1px dashed rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.04)', cursor: 'pointer', fontSize: 20, display: 'grid', placeItems: 'center', color: 'rgba(255,255,255,0.4)' }}>+</button>
                    )}
                    <input ref={projectPhotoRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={e => {
                      const file = e.target.files?.[0];
                      if (file && projectPhotos.length < 5) {
                        const reader = new FileReader();
                        reader.onload = () => setProjectPhotos(prev => [...prev, reader.result as string]);
                        reader.readAsDataURL(file);
                      }
                      e.target.value = '';
                    }} />
                ...
                <textarea className="w-input" rows={3} placeholder="Описание проделанной работы / рефлексия" value={projectForm.reflection} onChange={e => setProjectForm(f => ({ ...f, reflection: e.target.value }))} />
                <textarea className="w-input" rows={2} placeholder="Сценарий мероприятия (если было)" value={projectForm.scenario} onChange={e => setProjectForm(f => ({ ...f, scenario: e.target.value }))} />
                <button type="button" className="btn-primary-gold" onClick={() => void updateProject(selectedProject.id, { status: 'review', reflection: projectForm.reflection, scenario: projectForm.scenario, photos: projectPhotos })}>📤 Отправить вожатому на проверку</button>
              </div>
            </div>
          )}
          ...
          {selectedProject.status === 'review' && (
            <div className="fade-in" style={glassCard}>
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <div style={{ fontSize: 12, opacity: 0.6, marginTop: 6 }}>Ожидайте подтверждения</div>
              </div>
            </div>
          )}
          ...
          {selectedProject.status === 'approved' && (
            <div className="fade-in" style={glassCard}>
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                {badge && <div style={{ fontSize: 14, marginTop: 8 }}>{badge.emoji} Значок «{badge.title}» получен!</div>}
                {selectedProject.reviewNote && <div style={{ fontSize: 12, opacity: 0.7, marginTop: 10, fontStyle: 'italic' }}>«{selectedProject.reviewNote}»</div>}
              </div>
            </div>
          )}
          ...
          {selectedProject.status === 'rejected' && (
            <div className="fade-in" style={glassCard}>
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                {selectedProject.reviewNote && <div style={{ fontSize: 13, opacity: 0.8, marginTop: 8 }}>Комментарий: «{selectedProject.reviewNote}»</div>}
                <button type="button" className="btn-secondary" style={{ marginTop: 14 }} onClick={() => void updateProject(selectedProject.id, { status: 'in_progress' })}>✏️ Доработать</button>
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 720, margin: '0 auto', width: '100%' }}>
        <div className="fade-in" style={glassCard}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            {!projCreating && (
              <button type="button" className="btn-secondary" style={{ padding: '5px 12px', fontSize: 11 }} onClick={() => setProjCreating(true)}>+ Создать</button>
            )}
          </div>
          ...
          {projCreating && (
            <div style={{ display: 'grid', gap: 8, marginBottom: 14, padding: 12, borderRadius: 10, background: 'rgba(255,255,255,0.04)' }}>
              <input className="w-input" placeholder="Название проекта" value={projectForm.title} onChange={e => setProjectForm(f => ({ ...f, title: e.target.value }))} />
              <textarea className="w-input" rows={2} placeholder="Описание" value={projectForm.description} onChange={e => setProjectForm(f => ({ ...f, description: e.target.value }))} />
              {/* Badge picker */}
              <div>
                <div style={{ fontSize: 12, opacity: 0.65, marginBottom: 8 }}>🎯 Целевой значок{isFirstProject ? ' (первый проект → Реальный Движок)' : ''}</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 6 }}>
                  {PATH_BADGES.map(b => {
                    const selected = effectiveBadgeId === b.id;
                    const locked = isFirstProject && b.id !== '8.1';
                    return (
                      <button key={b.id} type="button" disabled={locked}
                        onClick={() => !isFirstProject && setProjectForm(f => ({ ...f, targetBadgeId: b.id }))}
                        style={{
                          display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
                          padding: '8px 4px', borderRadius: 10, cursor: locked ? 'not-allowed' : 'pointer',
                          border: selected ? `2px solid ${TEAM_ACCENT}` : '1px solid rgba(255,255,255,0.08)',
                          background: selected ? 'rgba(139,0,255,0.12)' : 'rgba(255,255,255,0.03)',
                          opacity: locked ? 0.35 : 1, transition: 'all 0.15s',
                        }}>
                        <div style={{ width: 40, height: 40 }}>
                          <BadgeIcon badgeId={b.id} badgeTitle={b.title} categoryId="8" emoji={b.emoji} size="responsive" />
                        </div>
                        <span style={{ fontSize: 9, textAlign: 'center', lineHeight: 1.2, fontWeight: selected ? 700 : 400 }}>{b.title}</span>
                      </button>
                    );
                  })}
                </div>
              </div>
              <textarea className="w-input" rows={2} placeholder="План / сценарий" value={projectForm.plan} onChange={e => setProjectForm(f => ({ ...f, plan: e.target.value }))} />
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="button" className="btn-primary-gold" style={{ padding: '6px 14px', fontSize: 12 }} onClick={() => void createProject(effectiveBadgeId)} disabled={!projectForm.title.trim()}>Создать</button>
                <button type="button" className="btn-secondary" style={{ padding: '6px 14px', fontSize: 12 }} onClick={() => { setProjCreating(false); setProjectForm({ title: '', description: '', plan: '', targetBadgeId: '', reflection: '', scenario: '' }); }}>Отмена</button>
              </div>
            </div>
          )}
          ...
          {projectsLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {engineProjects.map(proj => {
                ...
                return (
                  <button key={proj.id} type="button" onClick={() => setSelectedProjectId(proj.id)} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        {badge && <div style={{ width: 28, height: 28, flexShrink: 0 }}><BadgeIcon badgeId={badge.id} badgeTitle={badge.title} categoryId="8" emoji={badge.emoji} size="responsive" /></div>}
                        <div style={{ fontWeight: 700, fontSize: 13 }}>{proj.title}</div>
                      </div>
                      <span style={{ fontSize: 10, opacity: 0.6, whiteSpace: 'nowrap', flexShrink: 0, color: statusColors[proj.status] }}>{statusLabels[proj.status]}</span>
                    </div>
                    {proj.description && <div style={{ fontSize: 12, opacity: 0.65, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{proj.description}</div>}
                  </button>
                );
              })}
            </div>
  ...
  const cabinContent = activeTab === 'my-engines'
    ? renderMyEngines()
