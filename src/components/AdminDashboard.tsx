...
import { fetchInbox, generateRoleCode, performAction } from '../utils/adminApi';
...
function getItemTitle(item: RawInboxItem): string {
  ...
  switch (item.type) {
    case 'badge_request': {
      ...
      return (
        (data.badge_name as string) || humanizeId(data.badge_id as string) || 'Заявка на значок'
      );
    }
    ...
    case 'bro_submission':
      return (
        (data.task_title as string) || `БРО: ${humanizeId(data.task_id as string)}` || 'БРО-задание'
      );
    case 'badge_plan': {
    ...
    case 'council_initiative': {
      ...
      if (data.sourceType === 'ode' && ciDesc) {
        ...
        for (const line of lines) {
          if (
            line.startsWith('Длительность:') ||
            line.startsWith('Аудитория:') ||
            line.startsWith('Масштаб:')
          ) {
            meta.push(line.trim());
          }
        }
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const pendingItems = useMemo(
    () => items.filter((i) => i.status === 'pending' || i.status === 'done_pending'),
    [items]
  );
  ...
  const handleAction = useCallback(
    async (itemId: string, itemType: string, action: 'approve' | 'reject', comment?: string) => {
      setBusy(itemId);
      try {
        ...
        if (action === 'approve' && itemType === 'role_request') {
          ...
          if (result.roleCode) {
            ...
            try {
              void navigator.clipboard.writeText(result.roleCode);
            } catch {
              /* */
            }
          }
          ...
          >
            ...
            {displayItems.map((item) => (
              <InboxCard
                ...
                onReject={() =>
                  void handleAction(item.id, item.type, 'reject', rejectComment.trim())
                }
              ...
              <div style={{ marginTop: 16, display: 'flex', gap: 24 }}>
                <div>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1a2e', marginTop: 2 }}>
                    {ROLE_OPTIONS.find((r) => r.value === codeResult.role)?.label ||
                      codeResult.role}
                  </div>
                </div>
                <div>
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 12,
        background: '#fff',
        border: `1px solid ${isExpanded ? meta.color + '44' : '#e8e8ed'}`,
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={onToggleExpand}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 500, color: '#888' }}>{userName}</span>
            {userRoleLabel && (
              <span
                style={{
                  fontSize: 10,
        ...
        <div style={{ display: 'flex', gap: 6, flexShrink: 0, alignItems: 'flex-start' }}>
          ...
          <button
            ...
            style={{
              ...
              border: '1px solid #fecaca',
              borderRadius: 8,
              background: isRejectOpen ? '#fef2f2' : '#fff',
              color: '#dc2626',
              cursor: 'pointer',
              fontFamily: FONT,
              transition: 'all 0.15s',
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                {typeof ev.durationMs === 'number' && (
                  ...
                  >
                    ⏱ {Math.floor((ev.durationMs as number) / 60000)} мин{' '}
                    {Math.floor(((ev.durationMs as number) % 60000) / 1000)} сек
                  </span>
                )}
