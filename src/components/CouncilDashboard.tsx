...
import type { CouncilMember, CouncilProtocol, InitiativeComment } from '../utils/councilApi';
import {
  addMember,
  createInitiative,
  createProtocol,
...
const COUNCIL_GRADIENT =
  'linear-gradient(135deg, rgba(255, 215, 0, 0.08) 0%, rgba(184, 134, 11, 0.12) 100%)';
...
const mapLegacyInitiativeStatus = (raw?: string): InitiativeStatus => {
  ...
  if (s === 'new' || s === 'reviewing' || s === 'accepted' || s === 'rejected' || s === 'done')
    return s;
...
export const CouncilDashboard: React.FC<CouncilDashboardProps> = ({
    ...
    const load = async () => {
      try {
        const [initData, protoData, membData] = await Promise.allSettled([
          tok
            ? fetchInitiatives(tok)
            : fetch('/api/council/initiatives')
                .then((r) => r.json())
                .then((d) => d.initiatives || []),
          fetchProtocols(tok),
          fetchMembers(tok),
        ]);
        if (cancelled) return;
        ...
        const list = Object.entries(asMap)
          .map(([id, raw]) => {
            ...
            return {
              ...
              members: membersRaw.filter((m) => m && typeof m === 'object') as Array<{
                id?: string;
              }>,
            } as TeamListItem;
          })
  ...
  useEffect(() => {
    if (variant !== 'cabin' || (activeTab !== 'camp-management' && activeTab !== 'management'))
      return;
  ...
  const handleRequestJoinEngine = useCallback(
  ...
  const councilMainSection = (
    <div
      className={variant === 'cabin' ? 'council-cabin-section fade-in' : 'fade-in'}
      style={{
        display: 'grid',
        gap: 16,
        padding: '16px 18px',
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
        ...
        <div style={statCardStyle}>
          <div style={statNumberStyle}>
            {dashLoading
              ? '—'
              : (initStats.byStatus['accepted'] || 0) + (initStats.byStatus['done'] || 0)}
          </div>
          <div style={statLabelStyle}>Принято / Выполнено</div>
        </div>
        <div style={statCardStyle}>
          <div style={statNumberStyle}>
            {dashLoading ? '—' : members.length || initStats.activeAuthors}
          </div>
                    ...
                    >
                      ...
                      {p.decisions && p.decisions.length > 0 && (
                        <div style={{ marginBottom: 8 }}>
                          <div style={{ fontWeight: 600, color: COUNCIL_ACCENT, marginBottom: 4 }}>
                            Решения:
                          </div>
                      ...
                      {p.participants && p.participants.length > 0 && (
                        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>
                          Присутствовали: {p.participants.join(', ')}
                        </div>
                      )}
                    </div>
                  )}
                ...
                return (
                  <div
                    ...
                    style={{
                      padding: '10px 14px',
                      borderRadius: 10,
                      background: isMe ? 'rgba(46,204,113,0.06)' : 'rgba(15, 10, 42, 0.12)',
                      border: isMe
                  ...
                  >
                    <div
                      style={{
                        ...
                        fontSize: 13,
                        fontWeight: 700,
                        color: MEMBER_ROLE_COLORS[m.role] || '#fff',
                        flexShrink: 0,
                        overflow: 'hidden',
                        border: `1px solid ${MEMBER_ROLE_COLORS[m.role] || 'rgba(255,255,255,0.7)'}44`,
                      }}
                    >
                      {m.avatar &&
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div
                        style={{
                          fontSize: 11,
                          color: MEMBER_ROLE_COLORS[m.role] || 'rgba(255,255,255,0.5)',
                        }}
                      >
          ...
          if (isInCouncil) {
            return (
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: 12 }}>
                <button
                  type="button"
                  onClick={() => void handleLeaveCouncil()}
                  style={{
                    padding: '10px 28px',
                    borderRadius: 12,
                    fontSize: 14,
            ...
            return (
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: 12 }}>
                <button
                  type="button"
                  disabled={memberBusy}
                  onClick={() => void handleJoinCouncil()}
                  style={{
                    padding: '12px 32px',
                    borderRadius: 14,
                    fontSize: 15,
      ...
      {showProtocolModal && (
        ...
        >
          ...
          >
            <h4 style={{ margin: '0 0 16px', color: COUNCIL_ACCENT, fontSize: 18 }}>
              Новый протокол
            </h4>
            <input
              type="text"
              placeholder="Название (напр. Заседание #3)"
              maxLength={200}
              value={protoTitle}
            ...
            <textarea
              ...
              maxLength={5000}
              value={protoSummary}
              onChange={(e) => setProtoSummary(e.target.value)}
              style={{
                width: '100%',
                minHeight: 60,
                padding: 10,
            ...
            />
            <textarea
              placeholder="Решения (одно на строку)"
              value={protoDecisions}
              onChange={(e) => setProtoDecisions(e.target.value)}
              style={{
                width: '100%',
                minHeight: 50,
                padding: 10,
            ...
            <input
              ...
              value={protoParticipants}
              onChange={(e) => setProtoParticipants(e.target.value)}
              style={{
                width: '100%',
                padding: 10,
                borderRadius: 8,
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                type="button"
                disabled={protoBusy || !protoTitle.trim()}
  ...
  const enginesSection = (
    <div
      className="council-cabin-section fade-in"
      style={{
        display: 'grid',
        gap: 14,
        padding: '16px 18px',
    ...
    >
      ...
      {teamsLoading ? (
        ...
        <div className="profile-error profile-error--not-found">
          Не удалось загрузить список Движков: {teamsError}
        </div>
      ) : teams.length === 0 ? (
                  ...
                  {isMine ? (
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: '#fff',
                        opacity: joinEngineBusyId === team.id ? 0.5 : 1,
                      }}
                    >
  ...
  const handleVote = useCallback(
    async (id: string, direction: 'up' | 'down' = 'up') => {
      if (voteBusy) return;
      setVoteBusy(id);
      try {
        ...
        setInitiatives((prev) =>
          prev.map((item) =>
            item.id === id
              ? {
                  ...item,
                  votesUp: res.initiative.votesUp,
                  votesDown: res.initiative.votesDown ?? 0,
                  voters: res.initiative.voters,
                  downVoters: res.initiative.downVoters ?? [],
                }
              : item
          )
        );
    ...
    async (id: string, newStatus: string) => {
      ...
      try {
        ...
        setInitiatives((prev) =>
          prev.map((item) =>
            item.id === id
              ? { ...item, status: newStatus, readStatus: mapLegacyInitiativeStatus(newStatus) }
              : item
          )
        );
      } catch {
  ...
  const handleCreate = useCallback(async () => {
    ...
    setCreateError(null);
    try {
      await createInitiative(accessToken || '', {
      ...
      >
        ...
        >
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              <h3
                style={{ margin: 0, fontSize: 17, fontWeight: 700, color: '#fff', lineHeight: 1.3 }}
              >
                {detailItem.title}
              </h3>
              {detailItem.teamName && (
          ...
          >
            {(detailItem.authorNickname || detailItem.createdByNickname) && (
              <span>
                {detailItem.authorNickname || detailItem.createdByNickname}
                {detailItem.authorRole ? ` · ${detailItem.authorRole}` : ''}
                {detailItem.authorWing ? ` · ${detailItem.authorWing}` : ''}
              </span>
            )}
            ...
            <span
              style={{
                ...
                fontWeight: 600,
                border: `1px solid ${KANBAN_COLUMN_COLORS[detailItem.status || 'idea'] || '#FFD700'}44`,
              }}
            >
              {SERVER_STATUS_LABELS[detailItem.status || 'idea'] || detailItem.status}
            </span>
            {detailItem.createdAt && (
              <span>{new Date(detailItem.createdAt).toLocaleDateString('ru-RU')}</span>
            )}
            <span>
              За {detailItem.votesUp ?? 0} · Против {(detailItem as any).votesDown ?? 0}
            </span>
          </div>
          ...
          {detailCommentsLoading ? (
            ...
            detailComments.map((c) => (
              ...
              >
                ...
                >
                  <span style={{ fontSize: 11, fontWeight: 600, color: COUNCIL_ACCENT }}>
                    {c.nickname}
                  </span>
    ...
    >
      ...
      {initiativesLoading ? (
        ...
        <p className="profile-empty-state__text">
          Инициатив пока нет. Предложи идею — она появится на доске.
        </p>
      ) : (
        <div
          style={{
        ...
        >
          {SERVER_STATUSES.map((status) => {
            ...
            const columnItems = initiatives.filter((i) => {
              const s = i.status || 'idea';
              return (s === 'proposed' || s === 'new' ? 'idea' : s) === status;
            });
            return (
              <div
                key={status}
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          {item.sourceType === 'brodela' && (
                            <span style={{ color: '#c4b5fd' }}>Бродела</span>
                          )}
                        </div>

                        {authorName && (
                          <div style={{ fontSize: 11, opacity: 0.45, letterSpacing: '0.02em' }}>
                            {authorName}
                            {item.authorRole ? ` · ${item.authorRole}` : ''}
                            {item.authorWing ? ` · ${item.authorWing}` : ''}
                          </div>
                        )}

                        {item.description && (
                          <div style={{ fontSize: 11, opacity: 0.4, lineHeight: 1.4 }}>
                          ...
                          {isInCouncil ? (
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                              ...
                              <button
                                ...
                                title="Против"
                              >
                                Против: {(item as any).votesDown ?? 0}
                              </button>
                            </div>
                          ) : (
                            <div
      ...
      {showCreateModal && (
        ...
        >
          <div
            className="cab-card"
            style={{ maxWidth: 400, width: '90%', padding: '24px' }}
            onClick={(e) => e.stopPropagation()}
          >
            <h4 style={{ margin: '0 0 16px', color: COUNCIL_ACCENT, fontSize: 18 }}>
              Новая инициатива
            ...
            {createError && (
              <div style={{ fontSize: 12, color: '#ff6b6b', marginBottom: 12 }}>{createError}</div>
            )}
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                type="button"
  ...
  const managementSection = (
    ...
    >
      ...
      {initiativesLoading ? (
        ...
        <p className="profile-empty-state__text">
          Инициатив пока нет. Они появятся здесь после отправки из Движков.
        </p>
      ) : (
        <div
          style={{
              ...
              >
                ...
                >
                  {columnItems.map((item) => {
                    ...
                    const nextStatus =
                      colIdx < SERVER_STATUSES.length - 1 ? SERVER_STATUSES[colIdx + 1] : null;
                    const authorName = item.authorNickname || item.createdByNickname || '';
                    return (
                      <div
                        key={item.id}
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <span>За: {item.votesUp ?? 0}</span>
                            <span
                              style={{ width: 1, height: 10, background: 'rgba(255,255,255,0.2)' }}
                            />
                            <span>Против: {(item as any).votesDown ?? 0}</span>
                          </div>
                          ...
                          <div style={{ display: 'flex', gap: 6 }}>
                            {prevStatus && (
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  void handleStatusChange(item.id, prevStatus);
                                }}
                                className="cab-btn-glass"
                                style={{
                                  width: 28,
  ...
  const badgeSection = (
    ...
    >
      ...
      >
        <div
          className="council-cabin-badge-card__icon"
          style={{
            width: 120,
            height: 120,
            borderRadius: 16,
        ...
        >
          <BadgeIcon
            badgeId="8.6"
            badgeTitle="Совет Реального Лагеря"
            categoryId="8"
            emoji=""
            size="large"
          />
        ...
        <div className="council-cabin-badge-card__text">
          ...
          <div style={{ fontSize: 12, opacity: 0.65, marginTop: 2 }}>
            Нажми, чтобы открыть страницу значка
          </div>
        </div>
      </button>
      ...
      >
        <div style={{ fontSize: 13, fontWeight: 600, color: COUNCIL_ACCENT, marginBottom: 8 }}>
          Как получить значок
        </div>
        <ul
          style={{
      ...
      >
        ...
        <div style={{ display: 'grid', gap: 6 }}>
          {[
            'Протоколы заседаний / дорожные карты',
            '10–15 откликов участников и 3–5 вожатых',
            'Видео / фото участия в инициативах',
          ].map((item, i) => (
            <div
              key={i}
              style={{
      ...
      {isExpanded && (
        <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <p style={{ fontSize: '14px', opacity: 0.9, lineHeight: 1.6 }}>
            Совет Реального Лагеря — площадка для инициатив, развития культуры лагеря и принятия
            коллективных решений. Участники предлагают идеи, обсуждают их и воплощают в жизнь.
          </p>
          ...
          >
            ...
            <p style={{ margin: '0 0 12px', fontSize: '13px', opacity: 0.9, lineHeight: 1.5 }}>
              {myTeam ? (
                <>
                  Совет объединяет Движки. Твой Движок «<strong>{myTeam.name}</strong>» — выноси
                  идеи от имени Движка, участвуй в обсуждениях.
                </>
              ) : (
