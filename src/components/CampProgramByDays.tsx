...
export interface Skill4kDetail {
  skill: string;
  via: string[];
}
export interface PedagogyTech {
  name: string;
  code?: number;
  examples: string[];
}
export interface DayPedagogy {
  skills4k: Skill4kDetail[];
  technologies: PedagogyTech[];
}
...
const actDesc = (a: string | ActivityItem): string =>
...
export const CampProgramByDays: React.FC<{ defaultShiftLength?: 9 | 21 }> = ({
  defaultShiftLength = 21,
}) => {
  ...
  const detectShift = React.useCallback(
  ...
  useEffect(() => {
    void detectShift(null);
  }, [detectShift]);
  // Retry when token becomes available (for production login)
  useEffect(() => {
    if (accessToken) void detectShift(accessToken);
  }, [accessToken, detectShift]);
  ...
  const days =
    shiftLength === 9 ? (data?.days_9 ?? data?.days?.slice(0, 9) ?? []) : (data?.days ?? []);
  const currentDay = days.find((d) => d.day === activeDay) ?? days[0];
  const hasPedagogy = !!(
    currentDay?.pedagogy?.skills4k?.length || currentDay?.pedagogy?.technologies?.length
  );
    ...
    <div style={{ fontFamily: 'inherit', color: '#fff' }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                fontFamily: 'inherit',
                border: isActive
                  ? '2px solid rgba(199,119,48,0.85)'
      ...
      <div
        style={{
          height: 2,
          margin: '0 16px',
          background: 'linear-gradient(to right, rgba(199,119,48,0.6), transparent 85%)',
          borderRadius: 2,
        }}
      />
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {currentDay.pedagogy.skills4k.map((s, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          style={{ borderLeft: `2px solid ${color}60`, paddingLeft: 10 }}
                        >
                    ...
                    {currentDay.pedagogy.technologies.map((t, i) => (
                      <div key={i}>
                        ...
                        >
                          {t.name}
                          {t.code ? (
                            <span style={{ opacity: 0.45, fontWeight: 400 }}> (#{t.code})</span>
                          ) : null}
                        </div>
                        <div
                          style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}
                        >
                          {t.examples.join(', ')}
                        </div>
                      </div>
