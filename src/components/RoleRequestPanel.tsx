...
import { submitRoleRequest, fetchMyRoleRequests, type RoleRequest } from '../utils/roleRequestApi';
...
export const RoleRequestPanel: React.FC<RoleRequestPanelProps> = ({ accessToken, deviceId, currentRole }) => {
    ...
    return (
        <div style={{ padding: '16px 0', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <p style={{ margin: 0, fontSize: '13px', color: 'rgba(255,255,255,0.65)', lineHeight: 1.5 }}>
                Ваша текущая роль: <strong style={{ color: 'rgba(139, 0, 255, 0.9)' }}>{currentRole}</strong>.
                Подайте заявку на новую роль — администратор рассмотрит её.
            </p>
            ...
            {pendingRequest && (
                <div style={{
                    ...
                    ⏳ У вас уже есть заявка на роль <strong>{pendingRequest.desiredRole}</strong> — ожидает решения.
                </div>
            )}
                ...
                <>
                            ...
                            {AVAILABLE_ROLES.map((r) => (
                                <button
                                    ...
                                    style={{
                                        ...
                                        borderRadius: '10px',
                                        background: selectedRole === r.value ? 'rgba(139,0,255,0.2)' : 'rgba(255,255,255,0.04)',
                    ...
                    <button
                        ...
                        style={{
                            ...
                            fontWeight: 700,
                            color: '#fff',
                            background: selectedRole ? 'linear-gradient(135deg, #8B00FF, #4D00B4)' : 'rgba(255,255,255,0.08)',
                            border: 'none',
                            borderRadius: '12px',
            ...
            {myRequests.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '8px' }}>
                    <h4 style={{ margin: 0, fontSize: '13px', fontWeight: 700, color: 'rgba(255,255,255,0.8)' }}>
                        Мои заявки
                    </h4>
                    {loadingRequests && <p style={{ margin: 0, fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Загрузка...</p>}
                    {myRequests.map((req) => {
                        const st = statusLabel[req.status] || { text: req.status, color: 'rgba(255,255,255,0.6)' };
                        return (
