...
export function getPendingOAuthRole(): string | null {
  try {
    return localStorage.getItem(LS_OAUTH_ROLE);
  } catch {
    return null;
  }
}
...
export function clearPendingOAuthRole(): void {
  try {
    localStorage.removeItem(LS_OAUTH_ROLE);
  } catch {
    /* */
  }
}
...
export const RoleSelectionModal: React.FC<RoleSelectionModalProps> = ({
  ...
  const handleDevPinSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && data.accessToken) {
        try {
          localStorage.setItem('rl-dev-pin', trimmed);
        } catch {}
  ...
  const handleEmailMagicLink = useCallback(async () => {
    ...
    try {
      if (selectedRole) {
        try {
          localStorage.setItem(LS_OAUTH_ROLE, selectedRole);
        } catch {
          /* */
        }
      }
    ...
    const check = async () => {
      try {
        const res = await fetch(
          `${base}/api/role-requests?deviceId=${encodeURIComponent(deviceId)}`
        );
        ...
        if (req?.status === 'approved') {
          ...
          onResult({
            ...
            role: req.desiredRole as UserRole,
  ...
  const handleSelectRole = useCallback(
    (roleId: UserRole) => {
      try {
        localStorage.setItem(LS_KEY, roleId);
      } catch {
        /* */
      }
  ...
  const handleOAuth = useCallback(
    async (provider: 'google' | 'yandex' | 'vk') => {
  ...
  const handleSubmitRequest = useCallback(async () => {
    ...
    try {
      ...
      setSubmittedReq(
        rr.id ? { id: rr.id, desiredRole: rr.desiredRole || selectedRole || '' } : null
      );
      ...
      >
        ...
        {step === 'select' && (
          <>
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              <img
                src={`${import.meta.env.BASE_URL}${NAV_HOME_IMAGE}`}
                alt="Реальный Лагерь"
                style={{
                  width: 96,
                  height: 96,
            ...
            <button
              ...
              onMouseOver={(e) => {
                e.currentTarget.style.background = 'rgba(93,228,255,0.15)';
                e.currentTarget.style.borderColor = 'rgba(93,228,255,0.4)';
              }}
              onMouseOut={(e) => {
                e.currentTarget.style.background = 'rgba(93,228,255,0.1)';
                e.currentTarget.style.borderColor = 'rgba(93,228,255,0.2)';
              }}
            >
              ...
              {import.meta.env.DEV && (
                ...
                >
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#06b6d4' }}>
                    ⚡ Песочница — переключить сразу
                  </div>

                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', marginTop: 2 }}>
                    Dev-режим: мгновенная смена роли без кода
                  </div>
                </button>
              )}
        ...
        {step === 'request' && !reqDone && (
          <>
            ...
            <button
              type="button"
              disabled={reqBusy || !canSubmitRequest}
              ...
              style={{
                ...
                fontSize: 14,
                fontWeight: 700,
                cursor: canSubmitRequest ? 'pointer' : 'default',
              }}
            >
          ...
          <>
            ...
            <button
              ...
              style={{
                ...
                fontSize: 14,
                fontWeight: 700,
                cursor: devPin.trim() ? 'pointer' : 'default',
              }}
            >
        ...
        {step === 'done' && (
          <>
            <div style={{ textAlign: 'center', padding: '10px 0' }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: '#22c55e', marginBottom: 6 }}>
                Заявка отправлена!
              </div>

              <div style={{ fontSize: 13, opacity: 0.6, lineHeight: 1.5, marginBottom: 8 }}>
                Мы проверим вашу заявку и отправим
                <br />
                код доступа на почту
                {reqEmail ? (
                  <>
                    {' '}
                    <b style={{ color: '#d97706' }}>{reqEmail}</b>
                  </>
                ) : (
                  ''
                )}
              ...
              <div style={{ fontSize: 12, opacity: 0.4, lineHeight: 1.4, marginBottom: 16 }}>
                ...
                <br />
                Роль: <b style={{ color: '#f59e0b' }}>{roleLabel}</b>
              </div>
        ...
        {step === 'oauth-verify' && (
          <>
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: '#3b82f6', marginBottom: 4 }}>
                Заявка отправлена!
              </div>

              <div style={{ fontSize: 12, opacity: 0.55, lineHeight: 1.5 }}>
                Войдите через почту, чтобы мы знали кто вы.
                <br />
                Админ одобрит заявку на <b style={{ color: '#f59e0b' }}>{roleLabel}</b>
              </div>
            </div>
              ...
              {emailSent ? (
                <div style={{ textAlign: 'center', padding: '12px 0' }}>
                  ...
                  <div style={{ fontSize: 11, opacity: 0.5, marginTop: 4 }}>
                    Проверьте <b>{emailInput}</b> и нажмите на ссылку
                  </div>
                </div>
              ) : (
                <div style={{ display: 'flex', gap: 6 }}>
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') void handleEmailMagicLink();
                    }}
                  />
