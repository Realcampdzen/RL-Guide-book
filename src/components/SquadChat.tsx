...
export const SquadChat: React.FC<SquadChatProps> = ({
  squadId,
  accessToken,
  nickname: myNickname,
  deviceId,
  ...
  const handlePin = useCallback(
    async (msgId: string, pin: boolean) => {
      try {
        const res = await pinSquadMessage(
          isRealJwt ? accessToken : '',
          squadId,
          msgId,
          pin,
  ...
  const isWingChatLocked =
    chatType === 'wing' && typeof error === 'string' && error.includes('Authorization');
      ...
      >
        <div
          style={{
            fontSize: 56,
            marginBottom: 20,
            filter: 'drop-shadow(0 0 24px rgba(139,0,255,0.4))',
          }}
        >
          🔒
        </div>
        <h3 style={{ margin: '0 0 12px', fontSize: 22, color: '#e8f0ff', fontWeight: 700 }}>
          Чат Крыла недоступен
        </h3>
        <p
          style={{
            margin: 0,
        ...
        >
          Чтобы открыть доступ к общению, тебе необходимо пройти{' '}
          <strong style={{ color: '#fff' }}>Бросвящение</strong> и создать или присоединиться к
          своему <strong style={{ color: '#fff' }}>Крылу</strong>.
        </p>
      </div>
    );
  }
    ...
    >
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', marginBottom: 2 }}>
              {pinnedMsg.nickname || 'Участник'} ·{' '}
              {ROLE_LABELS[pinnedMsg.role || ''] || pinnedMsg.role}
            </div>
            <div
              style={{
                fontSize: 12,
          ...
          <button
            type="button"
            onClick={() => void handlePin(pinnedMsg.id, false)}
            title="Открепить"
            style={{
              padding: '2px 6px',
              borderRadius: 6,
      ...
      >
        {loading && messages.length === 0 ? (
          ...
          messages.map((m) => {
            const writer = members.find(
              (x) => x.deviceId === m.deviceId || (x.nickname && x.nickname === m.nickname)
            );
              ...
              >
                ...
                >
                  ...
                  >
                    <span
                      style={{
                        fontWeight: 600,
                        fontSize: 13,
                        color: isCounselor ? '#f472b6' : '#e8f0ff',
                      }}
                    >
                      {m.nickname || 'Участник'}
                    </span>
                    ...
                    {isPinned(m) && (
                      <span
                        style={{
                          fontSize: 11,
                          filter: 'drop-shadow(0 0 4px rgba(139, 0, 255, 0.5))',
                        }}
                        title="Закреплено"
                      >
                        📌
                    ...
                    >
                      <button
                        type="button"
                        title={isPinned(m) ? 'Открепить' : 'Закрепить'}
                        ...
                        onMouseLeave={(e) => {
                          e.currentTarget.style.background = isPinned(m)
                            ? 'rgba(139, 0, 255, 0.2)'
                            : 'transparent';
                      ...
                      <button
                        ...
                        title="Удалить"
                        onClick={() => {
                          if (window.confirm('Удалить сообщение?')) void handleDelete(m.id);
                        }}
                        style={{
                          padding: 4,
                          borderRadius: 6,
                  ...
                  <div
                    style={{
                      position: 'relative',
                      background: isPinned(m)
                        ? 'linear-gradient(135deg, rgba(139, 0, 255, 0.15), rgba(139, 0, 255, 0.05))'
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
        <textarea
          ...
          onFocus={(e) => {
            e.currentTarget.style.borderColor = 'rgba(139, 0, 255, 0.4)';
          }}
          onBlur={(e) => {
      ...
      <div
        style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', marginTop: 6, textAlign: 'right' }}
      >
