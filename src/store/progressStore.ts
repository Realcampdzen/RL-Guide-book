...
export interface ProgressStore {
  ...
  userData: IUserData;
  setUserData: (updater: IUserData | ((prev: IUserData) => IUserData)) => void;
  setPathFavToast: (t: PathFavToastState | null) => void;
...
export const applyTestDefaults = (data: IUserData, enabled: boolean): IUserData => {
...
export const normalizeUserData = (raw: any): IUserData => {
  const profile = {
    ...initialProfile,
    ...(raw?.profile || {}),
    stats: {
...
export const useProgressStore = create<ProgressStore>()((set, get) => {
  ...
  const getBadgePlan = (badgeId: string): IBadgePlan | undefined => {
    return get().userData.badgePlans?.[badgeId];
  };
  ...
  const toggleFavorite = (
    ...
    const prevFavorites = get().userData.favorites || [];
  ...
  const getLevelProgress = (levelId: BadgeLevelId) => {
    return get().userData.progress[levelId];
  };
  ...
  const resetProgress = () => {
    if (confirm('Вы уверены? Весь прогресс будет удален безвозвратно.')) {
      setUserData(applyTestDefaults(initialData, get().isTestMode));
    }
  };
