...
import { checkPlanApiAvailable, fetchBadgePlan, structureUserPlan } from '../../../utils/aiService';
    ...
    >
      ...
      >
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="profile-modal-plan-title"
          onClick={(e) => e.stopPropagation()}
        >
          <h3 id="profile-modal-plan-title" style={{ marginTop: 0, marginBottom: 8 }}>
            План получения: {planFormBadge.title}
          </h3>
          {planApiAvailable === false && !planResult && (
            ...
            >
              Для работы ИИ нужен запущенный backend. Запусти:{' '}
          ...
          {planResult && planStep !== 'context' ? (
            <>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 16px' }}>
                {planResult.checklistItems.map((item, i) => (
                  <li
                    key={i}
              ...
              {!planForm.squadProgramGrid?.trim() && !planForm.campProgram3d?.trim() && (
                ...
                >
                  Программа отряда и лагеря не указаны — использовалась типовая программа. Для
                  точной привязки шагов к мероприятиям нажми «Изменить контекст» и заполни
                  программу.
                </p>
              )}
              <button
                onClick={() => setPlanStep('context')}
                className="btn-secondary"
                style={{ width: '100%', marginBottom: 8 }}
                title="Добавить или изменить программу отряда/лагеря, день смены"
              >
                Изменить контекст
              </button>
              <button
                  ...
                  try {
                    ...
                    if (res) {
                      ...
                      showHint({
                        title: 'Ошибка API',
                        content:
                          'Не удалось дополнить план. Убедись, что backend запущен: npm run start:backend',
                      });
                  } catch (e) {
                    console.error('fetchBadgePlan (Дополнить):', e);
                    showHint({
                      title: 'Ошибка API',
                      content: 'Не удалось дополнить план. Запусти backend: npm run start:backend',
                    });
                  } finally {
                    setPlanBusy(false);
                  }
                ...
                className="btn-secondary"
                style={{ width: '100%', marginBottom: 8 }}
                title="Доработать план с учётом программы отряда, дня смены и мероприятий"
              >
                {planBusy ? 'Дополняем…' : 'Дополнить с учётом программы'}
              </button>
              <button
                onClick={async () => {
                  if (accessToken) {
                    ...
                    try {
                      ...
                      showHint({
                        title: 'План отправлен!',
                        content:
                          'План отправлен вожатому на проверку. Статус отобразится в карточке.',
                      });
                    } catch (e) {
                      ...
                      showHint({
                        ...
                        content:
                          e instanceof Error
                            ? e.message
                            : 'Не удалось отправить план. Проверьте подключение.',
                      });
                    } finally {
                      setPlanBusy(false);
                    }
                    ...
                    showHint({
                      title: 'Отправлено',
                      content:
                        'План открыт в Telegram. После подтверждения вожатым нажми «Вожатый утвердил».',
                    });
                  }
                }}
                disabled={planBusy}
                ...
                style={{ width: '100%', marginBottom: 8 }}
              >
                {planBusy ? 'Отправляем…' : 'Отправить план вожатому'}
              </button>
              <button
                onClick={() => {
                  ...
                  showHint({
                    title: 'Готово',
                    content: 'План утверждён. Отмечай выполнение шагов в карточке.',
                  });
                  handleClose();
                }}
                className="btn-secondary"
            ...
            <>
              <p style={{ fontSize: 12, opacity: 0.75, marginBottom: 12, lineHeight: 1.5 }}>
                Измени программу, день смены — и нажми «Дополнить», чтобы пересобрать план с учётом
                контекста.
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 12 }}>
                    ...
                    >
                      <input
                        type="radio"
                        checked={planForm.shiftLength === 21}
                        onChange={() =>
                          setPlanForm({
                            ...planForm,
                            shiftLength: 21,
                            currentDay: Math.min(21, planForm.currentDay),
                          })
                        }
                      />{' '}
                      21 день
                    ...
                    >
                      <input
                        type="radio"
                        checked={planForm.shiftLength === 9}
                        onChange={() =>
                          setPlanForm({
                            ...planForm,
                            shiftLength: 9,
                            currentDay: Math.min(9, planForm.currentDay),
                          })
                        }
                      />{' '}
                      9 дней
                ...
                <div>
                  ...
                  >
                    День смены (1–{planForm.shiftLength})
                  </label>
                  <input
                    type="number"
                    min={1}
                    max={planForm.shiftLength}
                    value={planForm.currentDay}
                    onChange={(e) =>
                      setPlanForm({
                        ...planForm,
                        currentDay: Math.min(
                          planForm.shiftLength,
                          Math.max(1, parseInt(e.target.value, 10) || 1)
                        ),
                      })
                    }
                    className="w-input"
                    style={{ width: '80px', padding: 8 }}
                  />
                </div>
                <div>
                  <label
                  ...
                  <textarea
                    placeholder="План-сетка отряда на ближайшие дни…"
                    className="w-input"
                    rows={2}
                    style={{ width: '100%', padding: 10, resize: 'vertical' }}
                    value={planForm.squadProgramGrid}
                    onChange={(e) => setPlanForm({ ...planForm, squadProgramGrid: e.target.value })}
                  />
                </div>
                <div>
                  <label
                  ...
                  <textarea
                    placeholder="План вожатых на ближайшие 3 дня…"
                    className="w-input"
                    rows={2}
                    style={{ width: '100%', padding: 10, resize: 'vertical' }}
                    value={planForm.squadPlan3d}
                    onChange={(e) => setPlanForm({ ...planForm, squadPlan3d: e.target.value })}
                  />
                </div>
                <div>
                  <label
                  ...
                  <textarea
                    placeholder="Общая план-сетка лагеря на ближайшие 3 дня…"
                    className="w-input"
                    rows={2}
                    style={{ width: '100%', padding: 10, resize: 'vertical' }}
                    value={planForm.campProgram3d}
                    onChange={(e) => setPlanForm({ ...planForm, campProgram3d: e.target.value })}
                  />
                </div>
                <div>
                  <label
            ...
            <>
              <p style={{ fontSize: 12, opacity: 0.75, marginBottom: 12, lineHeight: 1.5 }}>
                Редактируй шаги, добавляй свои, или дополни план с учётом программы смены.
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                {planChecklistItems.map((item, i) => (
                  <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setPlanChecklistItems((prev) => prev.filter((_, j) => j !== i))
                      }
                ...
                <button
                  type="button"
                  onClick={() => setPlanChecklistItems((prev) => [...prev, ''])}
                  className="btn-secondary"
                ...
                onClick={async () => {
                  ...
                  try {
                    const res = await fetchBadgePlan({
                      ...
                      existingChecklist:
                        planChecklistItems.filter((s) => s.trim()).length > 0
                          ? planChecklistItems.filter((s) => s.trim())
                          : undefined,
                    });
                    if (res) {
                      setPlanResult(res);
              ...
              <button
                onClick={() => {
                  ...
                  if (items.length === 0) {
                    showHint({
                      title: 'Добавь шаги',
                      content: 'Добавь хотя бы один шаг в чек-лист.',
                    });
                    return;
                  }
                  setPlanResult({
                ...
                style={{ width: '100%' }}
              >
                Отправить без дополнения
              </button>
            </>
          ) : (
            <>
              <p style={{ fontSize: 12, opacity: 0.75, marginBottom: 16, lineHeight: 1.5 }}>
                Заполни контекст и напиши свои мысли — ИИ поможет структурировать и дополнить план.
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div>
                  ...
                  <textarea
                    placeholder="Как ты видишь свой путь к этому значку? Запиши мысли, идеи, первые шаги…"
                    className="w-input"
                    rows={3}
                    style={{ width: '100%', padding: 10, resize: 'vertical' }}
                    value={planForm.myPlanDraft}
                    onChange={(e) => setPlanForm({ ...planForm, myPlanDraft: e.target.value })}
                  />
                </div>
              </div>
              {planForm.myPlanDraft?.trim() ? (
                <button
                  onClick={async () => {
                    setPlanBusy(true);
                    try {
                      const res = await structureUserPlan({
                        badgeId: planFormBadge.id,
                        badgeTitle: planFormBadge.title,
                        myPlanDraft: planForm.myPlanDraft.trim(),
                      });
                      if (res && res.checklistItems.length > 0) {
                        ...
                        showHint({
                          title: 'Ошибка API',
                          content:
                            'Не удалось структурировать план. Проверь, что backend запущен: npm run start:backend',
                        });
                    } catch (e) {
                      ...
                      showHint({
                        title: 'Ошибка API',
                        content:
                          'Не удалось структурировать план. Запусти backend: npm run start:backend',
                      });
                    } finally {
                      setPlanBusy(false);
                    }
                ...
                >
                  {planBusy ? 'Структурируем…' : 'Структурировать'}
                </button>
              ) : (
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (res) {
                        ...
                        setPlanError(
                          'Не удалось сгенерировать план. Убедись, что backend запущен: npm run start:backend'
                        );
                        showHint({
                          title: 'Ошибка API',
                          content:
                      ...
                      setPlanError(
                        'Не удалось сгенерировать план. Backend не отвечает или произошла ошибка сети. Запусти: npm run start:backend'
                      );
                      showHint({
                        title: 'Ошибка API',
                        content:
                          'Не удалось сгенерировать план. Запусти backend: npm run start:backend',
                      });
                    } finally {
                      setPlanBusy(false);
                    }
                  ...
                  title={
                    planApiAvailable === false
                      ? 'Backend не запущен — при клике покажем подсказку'
                      : ''
                  }
                >
                  {planBusy ? 'Генерируем…' : 'Сгенерировать план'}
                </button>
              )}
            </>
          )}
