...
export class Orchestrator {
  ...
  async start() {
    ...
    logger.info(`Active states: [${getActiveStates(this.config).join(', ')}]`, {
      component: 'orchestrator',
    });
    logger.info(`Workspace root: ${this.config.workspace.root}`, { component: 'orchestrator' });
    logger.info(`Max concurrent agents: ${this.config.agent.max_concurrent_agents}`, {
      component: 'orchestrator',
    });
    logger.info(`Poll interval: ${this.state.poll_interval_ms}ms`, { component: 'orchestrator' });
    const agentMode =
      this.config.codex.command === 'openai' || !this.config.codex.command
        ? 'openai-api'
    ...
    logger.info(`Polling every ${this.state.poll_interval_ms / 1000}s. Press Ctrl+C to stop.`, {
      component: 'orchestrator',
    });
  }
  ...
  async stop() {
    ...
    if (promises.length > 0) {
      logger.info(`Waiting for ${promises.length} running agent(s) to stop...`, {
        component: 'orchestrator',
      });
      await Promise.race([Promise.allSettled(promises), new Promise((r) => setTimeout(r, 10000))]);
    }
    ...
    try {
      ...
      for (const issue of candidates) {
        ...
        if (this.state.running.size >= this.state.max_concurrent_agents) {
          logger.debug('All agent slots occupied, skipping remaining candidates', {
            component: 'orchestrator',
          });
          break;
        }
    ...
    for (const [issueId, entry] of this.state.running) {
      try {
        ...
        if (isTerminalIssue(this.config, current)) {
          logger.info(
            `Issue ${current.identifier} is now terminal (${current.state}), stopping run`,
            {
              component: 'orchestrator',
              issue_id: issueId,
            }
  ...
  private async worker(issue: Issue, entry: RunningEntry, attempt: number | null) {
    ...
    try {
      ...
      if (result.success) {
        ...
        logger.info(
          `✅ Worker completed successfully for ${issue.identifier} (${runtimeSeconds.toFixed(1)}s)`,
          {
            component: 'orchestrator',
            issue_identifier: issue.identifier,
          }
      ...
      const currentAttempt = (attempt || 0) + 1;
      const backoff = Math.min(1000 * 2 ** currentAttempt, this.config.agent.max_retry_backoff_ms);
      this.scheduleRetry(issue, currentAttempt, backoff);
    } finally {
  ...
  private async safeTrackerComment(issueId: string, body: string) {
    try {
      ...
      logger.warn(`Tracker comment sync failed: ${err}`, {
        component: 'orchestrator',
        issue_id: issueId,
      });
    }
  ...
  private async safeTrackerState(issueId: string, stateName: string) {
    try {
      ...
      logger.warn(`Tracker state sync failed (${stateName}): ${err}`, {
        component: 'orchestrator',
        issue_id: issueId,
      });
    }
...
function isTerminalIssue(config: ServiceConfig, issue: Issue): boolean {
  ...
  return (
    issue.state === 'Done' || issue.labels.some((label) => tracker.terminal_labels.includes(label))
  );
}
