...
export class LinearTracker implements TrackerClient {
  ...
  async fetchIssueById(issueId: string): Promise<Issue | null> {
    const response = await this.graphql<{ issue: LinearIssueNode | null }>(ISSUE_BY_ID_QUERY, {
      id: issueId,
    });
  ...
  private async resolveStateId(stateName: string): Promise<string | null> {
    ...
    const response = await this.graphql<{
      teams?: { nodes?: Array<{ states?: { nodes?: LinearStateNode[] } | null }> | null } | null;
    }>(TEAM_STATES_QUERY, { teamKey: this.teamKey });
  ...
  private async fetchScopedIssues(): Promise<LinearIssueNode[]> {
    if (this.projectSlug) {
      const response = await this.graphql<{
        projects?: {
          nodes?: Array<{ issues?: { nodes?: LinearIssueNode[] } | null }> | null;
        } | null;
      }>(PROJECT_ISSUES_QUERY, { projectSlug: this.projectSlug });
      const project = response.projects?.nodes?.[0];
      return project?.issues?.nodes || [];
    }

    const response = await this.graphql<{
      teams?: { nodes?: Array<{ issues?: { nodes?: LinearIssueNode[] } | null }> | null } | null;
    }>(TEAM_ISSUES_QUERY, { teamKey: this.teamKey });
