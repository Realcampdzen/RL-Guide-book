...
function buildTrackerConfig(c: Record<string, unknown>): TrackerConfig {
  ...
  if (trackerKind === 'github') {
    ...
    const customMap = getNestedValue(c, 'tracker.label_state_map') as
      | Record<string, string>
      | undefined;
    const labelStateMap =
      customMap && typeof customMap === 'object'
    ...
    const tracker: GitHubTrackerConfig = {
      ...
      active_labels:
        asStringList(getNestedValue(c, 'tracker.active_labels')).length > 0
          ? asStringList(getNestedValue(c, 'tracker.active_labels'))
          : ['todo', 'in-progress'],
      terminal_labels:
        asStringList(getNestedValue(c, 'tracker.terminal_labels')).length > 0
  ...
  if (trackerKind !== 'linear') {
    logger.warn(`Unsupported tracker.kind "${trackerKind}", falling back to "linear"`, {
      component: 'config',
    });
  }
  ...
  const tracker: LinearTrackerConfig = {
    ...
    active_states:
      asStringList(getNestedValue(c, 'tracker.active_states')).length > 0
        ? asStringList(getNestedValue(c, 'tracker.active_states'))
        : DEFAULT_LINEAR_ACTIVE_STATES,
    terminal_states:
      asStringList(getNestedValue(c, 'tracker.terminal_states')).length > 0
        ? asStringList(getNestedValue(c, 'tracker.terminal_states'))
        : DEFAULT_LINEAR_TERMINAL_STATES,
    agent_started_state:
      asNullableString(getNestedValue(c, 'tracker.agent_started_state')) || 'In Progress',
    agent_done_state:
      asNullableString(getNestedValue(c, 'tracker.agent_done_state')) || 'In Review',
...
export function buildServiceConfig(workflow: WorkflowDefinition): ServiceConfig {
  ...
  const codex: CodexConfig = {
    ...
    turn_sandbox_policy:
      turnSandboxPolicy && typeof turnSandboxPolicy === 'object'
  ...
  const workspaceRoot = rawRoot ? expandPath(rawRoot) : resolve(tmpdir(), 'symphony_workspaces');
...
export function validateConfig(config: ServiceConfig): ValidationError[] {
  ...
  if (config.tracker.kind === 'github') {
    ...
    if (!config.tracker.token) {
      errors.push({
        field: 'tracker.token',
        message: 'Required: GitHub token ($GITHUB_TOKEN or explicit)',
      });
    }
  } else {
    if (!config.tracker.api_key) {
      errors.push({
        ...
        message: 'Required: Linear API key ($LINEAR_API_KEY or explicit)',
      });
    }
    if (!config.tracker.team_key) {
      errors.push({
        field: 'tracker.team_key',
        message: 'Required: Linear team key (for example PUT)',
      });
    }
  }
...
export function watchWorkflowFile(
  ...
  const handler = () => {
    try {
      ...
      if (errors.length > 0) {
        logger.error(
          `Config validation failed after reload: ${errors.map((e) => `${e.field}: ${e.message}`).join('; ')}`,
          { component: 'config' }
        );
        return;
      }
