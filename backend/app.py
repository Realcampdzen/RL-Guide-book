...
# Gmail SMTP for sending role approval emails
SMTP_EMAIL = os.getenv('SMTP_EMAIL', '').strip()
SMTP_PASSWORD = os.getenv('SMTP_PASSWORD', '').strip()  # Gmail App Password
...
def _send_role_approval_email(email: str, desired_role: str, role_code: str, name: str = "") -> dict:
    """Send role approval code via Gmail SMTP. Returns delivery metadata."""
    import smtplib
    from email.mime.text import MIMEText
    from email.mime.multipart import MIMEMultipart

    delivery = {"attempted": False, "sent": False, "provider": "gmail_smtp"}
    to_email = (email or "").strip().lower()
    if not to_email:
        delivery["error"] = "email_missing"
        return delivery
    if not SMTP_EMAIL or not SMTP_PASSWORD:
        delivery["error"] = "smtp_credentials_missing"
        return delivery

    role_label = _role_request_label_ru(desired_role)
    user_name = (name or "").strip()
    greeting = f"{user_name}," if user_name else "Здравствуйте!"

    subject = f"Реальный Лагерь -- код доступа ({role_label})"
    text_body = (
        f"{greeting}\n\n"
        f"Ваша заявка на роль \xab{role_label}\xbb одобрена!\n\n"
        f"Ваш код доступа: {role_code}\n\n"
        "Как войти:\n"
        "1. Откройте сайт Реального Лагеря\n"
        "2. Нажмите \xabВойти\xbb -> выберите свою роль\n"
        "3. Нажмите \xabУ меня есть код\xbb\n"
        f"4. Введите код: {role_code}\n\n"
        "Если вы не отправляли заявку -- просто проигнорируйте это письмо.\n\n"
        "-- Команда Реального Лагеря"
    )
    ...
    try:
        ...
        delivery["error"] = f"smtp_error: {str(exc)[:300]}"
    return delivery
...
def role_requests_create():
    ...
    role_label = _RR_ROLE_LABELS.get(desired_role, desired_role)
    tg_text = (
...
def role_requests_list():
    ...
    if show_all:
        ...
        payload, err = _require_roles(ORGANIZER_ROLES + ('educator',), allow_localhost_dev=True)
        if err is not None:
            return err[0], err[1]
        all_requests = _load_role_requests()
        result = []
        for rr in all_requests:
            if not isinstance(rr, dict):
                continue
            row = dict(rr)
            if row.get("status") == "approved" and AUTH_JWT_SECRET:
...
@app.route('/api/role-requests/<request_id>', methods=['PATCH'])
def role_requests_review(request_id):
    """
