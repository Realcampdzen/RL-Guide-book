...
export class ResponseGenerator {
  ...
  async generateResponse(userMessage, userId, conversationHistory = []) {
    ...
    switch (requestType) {
      ...
      case "where_am_i":
        ...
        break;
      case "meme_response":
        response = this.generateMemeResponse(userMessage);
        break;
      default:
  ...
  analyzeRequestType(message, context) {
    ...
    // Специальная обработка для мемов лагеря
    const memeTriggers = [
      "сырный наггетс", "чокопай", "лёня", "шапочку всем", "болт никому",
      "преодолеваем инерцию", "борщ", "анлак", "дискотека зашквар", "сигма",
      "мне сказали вам не говорить", "иначе всё пропало", "тот самый борщ",
      "бро это анлак", "лёня так крут", "лёня сам снится снам"
    ];
    if (memeTriggers.some(trigger => messageLower.includes(trigger))) {
  ...
  // Генерирует ответ на мемы лагеря
  generateMemeResponse(message) {
    const messageLower = message.toLowerCase();
    
    // Проверяем конкретные мемы
    if (messageLower.includes("сырный наггетс")) {
      return "Мне правда нельзя рассказывать. Дело в том что это очень интересная и очень важная тайна! 😶\nМне велели молчать. Шапочку всем, болт никому! 😎";
    }
    
    if (messageLower.includes("чокопай")) {
      return "Мне правда нельзя рассказывать. Дело в том что это очень интересная и очень важная тайна! 😶\nМне велели молчать. Шапочку всем, болт никому! 😎";
    }
    
    if (messageLower.includes("лёня")) {
      return "Лёня так крут, что звонит 911 и спрашивает, что у них случилось! 🦸‍♂️\nЛёня сам снится снам!\nВсе мемы про Чака Норриса на самом деле про Лёню! 😎";
    }
    
    if (messageLower.includes("шапочку всем") || messageLower.includes("болт никому")) {
      return "Шапочку всем, болт никому! 😎\nМне правда нельзя рассказывать. Дело в том что это очень интересная и очень важная тайна! 😶";
    }
    
    if (messageLower.includes("борщ")) {
      return "Тот самый Борщ! 🍲\nМне правда нельзя рассказывать. Дело в том что это очень интересная и очень важная тайна! 😶";
    }
    
    if (messageLower.includes("анлак")) {
      return "Бро, это анлак! 😎\nМне правда нельзя рассказывать. Дело в том что это очень интересная и очень важная тайна! 😶";
    }
    
    if (messageLower.includes("дискотека")) {
      return "Дискотека зашквар! 💃\nМне правда нельзя рассказывать. Дело в том что это очень интересная и очень важная тайна! 😶";
    }
    
    if (messageLower.includes("сигма")) {
      return "Сигма! 😎\nМне правда нельзя рассказывать. Дело в том что это очень интересная и очень важная тайна! 😶";
    }
